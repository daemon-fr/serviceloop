...
class RecoveryCurrentContractTest {
    ...
    @Test fun replacementRemovesUnownedBytesAcrossAllSixRootsButKeepsExternalFiles() = runBlocking {
        ...
        database.v16ServiceDao().renameCustomer("legacy-customer", "Changed after backup")
        recovery.restore(recovery.inspect(backup.bytes, password))
        assertEquals("Historical customer", database.v16ServiceDao().customer("legacy-customer")!!.name)
    ...
    @Test fun duplicateCurrentTableFailsBeforeReplacingDurableRows() = runBlocking {
        ...
        assertEquals("Historical customer", database.v16ServiceDao().customer("legacy-customer")!!.name)
        assertNull(database.v16ServiceDao().customer("unexpected"))
    }
    ...
    @Test fun unsupportedVersionsAndMalformedCurrentRowsRejectBeforeAdoption() = runBlocking {
        ...
        malformed.forEachIndexed { index, bytes ->
            ...
            assertEquals("Historical customer", database.v16ServiceDao().customer("legacy-customer")!!.name)
            assertEquals("legacy-customer", database.v16ServiceDao().site("legacy-site")!!.customerId)
        }
    }
    ...
    @Test fun concurrentBusinessWriteAfterBackupSnapshotRejectsMixedArchive() = runBlocking {
        ...
        assertEquals("Updated during backup", database.v16ServiceDao().customer("legacy-customer")!!.name)
