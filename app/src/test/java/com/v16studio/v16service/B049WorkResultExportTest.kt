...
class B049WorkResultExportTest {
    ...
    @Test fun issuerTrustAndDocumentingIdentityGateExportAndRevisionIdentityIsStable() = runTest {
        ...
        V16ServicePeerTrustStore(database).add(issuer, "Coordinator")
        val firstBytes = exchange.exportFinalRevisions(listOf("revision"))
        assertEquals(1, V16ServiceSyncEnvelopeCodec.decode(firstBytes).manifest.sections.first().version)
    ...
    @Test fun resultPhotosComeOnlyFromFrozenRevisionAndKeepIndependentFlags() = runTest {
        ...
        val repository = RoomV16ServiceRepository(database, object : com.v16studio.v16service.domain.BusinessTime {
    ...
    @Test fun firstExportPersistsOneCanonicalDerivativeAcrossServiceRestartAndNativeRelay() = runTest {
        ...
        database.v16ServiceDao().insertFinalPhotos(listOf(FinalPhotoEntryEntity("canonical-final", "final-work", 1,
        ...
        val retained = database.v16ServiceDao().retainedImage("FINAL_PHOTO", "canonical-final")!!
        ...
        database.v16ServiceDao().updateRetainedImage(retained.copy(originalDeletionRequestedAtEpochMillis = 3, originalDeletedAtEpochMillis = 4))
    ...
    @Test fun directPerformedAndWorkResultRelayHaveOneSourceFingerprint() = runTest {
        ...
        database.v16ServiceDao().insertFinalPhotos(listOf(FinalPhotoEntryEntity("bridge-final-photo", "final-work", 1,
        ...
        val context = ApplicationProvider.getApplicationContext<Context>()
        val coordinator = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        val receiver = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        ...
        try {
            V16ServiceDatabase.configureStage4Tracking(coordinator.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(coordinator.openHelper.writableDatabase)
            V16ServiceDatabase.configureStage4Tracking(receiver.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(receiver.openHelper.writableDatabase)
            val b = coordinator.v16ServiceDao()
            ...
            V16ServicePeerTrustStore(coordinator).add(technician, "Technician")
            ...
            assertEquals(1, effectiveImportedFinalResults(
                listOf(received.single().copy(sourcePayloadJson = privateSource.toString())),
                transferredResults).size)
            val privatelyCarried = JSONObject(transferredResults.single().sourcePayloadJson!!)
            privatelyCarried.getJSONObject("record").put("internalNotes", JSONObject.NULL)
                .put("finalInternalNote", JSONObject.NULL)
            assertThrows(IllegalArgumentException::class.java) {
                effectiveImportedFinalResults(listOf(received.single().copy(sourcePayloadJson = privateSource.toString())),
                    listOf(transferredResults.single().copy(sourcePayloadJson = privatelyCarried.toString())))
            }
            assertEquals(1, effectiveImportedFinalResults(
            ...
            V16ServicePeerTrustStore(receiver).add(technician, "Technician")
            V16ServicePeerTrustStore(receiver).add(issuer, "Coordinator")
