...
import com.v16studio.v16service.ui.validateHistoryDates
import com.v16studio.v16service.ui.servicePlanDueLabel
...
class DailyOperationsIntegrityTest {
    ...
    @Before fun setup() { db = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build(); V16ServiceDatabase.configureStage4Tracking(db.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(db.openHelper.writableDatabase); root = File(context.cacheDir, "sl3-${System.nanoTime()}").apply { mkdirs() } }
    ...
    @Test fun directoryCreationAndEditRetainStableIdentityAndPlanCreatesExactlyOneObligation() = runTest {
        ...
        assertEquals(1, db.v16ServiceDao().obligationCount(plan)); assertEquals(db.v16ServiceDao().plan(plan)!!.currentDueDate, db.v16ServiceDao().obligation(db.v16ServiceDao().plan(plan)!!.currentObligationId!!)!!.dueDate)
    }
    ...
    @Test fun customerAndRequiredFirstSiteCommitAtomicallyWithInheritedContact() = runTest {
        ...
        val customer = db.v16ServiceDao().customer(customerId)!!
        val site = db.v16ServiceDao().site(siteId)!!
    ...
    @Test fun dueDateEditUpdatesSameObligationWithoutConsumingIt() = runTest {
        val ids = foundation(); val before = db.v16ServiceDao().plan(ids.plan)!!; repo.updatePlan(ids.plan, PlanInput("Annual service", 1, "YEARS", "2026-10-10", dueDateChangeReason = "Customer requested schedule change")); val after = db.v16ServiceDao().plan(ids.plan)!!
        assertEquals(before.currentObligationId, after.currentObligationId); assertEquals(1, db.v16ServiceDao().obligationCount(ids.plan)); assertNull(db.v16ServiceDao().obligation(after.currentObligationId!!)!!.consumedAtEpochMillis); assertEquals("2026-10-10", db.v16ServiceDao().obligation(after.currentObligationId!!)!!.dueDate)
    }
    ...
    @Test fun bookedVisitCapturesLatestTemplateAtStartAndWorkingSnapshotsStayFrozen() = runTest {
        ...
        val firstVisit = repo.createVisit(listOf(ids.plan), "BOOKED", "2026-09-06", Instant.parse("2026-09-06T08:00:00Z").toEpochMilli()); val firstWork = db.v16ServiceDao().firstWorkItemId(firstVisit)!!
        ...
        assertEquals("Old wording",db.v16ServiceDao().checklistItems(oldSnapshot).single().label); assertEquals("New wording", repo.inspection(firstWork)!!.questions.single().label); repo.saveResponse(firstWork,repo.inspection(firstWork)!!.questions.single().snapshotItemId,ResponseDisposition.VALUE,"2.5",null);repo.markChecklistReviewed(firstWork);repo.savePublicWork(firstWork,"Checked");repo.saveCompletionDraft(firstWork,"PERFORMED",false,null,null,null,null);repo.saveBusinessProfile(BusinessProfile("Service Co","Alex",zoneId="Europe/Bucharest"));val record=repo.finalRecord((repo.finalizeVisit(firstVisit) as FinalizeResult.Success).recordId)!!;assertEquals("New wording",record.public.lines.single().checklist.single().label); assertEquals(3, db.v16ServiceDao().reusableTemplateRevisions(template).size)
    }

    @Test fun preCorrectionBookedSnapshotAtSameRevisionIsReusedAtStart() = runTest {
        val ids=foundation(); val template=repo.createTemplate("Legacy booked",listOf(TemplateItemDraft("Revision one check","STATUS",required=true))); repo.updatePlan(ids.plan,PlanInput("Annual service",1,"YEARS","2026-09-01",template)); val visit=repo.createVisit(listOf(ids.plan),"BOOKED","2026-09-06",1); val work=db.v16ServiceDao().firstWorkItemId(visit)!!; val captured=db.v16ServiceDao().workItem(work)!!.capturedObligationId!!; val snapshot=attachPreCorrectionBookedSnapshot(visit,ids.plan,template); val snapshotCount=db.v16ServiceDao().templateSnapshotCount(); val itemCount=db.v16ServiceDao().checklistSnapshotItemCount()
        repo.startVisit(visit)
        assertEquals("WORKING",repo.visit(visit)!!.state); assertEquals(snapshot,db.v16ServiceDao().workItem(work)!!.templateSnapshotId); assertEquals(snapshotCount,db.v16ServiceDao().templateSnapshotCount()); assertEquals(itemCount,db.v16ServiceDao().checklistSnapshotItemCount()); assertEquals("Revision one check",repo.inspection(work)!!.questions.single().label); assertEquals(1,db.v16ServiceDao().templateSnapshot(snapshot)!!.revision); assertEquals(captured,db.v16ServiceDao().workItem(work)!!.capturedObligationId); assertEquals(1,db.v16ServiceDao().visitOwnsClaim(visit,captured))
    }

    @Test fun recordPastRecurringWorkIsHistoryOnlyAndFinalizationDoesNotTouchCurrentObligation() = runTest {
        val ids=foundation(); repo.saveBusinessProfile(BusinessProfile("Service Co","Alex",zoneId="Europe/Bucharest")); val before=db.v16ServiceDao().plan(ids.plan)!!
        val visit=repo.createVisit(listOf(ids.plan),"HISTORICAL","2026-08-01"); val work=db.v16ServiceDao().firstWorkItemId(visit)!!
        assertNull(repo.dueServices().single().claimedVisitId); assertNull(db.v16ServiceDao().workItem(work)!!.capturedObligationId)
        ...
        val record=repo.finalRecord((repo.finalizeVisit(visit) as FinalizeResult.Success).recordId)!!; val after=db.v16ServiceDao().plan(ids.plan)!!
    ...
    @Test fun obligationReplacementBeforeStartBlocksWithoutRetargeting() = runTest {
        val ids=foundation(); val visit=repo.createVisit(listOf(ids.plan),"BOOKED","2026-09-06",1); val captured=db.v16ServiceDao().visitWorkItems(visit).single().capturedObligationId!!
        db.v16ServiceDao().insertObligations(listOf(com.v16studio.v16service.data.ServiceObligationEntity("replacement",ids.plan,2,"2027-09-01",2))); db.v16ServiceDao().setCurrentObligationForTest(ids.plan,"replacement")
        assertTrue(runCatching{repo.startVisit(visit)}.isFailure); assertEquals("BOOKED",repo.visit(visit)!!.state); assertEquals(captured,db.v16ServiceDao().visitWorkItems(visit).single().capturedObligationId)
    }

    @Test fun finalizeWinningPreventsLaterPartMutation() = runTest {
        val ids=foundation(); repo.saveBusinessProfile(BusinessProfile("Service Co","Alex",zoneId="Europe/Bucharest")); val visit=repo.createVisit(listOf(ids.plan),"WORKING","2026-09-05"); val work=db.v16ServiceDao().firstWorkItemId(visit)!!; repo.savePublicWork(work,"Done"); repo.saveCompletionDraft(work,"PERFORMED",false,null,null,null,null)
        val entered=CompletableDeferred<Unit>(); val release=CompletableDeferred<Unit>(); val delayed=RoomV16ServiceRepository(db,time,DraftWriteGate{entered.complete(Unit);release.await()},attachmentRoot=root)
    ...
    @Test fun cancelWinningPreventsStaleStartAndResolveWinningPreventsStaleFollowUpEdit() = runTest {
        val ids=foundation(); val visit=repo.createVisit(listOf(ids.plan),"BOOKED","2026-09-06",1); val entered=CompletableDeferred<Unit>(); val release=CompletableDeferred<Unit>(); val delayed=RoomV16ServiceRepository(db,time,DraftWriteGate{entered.complete(Unit);release.await()},attachmentRoot=root)
        ...
        val follow=repo.createFollowUp(FollowUpInput("CONTACT","Call","2026-09-06",ids.customer)); val editEntered=CompletableDeferred<Unit>(); val editRelease=CompletableDeferred<Unit>(); val delayedEdit=RoomV16ServiceRepository(db,time,DraftWriteGate{editEntered.complete(Unit);editRelease.await()},attachmentRoot=root)
    ...
    @Test fun finalizeWinningPreventsLatePhotoAndPublicWorkWritesAndCleansOwnedFile() = runTest {
        suspend fun readyVisit():Pair<String,String>{val ids=foundation();repo.saveBusinessProfile(BusinessProfile("Service Co","Alex",zoneId="Europe/Bucharest"));val visit=repo.createVisit(listOf(ids.plan),"WORKING","2026-09-05");val work=db.v16ServiceDao().firstWorkItemId(visit)!!;repo.savePublicWork(work,"Original");repo.saveCompletionDraft(work,"PERFORMED",false,null,null,null,null);return visit to work}
        val (photoVisit,photoWork)=readyVisit(); val entered=CompletableDeferred<Unit>(); val release=CompletableDeferred<Unit>(); val delayed=RoomV16ServiceRepository(db,time,DraftWriteGate{entered.complete(Unit);release.await()},attachmentRoot=root)
        ...
        val (workVisit,workItem)=readyVisit(); val workEntered=CompletableDeferred<Unit>(); val workRelease=CompletableDeferred<Unit>(); val delayedWork=RoomV16ServiceRepository(db,time,DraftWriteGate{workEntered.complete(Unit);workRelease.await()},attachmentRoot=root)
    ...
    @Test fun templateReorderPublishesNewRevisionWithoutChangingPriorOrder() = runTest {
        val template=repo.createTemplate("Order",listOf(TemplateItemDraft("First","STATUS"),TemplateItemDraft("Second","TEXT"))); val oldId=db.v16ServiceDao().reusableTemplate(template)!!.currentRevisionId
        repo.reviseTemplate(template,"Order",listOf(TemplateItemDraft("Second","TEXT"),TemplateItemDraft("First","STATUS"))); val current=db.v16ServiceDao().reusableTemplate(template)!!.currentRevisionId
        assertEquals(listOf("First","Second"),db.v16ServiceDao().reusableTemplateItems(oldId).map{it.label}); assertEquals(listOf("Second","First"),db.v16ServiceDao().reusableTemplateItems(current).map{it.label})
    }

    @Test fun bookingClaimsAreExclusiveAndCancellationReleasesWithoutChangingDueDate() = runTest {
        val ids = foundation(); val dueBefore = db.v16ServiceDao().plan(ids.plan)!!.currentDueDate; val visit = repo.createVisit(listOf(ids.plan), "BOOKED", "2026-09-06", 1)
        runCatching { repo.createVisit(listOf(ids.plan), "BOOKED", "2026-09-07", 2) }.onSuccess { fail("Expected exclusive claim") }
        repo.rescheduleVisit(visit, "2026-09-09", 3, "Customer requested another day"); assertEquals(dueBefore, db.v16ServiceDao().plan(ids.plan)!!.currentDueDate)
        repo.cancelVisit(visit, "Customer unavailable"); assertEquals(dueBefore, db.v16ServiceDao().plan(ids.plan)!!.currentDueDate); assertNull(repo.dueServices().single().claimedVisitId)
        assertEquals(setOf("RESCHEDULED", "CANCELED"), db.v16ServiceDao().visitScheduleEvents(visit).map { it.eventType }.toSet())
    ...
    @Test fun appointmentTimePersistsReschedulesClearsAndRestoresWithoutMidnightSurrogate() = runTest {
        ...
        assertEquals(time.zoneId.id, db.v16ServiceDao().visit(visit)!!.appointmentZoneId)
        ...
        val rescheduled = db.v16ServiceDao().visitScheduleEvents(visit).last()
        ...
        val cleared = db.v16ServiceDao().visitScheduleEvents(visit).last()
        ...
        assertNull(repo.visit(visit)!!.scheduledAtEpochMillis)
        assertEquals("RESTORED", db.v16ServiceDao().visitScheduleEvents(visit).last().eventType)
    }

    @Test fun cancelledBookingRestoresSameVisitAndClaimWithoutChangingDueDate() = runTest {
        val ids=foundation(); val due=db.v16ServiceDao().plan(ids.plan)!!.currentDueDate; val visit=repo.createVisit(listOf(ids.plan),"BOOKED","2026-09-06",1); val obligation=db.v16ServiceDao().visitWorkItems(visit).single().capturedObligationId!!
        repo.cancelVisit(visit,"Mistake"); repo.restoreVisit(visit,"2026-09-06")
        assertEquals("BOOKED",repo.visit(visit)!!.state); assertEquals(1,db.v16ServiceDao().visitOwnsClaim(visit,obligation)); assertEquals(due,db.v16ServiceDao().plan(ids.plan)!!.currentDueDate); assertEquals(listOf("CANCELED","RESTORED"),db.v16ServiceDao().visitScheduleEvents(visit).map{it.eventType})
    }

    @Test fun restoreRejectsStaleOrAlreadyClaimedObligationAtomically() = runTest {
        val ids=foundation(); val visit=repo.createVisit(listOf(ids.plan),"BOOKED","2026-09-06",1); repo.cancelVisit(visit,"Mistake"); val old=db.v16ServiceDao().visitWorkItems(visit).single().capturedObligationId!!
        db.v16ServiceDao().insertObligations(listOf(com.v16studio.v16service.data.ServiceObligationEntity("replacement",ids.plan,2,"2027-09-01",2))); db.v16ServiceDao().setCurrentObligationForTest(ids.plan,"replacement")
        assertTrue(runCatching{repo.restoreVisit(visit,"2026-09-06")}.isFailure); assertEquals("CANCELED",repo.visit(visit)!!.state); assertNull(db.v16ServiceDao().claimForObligation(old))
    }
    ...
    @Test fun oneOffOnlyCancelledVisitRestoresWithoutClaims() = runTest {
        ...
        assertEquals("BOOKED",repo.visit(visit)!!.state); assertTrue(db.v16ServiceDao().visitWorkItems(visit).all{it.capturedObligationId==null})
    }
    ...
    @Test fun restoreRejectsConsumedOrInactivePlanAndConcurrentAttemptsLeaveOneClaim() = runTest {
        ...
        val (consumedIds, consumedVisit)=cancelled(); val obligation=db.v16ServiceDao().visitWorkItems(consumedVisit).single().capturedObligationId!!; db.v16ServiceDao().consumeObligation(obligation,consumedIds.plan,2,"revision"); assertTrue(runCatching { repo.restoreVisit(consumedVisit,"2026-09-06") }.isFailure); assertEquals("CANCELED",repo.visit(consumedVisit)!!.state)
        val (inactiveIds,inactiveVisit)=cancelled(); db.v16ServiceDao().updatePlan(db.v16ServiceDao().plan(inactiveIds.plan)!!.copy(state="INACTIVE")); assertTrue(runCatching { repo.restoreVisit(inactiveVisit,"2026-09-06") }.isFailure); assertEquals("CANCELED",repo.visit(inactiveVisit)!!.state)
        val (raceIds,raceVisit)=cancelled(); val attempts=listOf(async { runCatching{repo.restoreVisit(raceVisit,"2026-09-06")} },async { runCatching{repo.restoreVisit(raceVisit,"2026-09-06")} }).awaitAll(); assertEquals(1,attempts.count{it.isSuccess}); val claimed=db.v16ServiceDao().visitWorkItems(raceVisit).single().capturedObligationId!!; assertEquals(1,db.v16ServiceDao().visitOwnsClaim(raceVisit,claimed)); assertEquals("BOOKED",repo.visit(raceVisit)!!.state)
    }
    ...
    @Test fun oneVisitRejectsPlansFromDifferentSites() = runTest {
        ...
        val failure = runCatching { repo.createVisit(listOf(first.plan, secondPlan), "WORKING", "2026-09-05") }.exceptionOrNull(); assertTrue(failure is IllegalArgumentException); assertEquals(0, db.v16ServiceDao().visitCount())
    }
    ...
    @Test fun bookedStartRequiresClaimStillOwnedByThatVisit() = runTest {
        val ids=foundation(); val visit=repo.createVisit(listOf(ids.plan),"BOOKED","2026-09-06",1); db.v16ServiceDao().releaseVisitClaims(visit)
    ...
    @Test fun multiMachineFinalizationAdvancesOnlyTheExplicitlyFulfilledLine() = runTest {
        ...
        val visit=repo.createVisit(listOf(ids.plan,planB),"WORKING","2026-09-05"); val lines=db.v16ServiceDao().visitWorkItems(visit).associateBy{it.servicePlanId}
        ...
        val oldB=db.v16ServiceDao().plan(planB)!!.currentObligationId; val result=repo.finalizeVisit(visit) as FinalizeResult.Success; val record=repo.finalRecord(result.recordId)!!
        assertEquals("2027-09-05",db.v16ServiceDao().plan(ids.plan)!!.currentDueDate); assertEquals(oldB,db.v16ServiceDao().plan(planB)!!.currentObligationId); assertNull(db.v16ServiceDao().obligation(oldB!!)!!.consumedAtEpochMillis)
    ...
    @Test fun contactAndFollowUpLifecycleHaveNoServiceSideEffects() = runTest {
        ...
        assertEquals(obligation, db.v16ServiceDao().plan(ids.plan)!!.currentObligationId); assertNull(db.v16ServiceDao().obligation(obligation!!)!!.consumedAtEpochMillis); assertEquals("CANCELLED", repo.followUp(follow)!!.state)
    }

    @Test fun partsAndSelectedPhotosAreFrozenIntoFinalPublicSnapshot() = runTest {
        val ids = foundation(); repo.saveBusinessProfile(BusinessProfile("Service Co", "Alex", zoneId = "Europe/Bucharest")); val visit = repo.createVisit(listOf(ids.plan), "WORKING", "2026-09-05"); val work = db.v16ServiceDao().firstWorkItemId(visit)!!
        ...
        assertFalse(db.v16ServiceDao().attachment(shownPhotoId)!!.includedInCustomerReport)
        ...
        val finalWork = db.v16ServiceDao().finalWorkItems(record.public.revisionId).single()
        val frozen = db.v16ServiceDao().finalPhotos(finalWork.id)
    ...
    @Test fun finalizationWaitsForPhotoInclusionCheckpointAndFreezesItsSavedTruth() = runTest {
        ...
        val visit = repo.createVisit(listOf(ids.plan), "WORKING", "2026-09-05"); val work = db.v16ServiceDao().firstWorkItemId(visit)!!
        ...
        val delayed = RoomV16ServiceRepository(db, time, DraftWriteGate { writeEntered.complete(Unit); releaseWrite.await() }, attachmentRoot = root)
        ...
        val finalWork = db.v16ServiceDao().finalWorkItems(revisionId).single()
        assertEquals(listOf(photoId), db.v16ServiceDao().finalPhotos(finalWork.id).map { it.sourceAttachmentId })
        assertTrue(db.v16ServiceDao().attachment(photoId)!!.includedInCustomerReport)
    }
    ...
    @Test fun untouchedRecurringAndAdHocServicesStayNotStartedAndNewRowsAreUnresolved() = runTest {
        ...
        val recurring = db.v16ServiceDao().firstWorkItemId(recurringVisit)!!
        assertNull(db.v16ServiceDao().workItem(recurring)!!.fulfillsCurrentObligation)
        ...
        val adHoc = db.v16ServiceDao().firstWorkItemId(adHocVisit)!!
        assertNull(db.v16ServiceDao().workItem(adHoc)!!.fulfillsCurrentObligation)
    ...
    @Test fun legacyFalseWithoutOutcomeDoesNotCreateActivityOrResumePriority() = runTest {
        ...
        val items = db.v16ServiceDao().visitWorkItems(visit)
        db.v16ServiceDao().updateWorkItem(items[1].copy(outcome = null, fulfillsCurrentObligation = false))
    ...
    @Test fun performedReadinessRequiresChecklistButAutomaticallyFulfillsEligibleCurrentWork() = runTest {
        ...
        val work = db.v16ServiceDao().firstWorkItemId(visit)!!
    ...
    @Test fun reviewProjectsNaturalChecklistResultsAndHidesUnansweredOptionalAndPrivateDrafts() = runTest {
        ...
        val activeAnswer = db.v16ServiceDao().responses(work).first { it.checklistItemSnapshotId == questions.getValue("Power cable").snapshotItemId }
        db.v16ServiceDao().updateResponse(activeAnswer.id, "OK", null, null, null, activeAnswer.modifiedAtEpochMillis, "INACTIVE_ISSUE_PRIVATE", "INACTIVE_NA_PRIVATE")
    ...
    @Test fun partlyPerformedIsReadyWithoutBlanketChecklistGateAndNeedsWorkWhenMissing() = runTest {
        ...
        assertEquals("2026-09-01", db.v16ServiceDao().plan(ids.plan)!!.currentDueDate)
    ...
    @Test fun invalidExplicitAnswerBlocksPartlyAndNotPerformedReadiness() = runTest {
        ...
        val work = db.v16ServiceDao().firstWorkItemId(visit)!!
        val question = repo.inspection(work)!!.questions.single()
        db.v16ServiceDao().persistResponse(WorkingResponseEntity("invalid", work, question.snapshotItemId, "OK", null, null, null, 1), visit)
        ...
        val secondWork = db.v16ServiceDao().firstWorkItemId(secondVisit)!!
        val secondItem = db.v16ServiceDao().workItem(secondWork)!!
        val snapshot = TemplateSnapshotEntity("snapshot-invalid-not", null, "Ad hoc text inspection", 1, 1)
        db.v16ServiceDao().insertTemplateSnapshots(listOf(snapshot))
        db.v16ServiceDao().insertChecklistItems(listOf(ChecklistItemSnapshotEntity("question-invalid-not", snapshot.id, 1, "Reading", "TEXT", null, false, null)))
        db.v16ServiceDao().updateWorkItem(secondItem.copy(templateSnapshotId = snapshot.id))
        db.v16ServiceDao().persistResponse(WorkingResponseEntity("invalid-not", secondWork, "question-invalid-not", "OK", null, null, null, 1), secondVisit)
    ...
    @Test fun nextServiceSkipsReadyPartlyAndNotPerformedButKeepsUntouchedCandidate() = runTest {
        ...
        val first = db.v16ServiceDao().visitWorkItems(visit).first().id
        val second = db.v16ServiceDao().visitWorkItems(visit).last().id
        ...
        val notPerformed = db.v16ServiceDao().visitWorkItems(secondVisit).first().id
        val untouched = db.v16ServiceDao().visitWorkItems(secondVisit).last().id
    ...
    @Test fun savedPrivateNoteIsReadByFreshRepositoryWhileVisitRemainsWorking() = runTest {
        ...
        val freshReader = RoomV16ServiceRepository(db, time, attachmentRoot = root)
    ...
    @Test fun workingEvidenceMutationsPersistAndFinalEvidenceRejectsWorkingEdits() = runTest {
        ...
        val path = db.v16ServiceDao().attachment(photo)!!.storedRelativePath
    ...
    @Test fun failedPhotoMetadataWriteDoesNotLeaveAFalseSavedFile() = runTest {
        val ids=foundation(); val visit=repo.createVisit(listOf(ids.plan),"WORKING","2026-09-05"); val work=db.v16ServiceDao().firstWorkItemId(visit)!!
    ...
    @Test fun batchPhotoVisibilityUsesCanonicalPathAndDeleteRollsBackAllFilesOnDatabaseFailure() = runTest {
        ...
        val files = listOf(first, second).map { id -> File(root, db.v16ServiceDao().attachment(id)!!.storedRelativePath) }
    ...
    @Test fun missingSelectedPhotoBlocksFinalizationWithoutChangingVisitOrObligation() = runTest {
        val ids=foundation(); repo.saveBusinessProfile(BusinessProfile("Service Co","Alex",zoneId="Europe/Bucharest")); val visit=repo.createVisit(listOf(ids.plan),"WORKING","2026-09-05"); val work=db.v16ServiceDao().firstWorkItemId(visit)!!; repo.savePublicWork(work,"Completed checks"); repo.saveCompletionDraft(work,"PERFORMED",false,null,null,null,null); val photoId=repo.savePhoto(work,testImageBytes(Color.YELLOW),"selected.png","image/png",true,"Required evidence"); File(root,db.v16ServiceDao().attachment(photoId)!!.storedRelativePath).delete()
        assertTrue(repo.finalizeVisit(visit) is FinalizeResult.Blocked); assertEquals("WORKING",repo.visit(visit)!!.state); assertNull(db.v16ServiceDao().obligation(db.v16ServiceDao().plan(ids.plan)!!.currentObligationId!!)!!.consumedAtEpochMillis)
    }
    ...
    @Test fun correctionAppendsImmutableRevisionAndTextOnlyChangeDoesNotAdvanceSchedule() = runTest {
        val ids = foundation(); val recordId = finalizedRecord(ids); val beforePlan = db.v16ServiceDao().plan(ids.plan)!!
        ...
        assertEquals(original.public.customerName, db.v16ServiceDao().finalRevision(original.public.revisionId)!!.customerName)
        assertEquals(original.public.revisionId, db.v16ServiceDao().finalRevision(correctedRevision)!!.supersedesRevisionId)
        assertEquals(beforePlan.currentDueDate, db.v16ServiceDao().plan(ids.plan)!!.currentDueDate)
    ...
    @Test fun correctionCanReplaceCapturedChecklistPartsAndSelectedEvidenceWithoutMutatingOriginal() = runTest {
        ...
        val visit = repo.createVisit(listOf(ids.plan), "WORKING", "2026-09-05"); val work = db.v16ServiceDao().firstWorkItemId(visit)!!; val question = repo.inspection(work)!!.questions.single()
        ...
        assertEquals("OK", db.v16ServiceDao().finalChecklistItems(original.public.lines.single().let { db.v16ServiceDao().finalWorkItems(original.public.revisionId).single().id }).single().disposition)
    }
    ...
    @Test fun correctionEvidenceIsNormalizedAndOpenDraftBackupRoundTripsWithDraftOwnership() = runTest {
        ...
        val evidence = db.v16ServiceDao().attachmentsForOwner("CORRECTION_DRAFT", draft.id).single(); val stored = File(root, evidence.storedRelativePath)
        ...
        val restored = db.v16ServiceDao().attachmentsForOwner("CORRECTION_DRAFT", draft.id).single()
    ...
    @Test fun discardedCorrectionEvidenceRemovesMetadataFileDirectoryAndStaysAbsentFromBackup() = runTest {
        ...
        val evidence = db.v16ServiceDao().attachmentsForOwner("CORRECTION_DRAFT", draft.id).single(); val file = File(root, evidence.storedRelativePath); val directory = file.parentFile!!
        assertTrue(repo.discardCorrection(recordId)); assertNull(db.v16ServiceDao().attachment(evidence.id)); assertFalse(file.exists()); assertFalse(directory.exists())
        ...
        repo.erase(true, "ERASE"); repo.restoreBackup(inspection, "REPLACE", false)
        assertNull(db.v16ServiceDao().attachment(evidence.id)); assertFalse(File(root, evidence.storedRelativePath).exists())
    }
    ...
    @Test fun committedCorrectionEvidenceGetsRevisionOwnershipAndRoundTripsSelectedAndUnselectedPolicy() = runTest {
        ...
        val revisionId = repo.commitCorrection(recordId); val owned = db.v16ServiceDao().attachmentsForOwner("FINAL_REVISION", revisionId)
        ...
        val finalPhotos = db.v16ServiceDao().finalPhotos(db.v16ServiceDao().finalWorkItems(revisionId).single().id)
        ...
        assertTrue(db.v16ServiceDao().attachmentsForOwner("CORRECTION_DRAFT", draft.id).isEmpty())
        ...
        repo.erase(true, "ERASE"); repo.restoreBackup(inspection, "REPLACE", false)
        assertEquals(2, db.v16ServiceDao().attachmentsForOwner("FINAL_REVISION", revisionId).size)
        assertEquals(2, db.v16ServiceDao().finalPhotos(db.v16ServiceDao().finalWorkItems(revisionId).single().id).count { it.addedInCorrection })
    }

    @Test fun backupInspectionRejectsAuthenticatedStructuralAndCurrentIdentityDamageWithoutMutatingLiveData() = runTest {
        val ids = foundation(); val password = "structural validation".toCharArray(); val dao = db.v16ServiceDao(); val visit = repo.createVisit(listOf(ids.plan), "BOOKED", "2026-09-06", 1); val work = dao.firstWorkItemId(visit)!!
    ...
    @Test fun correctionFollowUpEffectsAreExplicitAndAtomic() = runTest {
        val ids = foundation(); repo.saveBusinessProfile(BusinessProfile("Service Co","Alex",zoneId="Europe/Bucharest")); val visit=repo.createVisit(listOf(ids.plan),"WORKING","2026-09-05"); val work=db.v16ServiceDao().firstWorkItemId(visit)!!; val follow=repo.createCorrectiveFollowUp(work,"Inspect leak","2026-09-10",""); repo.savePublicWork(work,"Done"); repo.saveCompletionDraft(work,"PERFORMED",false,null,null,null,null); val record=(repo.finalizeVisit(visit) as FinalizeResult.Success).recordId
    ...
    @Test fun latestCountedCorrectionAndVoidReconcileScheduleAndProvenance() = runTest {
        ...
        val afterCorrection=db.v16ServiceDao().plan(ids.plan)!!; assertEquals("2026-09-01",afterCorrection.currentDueDate); assertNull(afterCorrection.lastCountedRevisionId)
        val nextRecord=finalizedRecord(ids); val currentRevision=repo.finalRecord(nextRecord)!!.public.revisionId; assertEquals(currentRevision,db.v16ServiceDao().plan(ids.plan)!!.lastCountedRevisionId)
        repo.voidRecord(nextRecord,"Service was attributed in error",null); val afterVoid=db.v16ServiceDao().plan(ids.plan)!!; assertEquals("2026-09-01",afterVoid.currentDueDate); assertNull(afterVoid.lastCountedRevisionId)
    }
    ...
    @Test fun recordsPackageContainsAdoptedReadableOwnershipTables() = runTest {
        ...
        assertTrue(readme.contains("V16 Service records export")); assertTrue(readme.contains("Generated with V16 Service")); assertTrue(readme.contains("Generated: 2026-09-05T10:00:00Z")); assertTrue(readme.contains("Scope: customer CU-001")); assertTrue(readme.contains("Include inactive: yes")); assertTrue(readme.contains("Include private data: no")); assertTrue(readme.contains("Include previous revisions: yes"))
        ...
        contents.filterKeys { it.endsWith(".csv") }.values.forEach { csv -> assertFalse(csv.toString(Charsets.UTF_8).contains("Generated with V16 Service")) }
    }
    ...
    @Test fun incompleteRestorePersistsMissingAttachmentAvailability() = runTest {
        ...
        val stored = db.v16ServiceDao().attachment(attachment)!!; File(root, stored.storedRelativePath).delete()
        ...
        assertEquals("MISSING", db.v16ServiceDao().attachment(attachment)!!.availability)
        assertTrue(db.v16ServiceDao().recoveryMetadata()!!.restoredFromIncompleteCopy)
    }
    ...
    @Test fun completeBackupRoundTripsCombinedSl4DispatchAndWorkingResponseDraftState() = runTest {
        ...
        val due=db.v16ServiceDao().plan(ids.plan)!!.currentDueDate;repo.updatePlan(ids.plan,PlanInput("Annual service",1,"YEARS",due,template))
        val workingVisit=repo.createVisit(listOf(ids.plan),"WORKING","2026-09-06");val work=db.v16ServiceDao().firstWorkItemId(workingVisit)!!;val question=repo.inspection(work)!!.questions.single()
        ...
        repo.erase(true,"ERASE");assertNull(db.v16ServiceDao().finalRecord(recordId));assertNull(db.dispatchDao().visitBinding("DV-INTEGRATED"));assertEquals(identity.technicianId,db.dispatchDao().technicianIdentity()!!.technicianId);assertTrue(db.dispatchDao().teams().isEmpty());assertTrue(db.dispatchDao().outboxVisits().isEmpty())
        repo.restoreBackup(inspection,"REPLACE",false)

        assertEquals(correction.id,db.v16ServiceDao().correctionDraftForRecord(recordId)!!.id);assertEquals(recordId,repo.finalRecord(recordId)!!.public.recordId)
    ...
    private suspend fun finalizedRecord(ids: Ids): String { repo.saveBusinessProfile(BusinessProfile("Service Co", "Alex", zoneId = "Europe/Bucharest")); val visit=repo.createVisit(listOf(ids.plan),"WORKING","2026-09-05"); val work=db.v16ServiceDao().firstWorkItemId(visit)!!; repo.savePublicWork(work,"Annual service completed"); repo.saveCompletionDraft(work,"PERFORMED",true,null,"2027-09-05",true,null); return (repo.finalizeVisit(visit) as FinalizeResult.Success).recordId }
    private suspend fun attachPreCorrectionBookedSnapshot(visit:String,plan:String,template:String):String { val dao=db.v16ServiceDao(); val master=dao.reusableTemplate(template)!!; val revision=dao.reusableTemplateRevision(master.currentRevisionId)!!; val snapshot=stableTestId("template-snapshot",visit,plan,revision.id); dao.insertTemplateSnapshots(listOf(com.v16studio.v16service.data.TemplateSnapshotEntity(snapshot,master.id,revision.nameSnapshot,revision.revisionNumber,1))); dao.insertChecklistItems(dao.reusableTemplateItems(revision.id).map{item->com.v16studio.v16service.data.ChecklistItemSnapshotEntity(stableTestId("snapshot-item",snapshot,item.id),snapshot,item.position,item.label,item.responseType,item.unit,item.required,item.privateGuidance)}); val work=dao.firstWorkItemId(visit)!!; db.openHelper.writableDatabase.execSQL("UPDATE work_items SET templateSnapshotId=? WHERE id=?",arrayOf(snapshot,work)); return snapshot }
