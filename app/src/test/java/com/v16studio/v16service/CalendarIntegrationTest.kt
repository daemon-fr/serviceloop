...
import com.v16studio.v16service.calendar.*
...
class CalendarIntegrationTest {
    ...
    @Before fun setUp(){context=ApplicationProvider.getApplicationContext();store=CalendarDeviceStore(context);store.resetForDatasetReplacement();db=Room.inMemoryDatabaseBuilder(context,V16ServiceDatabase::class.java).allowMainThreadQueries().build();gateway=FakeCalendarGateway();scope=CoroutineScope(SupervisorJob());coordinator=CalendarCoordinator(context,db,gateway,store,scope);runBlocking{seed("dataset-a")}}
    ...
    @Test fun roomObservationUpdatesAndCancelsWithoutExplicitReconcile()=runBlocking{
        ...
        val eventId=gateway.events.keys.single();val visit=db.v16ServiceDao().visit("visit")!!
        db.v16ServiceDao().updateVisit(visit.copy(scheduledAtEpochMillis=visit.scheduledAtEpochMillis!!+60_000,modifiedAtEpochMillis=2))
        await{gateway.updates==1};assertEquals(eventId,gateway.events.keys.single())
        db.v16ServiceDao().updateCustomer(db.v16ServiceDao().customer("customer")!!.copy(name="Updated Customer"));db.v16ServiceDao().updateSite(db.v16ServiceDao().site("site")!!.copy(name="Updated Site",address="2 New Road"))
        await{gateway.updates>=2&&gateway.events[eventId]?.title=="V16 Service · Updated Site"};assertEquals("2 New Road",gateway.events[eventId]!!.location);assertTrue(gateway.events[eventId]!!.description.contains("Updated Customer"))
        db.v16ServiceDao().updateVisit(db.v16ServiceDao().visit("visit")!!.copy(state="CANCELED",cancellationReason="Cancelled",cancelledAtEpochMillis=3))
    ...
    @Test fun createIsIdempotentAndUpdateUsesSameEvent()=runBlocking{
        ...
        val eventId=gateway.events.keys.single();val visit=db.v16ServiceDao().visit("visit")!!
        db.v16ServiceDao().updateVisit(visit.copy(scheduledAtEpochMillis=visit.scheduledAtEpochMillis!!+2*60*60*1000,modifiedAtEpochMillis=2))
    ...
    @Test fun datasetReplacementDropsBindingsWithoutDeletingExternalEvent()=runBlocking{
        ...
        db.v16ServiceDao().upsertRecoveryMetadata(metadata("dataset-b"));coordinator.reconcile()
    ...
    @Test fun sameDatasetAdoptionDropsCalendarBindingButRollbackKeepsIt()=runBlocking{
        ...
        val before=db.v16ServiceDao().recoveryMetadata()!!
        ...
        db.v16ServiceDao().upsertRecoveryMetadata(before.copy(adoptionToken="restored-copy"))
        coordinator.reconcile()
    ...
    @Test fun disableRetainsEventAndReenableUpdatesWithoutDuplicate()=runBlocking{
        ...
        val visit=db.v16ServiceDao().visit("visit")!!;db.v16ServiceDao().updateVisit(visit.copy(scheduledAtEpochMillis=visit.scheduledAtEpochMillis!!+60_000,modifiedAtEpochMillis=2));coordinator.reconcile()
    ...
    @Test fun cancellationFailureRetainsPendingLinkWithoutBusinessRollback()=runBlocking{
        ...
        val visit=db.v16ServiceDao().visit("visit")!!;db.v16ServiceDao().updateVisit(visit.copy(state="CANCELED",cancellationReason="Customer requested",cancelledAtEpochMillis=3));coordinator.reconcile()
        assertEquals("CANCELED",db.v16ServiceDao().visit("visit")!!.state);assertEquals(1,coordinator.runtimeState().problemCount);assertEquals(1,gateway.events.size)
    }

    @Test fun workingAndFinalizedStatesRetainHistoricalEvent()=runBlocking{
        coordinator.setEnabled(true);coordinator.select(gateway.calendar);val dao=db.v16ServiceDao();val visit=dao.visit("visit")!!
    ...
    @Test fun eventContentIsMinimalAndPrivateFieldsCannotLeak(){
        ...
        assertEquals("V16 Service · Site",event.title);assertEquals("1 Public Road",event.location);assertEquals("Visit V-1\nCustomer",event.description);assertEquals(60*60*1000,event.endMillis-event.startMillis)
        val rendered=listOf(event.title,event.location,event.description).joinToString(" ")
        listOf("secret access","private customer","dispatch instruction","phone@example","invalid-redaction-sentinel").forEach{assertFalse(rendered.contains(it))}
    }

    private suspend fun seed(dataset:String){val dao=db.v16ServiceDao();dao.upsertRecoveryMetadata(metadata(dataset));dao.insertCustomers(listOf(CustomerEntity("customer","CU-1","Customer",privateNote="private customer")));dao.insertSites(listOf(SiteEntity("site","customer","ST-1","Site","1 Public Road","secret access")));dao.insertVisits(listOf(visit()))}
...
class CalendarDispatchObservationTest {
    private lateinit var context:Context;private lateinit var db:V16ServiceDatabase;private lateinit var store:CalendarDeviceStore;private lateinit var gateway:FakeCalendarGateway;private lateinit var coordinator:CalendarCoordinator;private lateinit var scope:CoroutineScope;private lateinit var dispatch:DispatchPackageService
    @Before fun setup()=runBlocking{context=ApplicationProvider.getApplicationContext();store=CalendarDeviceStore(context);store.resetForDatasetReplacement();db=Room.inMemoryDatabaseBuilder(context,V16ServiceDatabase::class.java).allowMainThreadQueries().build();db.v16ServiceDao().upsertRecoveryMetadata(metadata("dispatch-calendar"));gateway=FakeCalendarGateway();scope=CoroutineScope(SupervisorJob());coordinator=CalendarCoordinator(context,db,gateway,store,scope);dispatch=DispatchPackageService(db,context.cacheDir);coordinator.setEnabled(true);coordinator.select(gateway.calendar);coordinator.start()}
    ...
    @Test fun dispatchCreateGenerationUpdateAndWithdrawalProjectAutomatically()=runBlocking{
        ...
        dispatch.import(dispatch.preview(withdrawn(newer,3)),dispatch.identity().technicianId);await{gateway.events.isEmpty()&&store.read("dispatch-calendar").links.isEmpty()};assertEquals("CANCELED",db.v16ServiceDao().visit(local)!!.state)
    }
    ...
    @Test fun withdrawalDeleteFailureKeepsBusinessStateAndRetriesOnLaterInvalidation()=runBlocking{
        ...
        assertEquals("CANCELED",db.v16ServiceDao().visit(local)!!.state);assertEquals(1,gateway.events.size)
        gateway.failDeletes=false;val visit=db.v16ServiceDao().visit(local)!!;db.v16ServiceDao().updateVisit(visit.copy(modifiedAtEpochMillis=visit.modifiedAtEpochMillis+1));await{gateway.events.isEmpty()&&local !in store.read("dispatch-calendar").links}
    }
