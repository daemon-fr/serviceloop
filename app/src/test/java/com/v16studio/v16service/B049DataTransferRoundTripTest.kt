...
class B049DataTransferRoundTripTest {
    ...
    private lateinit var a: V16ServiceDatabase
    private lateinit var b: V16ServiceDatabase
    private lateinit var rootA: File
    private lateinit var rootB: File

    @Before fun setup() {
        a = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        b = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        V16ServiceDatabase.configureStage4Tracking(a.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(a.openHelper.writableDatabase)
        V16ServiceDatabase.configureStage4Tracking(b.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(b.openHelper.writableDatabase)
        V16ServiceDatabase.configureReminderDefaults(a.openHelper.writableDatabase)
        V16ServiceDatabase.configureReminderDefaults(b.openHelper.writableDatabase)
    ...
    private suspend fun trustedReceiver(sourceId: String) {
        V16ServicePeerTrustStore(b).add(sourceId, "Workspace A")
    }

    @Test fun sameSourceIdAcrossNativeEntityTypesKeepsItsHierarchy() = runBlocking {
        val source = a.v16ServiceDao()
        ...
        val sourceId = V16ServicePeerTrustStore(a).localIdentity().technicianId
    ...
    @Test fun contactNoteDateSelectionUsesBusinessZoneAtMidnight() = runBlocking {
        ...
        suspend fun count(date: String): Int {
            val scope = V16ServiceScopeFilter(customerId = "c", fromDate = LocalDate.parse(date), toDate = LocalDate.parse(date))
    ...
    private suspend fun seedCompletedWorkWithEvidence(): String {
        ...
        return V16ServicePeerTrustStore(a).localIdentity().technicianId
    }
    ...
    @Test fun performedWorkAndStandaloneImageArchiveStayImmutableAndRelayOriginalOrigin() = runBlocking {
        ...
        val visit = AggregateReportService(b, RoomV16ServiceRepository(b, ClockBusinessTime(java.time.Clock.fixed(Instant.parse("2026-09-24T10:00:00Z"), java.time.ZoneId.of("UTC")), java.time.ZoneId.of("UTC")), attachmentRoot = rootB), rootB)
        val reportable = visit.reportable(V16ServiceScopeFilter(customerId = dao.allCustomers().single().id))
        ...
        var rendered: List<com.v16studio.v16service.domain.PublicReportModel> = emptyList()
        val reports = AggregateReportService(b, RoomV16ServiceRepository(b, ClockBusinessTime(java.time.Clock.systemUTC(), java.time.ZoneId.of("UTC")), attachmentRoot = rootB), rootB,
            AggregateReportWriter { models, _, _, _, target, _ -> rendered = models; target.writeBytes(byteArrayOf(1, 2, 3)); 1 })
        reports.generate(V16ServiceScopeFilter(customerId = dao.allCustomers().single().id), listOf(reportable.single().key))
        ...
        val restored = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        try {
            ...
            assertEquals(dao.allDataTransferBindings(), restored.v16ServiceDao().allDataTransferBindings())
            assertEquals(dao.allTransferredFinalResults(), restored.v16ServiceDao().allTransferredFinalResults())
            assertEquals(dao.allTransferredEvidence(), restored.v16ServiceDao().allTransferredEvidence())
            assertTrue(restored.v16ServiceDao().allTransferredEvidence().all { File(restoreRoot, it.relativePath).isFile })
        ...
        val comparison = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        val comparisonRoot = File(context.cacheDir, "b049-transfer-compare-${System.nanoTime()}").apply { mkdirs() }
        try {
            V16ServiceDatabase.configureStage4Tracking(comparison.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(comparison.openHelper.writableDatabase)
            V16ServicePeerTrustStore(comparison).add(sourceId, "Original workspace")
            V16ServicePeerTrustStore(comparison).add(relay.exporterId, "Relay workspace")
        ...
        val standaloneB = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        val rootStandalone = File(context.cacheDir, "b049-transfer-img-${System.nanoTime()}").apply { mkdirs() }
        try {
            V16ServiceDatabase.configureStage4Tracking(standaloneB.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(standaloneB.openHelper.writableDatabase)
            V16ServicePeerTrustStore(standaloneB).add(sourceId, "Workspace A")
            ...
            assertEquals(0, standaloneB.v16ServiceDao().allVisits().size)
            assertEquals(2, standaloneB.v16ServiceDao().allTransferredEvidence().size)
            assertTrue(standaloneB.v16ServiceDao().allTransferredEvidence().all { File(rootStandalone, it.relativePath).isFile })
    ...
    @Test fun evidenceAndPerformedHistoryAssociateInEitherArrivalOrder() = runBlocking {
        ...
        suspend fun importInto(db: V16ServiceDatabase, root: File, bytes: ByteArray) {
            val service = DataTransferImportService(db, root)
            service.import(service.preview(bytes))
        }
        trustedReceiver(sourceId)
        importInto(b, rootB, evidence)
        assertEquals(2, b.v16ServiceDao().allTransferredEvidence().size)
        assertTrue(b.v16ServiceDao().allTransferredEvidence().all { it.transferredFinalResultId == null })
        ...
        assertTrue(b.v16ServiceDao().allTransferredEvidence().all { row ->
        ...
        val linked = b.v16ServiceDao().allTransferredEvidence()
        assertEquals(2, b.v16ServiceDao().allTransferredFinalResults().size)
        assertTrue(linked.all { it.transferredFinalResultId != null })
        importInto(b, rootB, evidence)
        importInto(b, rootB, history)
        assertEquals(linked, b.v16ServiceDao().allTransferredEvidence())

        val reverse = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        val reverseRoot = File(context.cacheDir, "b049-reverse-${System.nanoTime()}").apply { mkdirs() }
        try {
            V16ServiceDatabase.configureStage4Tracking(reverse.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(reverse.openHelper.writableDatabase)
            V16ServicePeerTrustStore(reverse).add(sourceId, "Original workspace")
            importInto(reverse, reverseRoot, history)
            assertEquals(0, reverse.v16ServiceDao().allTransferredEvidence().size)
            ...
            val reverseEvidence = reverse.v16ServiceDao().allTransferredEvidence()
        ...
        val wrongResult = b.v16ServiceDao().allTransferredFinalResults().first { it.id != firstEvidence.transferredFinalResultId }
        ...
        assertEquals(wrongResult.id, b.v16ServiceDao().allTransferredEvidence().first { it.id == firstEvidence.id }.transferredFinalResultId)
    }
    ...
    @Test fun nativePlanCannotAttachToStagedOneTimeCustomer() = runBlocking {
        ...
        assertEquals(0, b.v16ServiceDao().allPlans().size)
    }

    @Test fun literalNullFreeTextRemainsTextThroughNativeImportAndRetry() = runBlocking {
        a.v16ServiceDao().insertCustomers(listOf(CustomerEntity("literal", "CU-N", "Literal customer", privateNote = "null")))
        val sourceId = V16ServicePeerTrustStore(a).localIdentity().technicianId
        ...
        val importer = DataTransferImportService(b, rootB)
        importer.import(importer.preview(bytes))
        assertEquals("null", b.v16ServiceDao().allCustomers().single().privateNote)
    ...
    @Test fun malformedCurrentNativeRowsRejectBeforeRoomMutation() = runBlocking {
        ...
        listOf(
            ...
            assertEquals(0, b.v16ServiceDao().allCustomers().size)
            assertEquals(0, b.v16ServiceDao().allPlans().size)
        }
    }
    ...
    @Test fun evidenceImportedBeforeHistoryLinksToExactFinalRevisionOnLaterImport() = runBlocking {
        ...
        val results = b.v16ServiceDao().allTransferredFinalResults()
        val linked = b.v16ServiceDao().allTransferredEvidence()
    ...
    @Test fun untrustedSourceAndConflictingBoundRecordCannotPartiallyWrite() = runBlocking {
        ...
        assertEquals(0, b.v16ServiceDao().allCustomers().size)
        V16ServicePeerTrustStore(b).add(sourceId, "Workspace A")
        b.v16ServiceDao().insertCustomers(listOf(CustomerEntity("collision", "CU-1", "Different customer")))
        ...
        assertEquals(1, b.v16ServiceDao().allCustomers().size)
        assertEquals(0, b.v16ServiceDao().allSites().size)
    }

    @Test fun privateFieldsOmittedBySenderDoNotEraseReceiverValuesWhenMatching() = runBlocking {
        a.v16ServiceDao().insertCustomers(listOf(CustomerEntity("c", "CU-PRIVATE", "Same shared name", privateNote = "Sender private value")))
        b.v16ServiceDao().insertCustomers(listOf(CustomerEntity("local-c", "CU-PRIVATE", "Same shared name", privateNote = "Receiver-only private value")))
        val sourceId = V16ServicePeerTrustStore(a).localIdentity().technicianId
        V16ServicePeerTrustStore(b).add(sourceId, "Workspace A")
        ...
        importer.import(preview)
        assertEquals("Receiver-only private value", b.v16ServiceDao().customer("local-c")?.privateNote)
        assertEquals(1, b.v16ServiceDao().allCustomers().size)
    }
    ...
    @Test fun privacyOffNativeCustomerDataCarriesOperationalFieldsAndDetectsPhoneDrift() = runBlocking {
        ...
        val selection = ExportCenterSelection(V16ServiceScopeFilter(customerId = "c"), families, includePrivate = false)
        ...
        val conflictDb = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        val conflictRoot = File(context.cacheDir, "b049-transfer-conflict-${System.nanoTime()}").apply { mkdirs() }
        try {
            V16ServiceDatabase.configureStage4Tracking(conflictDb.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(conflictDb.openHelper.writableDatabase)
            val conflictDao = conflictDb.v16ServiceDao()
            ...
            V16ServicePeerTrustStore(conflictDb).add(sourceId, "Workspace A")
        ...
        val importer = DataTransferImportService(b, rootB)
        importer.import(importer.preview(bytes))
        val importedDao = b.v16ServiceDao()
    ...
    @Test fun matchedContactsKeepLocalOrderAndNewContactsAppendBySourceOrderWithRetrySafety() = runBlocking {
        ...
        val sourceId = V16ServicePeerTrustStore(a).localIdentity().technicianId
        b.v16ServiceDao().insertCustomers(listOf(CustomerEntity("local-c", "CU-CONTACT", "Contact customer")))
        b.v16ServiceDao().insertCustomerContacts(listOf(
    ...
    @Test fun customerDataContactsRespectCustomerLifecycleAndScope() = runBlocking {
        ...
        val scoped = rows(exporter.export(ExportCenterSelection(
            scope = V16ServiceScopeFilter(customerId = "customer-a"),
            families = ExportPreset.CUSTOMER_DATA.families,
            includeInactive = true,
        )))
    ...
    @Test fun inactiveHistoryEvidenceAndDisabledTemplateDependenciesCloseWithoutUnrelatedInactiveRows() = runBlocking {
        ...
        val scopedBytes = DataTransferExportService(a, rootA).export(selection.copy(scope = V16ServiceScopeFilter(customerId = "c")))
    ...
    @Test fun transferredEvidenceRetryComparesImmutableMetadataAndBytesWithoutMutationOnConflict() = runBlocking {
        ...
        val originalEvidence = b.v16ServiceDao().allTransferredEvidence().first()
        ...
        val countBefore = b.v16ServiceDao().allTransferredEvidence().size
        ...
        assertEquals(countBefore, b.v16ServiceDao().allTransferredEvidence().size)
