...
import com.v16studio.v16service.data.AttachmentEntity
import com.v16studio.v16service.data.ChecklistItemSnapshotEntity
import com.v16studio.v16service.data.CustomerEntity
import com.v16studio.v16service.data.DraftWriteGate
...
import com.v16studio.v16service.data.ServiceObligationEntity
import com.v16studio.v16service.data.ServicePlanEntity
import com.v16studio.v16service.data.SiteEntity
import com.v16studio.v16service.data.TemplateSnapshotEntity
...
import com.v16studio.v16service.data.WorkingResponseEntity
import com.v16studio.v16service.data.WorkingVisitEntity
import com.v16studio.v16service.data.WorkingInputBufferEntity
...
class PersistenceIntegrityTest {
    ...
    @Test fun customerSiteEquipmentAndPlansPersistWithIndependentStableIdentity() = runTest {
        seedFoundation()
        val rows = database.v16ServiceDao().equipmentPlans("equipment-1")
        assertEquals(1, rows.size)
        assertEquals("customer-1", database.v16ServiceDao().site("site-1")?.customerId)
        assertEquals("site-1", database.v16ServiceDao().equipment("equipment-1")?.siteId)
        assertEquals("equipment-1", database.v16ServiceDao().plan("plan-1")?.equipmentId)

        database.v16ServiceDao().renameCustomer("customer-1", "Renamed customer")
        database.v16ServiceDao().renameEquipment("equipment-1", "Renamed machine")
        assertEquals("customer-1", database.v16ServiceDao().customer("customer-1")?.id)
        assertEquals("equipment-1", database.v16ServiceDao().equipment("equipment-1")?.id)
        assertNotEquals(database.v16ServiceDao().equipment("equipment-1")?.name, database.v16ServiceDao().equipment("equipment-1")?.id)
    }

    @Test fun currentObligationIdentitySurvivesReloadAndIsNotDueDateOrDisplayText() = runTest {
        seedFoundation()
        val plan = database.v16ServiceDao().plan("plan-1")!!
        val obligation = database.v16ServiceDao().obligation(plan.currentObligationId!!)!!
        ...
        val reconstructed = RoomV16ServiceRepository(database, time).equipment("equipment-1")!!
    ...
    @Test fun capturedSnapshotRemainsFixedWhenMasterDataIsRenamed() = runTest {
        ...
        database.v16ServiceDao().renameCustomer("customer-1", "Current customer changed")
        database.v16ServiceDao().renameEquipment("equipment-1", "Current equipment changed")
        val draft = repository.inspection("work-1")!!
        assertEquals("Captured customer", database.v16ServiceDao().visit("visit-1")?.customerNameSnapshot)
    ...
    @Test fun durableDraftSurvivesRepositoryReconstruction() = runTest {
        seedFoundation()
        val firstRepository = RoomV16ServiceRepository(database, time)
        firstRepository.saveResponse("work-1", "check-1", ResponseDisposition.OK, null, null)
        val reconstructedRepository = RoomV16ServiceRepository(database, time)
    ...
    @Test fun atomicQuestionTransitionClearsRawFamilyAndSurvivesFreshLoad() = runTest {
        ...
        val freshDraft = RoomV16ServiceRepository(database, time).inspection("work-1")!!
    ...
    @Test fun atomicIssueToOkPreservesInactiveIssueDraftAndFreshLoadStaysOk() = runTest {
        ...
        val freshQuestion = RoomV16ServiceRepository(database, time).inspection("work-1")!!.questions.single()
    ...
    @Test fun failedDraftWriteDoesNotPersistTheOptimisticAnswer() = runTest {
        seedFoundation()
        val repository = RoomV16ServiceRepository(database, time, DraftWriteGate { error("controlled write failure") })
        try {
            repository.saveResponse("work-1", "check-1", ResponseDisposition.OK, null, null)
        ...
        assertEquals(ResponseDisposition.NOT_CHECKED, RoomV16ServiceRepository(database, time).inspection("work-1")!!.questions.single().disposition)
    }

    @Test fun issueReasonBecomesInactiveDraftAfterTransitionToOk() = runTest {
        seedFoundation()
        database.v16ServiceDao().upsertResponses(listOf(WorkingResponseEntity("response-1", "work-1", "check-1", "ISSUE_FOUND", null, null, "Fraying edge", 2, issueFoundReasonDraft = "Fraying edge")))

        RoomV16ServiceRepository(database, time).saveResponse("work-1", "check-1", ResponseDisposition.OK, null, "Fraying edge")

        val question = RoomV16ServiceRepository(database, time).inspection("work-1")!!.questions.single()
    ...
    @Test fun savedNumericValueSurvivesAsInactiveDraft() = runTest {
        ...
        RoomV16ServiceRepository(database, time).saveResponse("work-1", "check-number", ResponseDisposition.NOT_APPLICABLE, "1240.5", "Not fitted")

        val question = RoomV16ServiceRepository(database, time).inspection("work-1")!!.questions.first { it.snapshotItemId == "check-number" }
    ...
    @Test fun finalizationBlocksDocumentedWorkWithOrphanedRawBufferWithoutAutosaveCoordinator() = runTest {
        seedFoundation()
        val repository = RoomV16ServiceRepository(database, time)
        database.v16ServiceDao().upsertWorkingInputBuffer(
        ...
        assertEquals(null, database.v16ServiceDao().finalRecordForVisit("visit-1"))
    }
    ...
    @Test fun inactiveDraftsDoNotLeakIntoOkFinalSnapshot() = runTest {
        ...
        val result=repository.finalizeVisit("visit-1") as com.v16studio.v16service.domain.FinalizeResult.Success
        val record=database.v16ServiceDao().finalRecord(result.recordId)!!
        val finalWork=database.v16ServiceDao().finalWorkItems(record.currentRevisionId).single()
        val finalAnswer=database.v16ServiceDao().finalChecklistItems(finalWork.id).single()
    ...
    @Test fun harmlessStatusChangePersistsWithoutContradictoryFields() = runTest {
        ...
        RoomV16ServiceRepository(database, time).saveResponse("work-1", "check-1", ResponseDisposition.OK, "stale", "stale")
    ...
    @Test fun performedOutcomeAutomaticallyFulfillsEligibleCurrentObligation() = runTest {
        seedFoundation()
        val line = RoomV16ServiceRepository(database, time).completionLines("visit-1").single()
    ...
    @Test fun fulfillmentProjectionKeepsPerformedAutomaticPartlyExplicitAndNotPerformedNonFulfilling() = runTest {
        ...
        val lines = RoomV16ServiceRepository(database, time).completionLines("visit-1").associateBy { it.workItemId }
        ...
        assertTrue(lines.getValue("work-unreviewed").blockers.any { it.kind == com.v16studio.v16service.domain.CompletionBlockerKind.CHECKLIST_INCOMPLETE })
    }
    ...
    @Test fun explicitEligibleFulfillmentUsesCapturedIntervalFromActualServiceDate() = runTest {
        ...
        val line = RoomV16ServiceRepository(database, time).completionLines("visit-1").first { it.workItemId == "work-fulfilled" }
        assertEquals(FulfillmentEligibility.ELIGIBLE, line.fulfillmentEligibility)
        assertTrue(line.fulfillsCurrentObligation == true)
        assertEquals("2026-12-05", line.proposedNextDueDate)
    }
    ...
    @Test fun notPerformedOneOffAndHistoryOnlyWorkNeverClaimAnOutstandingDueDate() = runTest {
        ...
        val historyWork = database.v16ServiceDao().visitWorkItems(historyVisit).single().id
    ...
    @Test fun attachmentUsesStableOwnerAndOwnedPathNotDisplayNameOrExternalUri() = runTest {
        ...
        database.v16ServiceDao().insertAttachments(listOf(attachment))
        val reloaded = database.v16ServiceDao().attachment("attachment-1")!!
    ...
    private suspend fun insertTextAndNumberQuestions() {
        database.v16ServiceDao().insertChecklistItems(
            listOf(
