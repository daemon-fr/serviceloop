...
class B049DataTransferCodecTest {
    ...
    @Test fun rejectsUndeclaredAndMissingSectionsAndUnsafeBinaryName() {
        val bytes = DataTransferCodec.encode(exporter, families = mapOf(DataTransferFamily.REGISTER to "{\"version\":1}".toByteArray()), sourceWorkspaceId = exporter)
        val original = V16ServiceSyncEnvelopeCodec.decode(bytes)
        val added = original.manifest.copy(sections = original.manifest.sections + V16ServiceSyncSectionDeclaration("extra", 1, "extra.json"))
        val withExtra = V16ServiceSyncEnvelopeCodec.encode(added, original.sections + ("extra" to "{\"version\":1}".toByteArray()))
        ...
        val withoutRegister = V16ServiceSyncEnvelopeCodec.encode(missing, original.sections - "register")
    ...
    @Test fun rejectsUnsupportedTransferVersionSourceIdentityAndBinaryMetadataDrift() {
        ...
        malformed { it.put("sourceWorkspaceId", "not-a-v16service-ID") }
