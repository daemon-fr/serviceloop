...
import com.v16studio.v16service.ui.workResultCommitMessage
...
class B049WorkResultImportTest {
    ...
    @Before fun setup() = kotlinx.coroutines.runBlocking {
        ...
        val dao = database.v16ServiceDao(); val dispatch = database.dispatchDao()
        ...
        dispatch.insertTrustedV16ServiceId(TrustedV16ServiceIdEntity(exporter, "Field technician", 1, 1))
    ...
    @Test fun conflictingCurrentRevisionNumbersCannotChooseReportTruth() = runTest {
        ...
        val rows = database.v16ServiceDao().appliedRemoteFinalResultsIncludingVoids()
    ...
    @Test fun partialThenCompleteAndRetriesPreserveRemoteTruthAndAdvanceOnce() = runTest {
        ...
        assertEquals(1, database.v16ServiceDao().appliedWorkResultReceipts("dispatch-v").size)
        assertEquals("DISPATCHED", database.dispatchDao().outboxVisit("dispatch-v")!!.outboxStatus.name)
        assertEquals("2027-09-01", database.v16ServiceDao().plan("plan")!!.currentDueDate)
        assertEquals("BOOKED", database.v16ServiceDao().visit("v")!!.state)
        ...
        assertEquals(2, database.v16ServiceDao().obligationCount("plan"))
        ...
        assertEquals("APPLIED", database.v16ServiceDao().workResultReceipt(exporter, "result-1", "revision-1-corrected")!!.status)
        assertEquals(2, database.v16ServiceDao().obligationCount("plan"))
        ...
        assertEquals("COMPLETED", database.v16ServiceDao().visit("v")!!.state)
        assertEquals(3, database.v16ServiceDao().reportableRemoteFinalResults().size)
        assertEquals(2, database.v16ServiceDao().obligationCount("plan"))
    ...
    @Test fun staleResultIsRetainedWithoutRecurrenceAndWrongTargetIsRejected() = runTest {
        ...
        assertEquals("2026-09-01", database.v16ServiceDao().plan("plan")!!.currentDueDate)
        assertEquals(1, database.v16ServiceDao().obligationCount("plan"))
        assertEquals(0, database.v16ServiceDao().reportableRemoteFinalResults().size)
        assertEquals("DISPATCHED", database.dispatchDao().outboxVisit("dispatch-v")!!.outboxStatus.name)
    ...
    @Test fun canceledDispatchRetainsSourceWithoutCompletingVisitOrAdvancingPlan() = runTest {
        ...
        assertEquals("STALE", database.v16ServiceDao().workResultReceipt(exporter, "result-1", "revision-1")!!.status)
    ...
    @Test fun independentAuthorsWithSameResultTokensBothRemainEffective() = runTest {
        ...
        val dispatch = database.dispatchDao()
        dispatch.insertTrustedV16ServiceId(TrustedV16ServiceIdEntity(secondAuthor, "Second technician", 1, 1))
        ...
        val effective = effectiveRemoteResults(database.v16ServiceDao().reportableRemoteFinalResults())
    ...
    @Test fun correctionChangingAppliedRecurrenceReportsItsCommittedConflict() = runTest {
        ...
        assertEquals("CONFLICT", database.v16ServiceDao().workResultReceipt(exporter, "result-1", "revision-1-conflicting")!!.status)
        assertEquals("2027-09-01", database.v16ServiceDao().plan("plan")!!.currentDueDate)
        assertEquals(2, database.v16ServiceDao().obligationCount("plan"))
    ...
    @Test fun everyoneAssignmentAcceptsASelectedTeamParticipant() = runTest {
        ...
        assertEquals("APPLIED", database.v16ServiceDao().workResultReceipt(exporter, "result-2", "revision-2")!!.status)
    }
    ...
    @Test fun missingRecordedAtIsRejectedEvenWhenPackageHasGeneratedAt() = runTest {
        ...
        assertNull(database.v16ServiceDao().workResultReceipt(exporter, "result-2", "revision-2"))
    }
    ...
    @Test fun v1SourceExecutionIdentityIsStoredWithoutCoordinatorSubstitution() = runTest {
        ...
        assertEquals(1, V16ServiceSyncEnvelopeCodec.decode(bytes).manifest.sections.first().version)
        val service = WorkResultImportService(database, root)
        assertEquals("APPLIED", service.import(bytes).items.single().status)
        val stored = database.v16ServiceDao().remoteFinalResult(exporter, "result-2", "revision-2")!!
    ...
    @Test fun contradictoryResultSemanticsRejectBeforeAnyReceiptOrObligationEffect() = runTest {
        ...
        assertNull(database.v16ServiceDao().workResultReceipt(exporter, "result-1", "revision-1"))
        assertEquals(1, database.v16ServiceDao().obligationCount("plan"))
    }
    ...
    @Test fun readableContactNoteDateBoundsUseInclusiveBusinessZoneDays() = runTest {
        ...
        val bytes = ExportCenterService(database, root).export(ExportCenterSelection(
            scope = V16ServiceScopeFilter(fromDate = java.time.LocalDate.parse("2026-09-24"), toDate = java.time.LocalDate.parse("2026-09-24")),
    ...
    @Test fun untrustedResultLeavesNoReceiptOrRemoteFinalTruth() = runTest {
        V16ServicePeerTrustStore(database).remove(exporter)
        ...
        assertNull(database.v16ServiceDao().workResultReceipt(exporter, "result-2", "revision-2"))
        assertNull(database.v16ServiceDao().remoteFinalResult(exporter, "result-2", "revision-2"))
        assertEquals("DISPATCHED", database.dispatchDao().outboxVisit("dispatch-v")!!.outboxStatus.name)
    }
    ...
    @Test fun aggregateSelectsOneVisitForTwoRemoteServicesAndFreezesBothRevisions() = runTest {
        ...
        val repository = RoomV16ServiceRepository(database, time, attachmentRoot = root)
        var renderedLines = 0
