...
class B049ImageCleanupTest {
    ...
    @Before fun setup() = kotlinx.coroutines.runBlocking {
        ...
        dao.insertFinalPhotos(listOf(FinalPhotoEntryEntity("final-photo-entry","final-item",1,"final-photo",relative(finalFile),WorkResultPackageCodec.sha256(bytes),bytes.size.toLong(),"image/jpeg","Final image")))
    }
    ...
    @Test fun neverKeepsAllAndAgedFinalGetsVerifiedCopyBeforeRemoval() = runTest {
        ...
        assertEquals(1, result.originalsRemoved)
    ...
    @Test fun aggregateFreezesExactSourceAndCreatesNewPdfFromRetainedTruth() = runTest {
        ...
        assertEquals(listOf("LOCAL:revision"), sources.map { it.key })
