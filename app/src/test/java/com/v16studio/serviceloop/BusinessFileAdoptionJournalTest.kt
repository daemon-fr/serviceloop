...
import com.v16studio.serviceloop.data.BusinessFileAdoptionJournal
import com.v16studio.serviceloop.data.RecoveryMetadataEntity
import com.v16studio.serviceloop.data.ServiceLoopDatabase
import com.v16studio.serviceloop.data.TransferredEvidenceEntity
...
class BusinessFileAdoptionJournalTest {
    ...
    @Test fun interruptedUncommittedAdoptionRemovesOnlyItsOwnedBytes() {
        ...
        val db = Room.inMemoryDatabaseBuilder(context, ServiceLoopDatabase::class.java).allowMainThreadQueries().build()
        try {
    ...
    @Test fun interruptedCommittedAdoptionKeepsReferencedBytes() = runBlocking {
        ...
        try {
            ...
            val hash = com.v16studio.serviceloop.data.WorkResultPackageCodec.sha256(bytes)
            BusinessFileAdoptionJournal.begin(db, root, mapOf(path to bytes)).adopt(path, bytes)
            db.serviceLoopDao().insertTransferredEvidence(listOf(TransferredEvidenceEntity(
    ...
    @Test fun damagedJournalRestrictsRecoveryWithoutDeletingEvidence() = runBlocking {
        ...
        val db = Room.inMemoryDatabaseBuilder(context, ServiceLoopDatabase::class.java).allowMainThreadQueries().build()
        try {
            db.serviceLoopDao().upsertRecoveryMetadata(RecoveryMetadataEntity(datasetId = "dataset",
