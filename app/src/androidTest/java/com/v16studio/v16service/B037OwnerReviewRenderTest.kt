...
class B037OwnerReviewRenderTest {
    ...
    fun captureOwnerReviewSurfacesForLightAndDarkInspection() {
        ...
        val viewModel = V16ServiceViewModel(
            RoomV16ServiceRepository(database, ClockBusinessTime(zoneId = ZoneId.of("Europe/Bucharest"))),
        ) {}
        val state = UiState(
            loading = false,
        ...
        try {
            ...
            compose.onNodeWithTag("visit-site-site-select").performClick()
        ...
        compose.runOnUiThread {
            compose.activity.setContent {
                V16ServiceTheme(darkTheme = darkTheme) {
                    Surface(Modifier.fillMaxSize().systemBarsPadding(), color = LocalV16ServiceTokens.current.canvas) {
