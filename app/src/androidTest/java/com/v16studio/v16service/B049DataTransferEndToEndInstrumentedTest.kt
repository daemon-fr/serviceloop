...
class B049DataTransferEndToEndInstrumentedTest {
    @Test fun twoWorkspacesMergeSafeCurrentStateAndImmutableHistoryThenRelayOriginalOrigin() = runBlocking {
        ...
        val workspaceA = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        val workspaceB = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        ...
        try {
            V16ServiceDatabase.configureStage4Tracking(workspaceA.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(workspaceA.openHelper.writableDatabase)
            V16ServiceDatabase.configureStage4Tracking(workspaceB.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(workspaceB.openHelper.writableDatabase)
            val a = workspaceA.v16ServiceDao()
            ...
            val sourceId = V16ServicePeerTrustStore(workspaceA).localIdentity().technicianId
            V16ServicePeerTrustStore(workspaceB).add(sourceId, "Workspace A")
            ...
            val b = workspaceB.v16ServiceDao()
            ...
            val report = AggregateReportService(workspaceB, RoomV16ServiceRepository(workspaceB, ClockBusinessTime(zoneId = ZoneId.of("UTC")), attachmentRoot = rootB), rootB)
                .reportable(V16ServiceScopeFilter(customerId = importedCustomer.id)).single()
            ...
            fun subset(families: Set<DataTransferFamily>) = DataTransferCodec.encode(
                ...
                options = decodedPackage.options)
            val workspaceC = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
            val rootC = File(context.cacheDir, "b049-transfer-c-device-${System.nanoTime()}").apply { mkdirs() }
            try {
                V16ServiceDatabase.configureStage4Tracking(workspaceC.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(workspaceC.openHelper.writableDatabase)
                V16ServicePeerTrustStore(workspaceC).add(sourceId, "Workspace A")
                ...
                assertNull(workspaceC.v16ServiceDao().allTransferredEvidence().single().transferredFinalResultId)
                ...
                val c = workspaceC.v16ServiceDao()
                ...
                val aggregate = AggregateReportService(workspaceC,
                    RoomV16ServiceRepository(workspaceC, ClockBusinessTime(zoneId = ZoneId.of("UTC")), attachmentRoot = rootC), rootC,
                    AggregateReportWriter { models, _, _, _, target, _ -> rendered = models; target.writeBytes(byteArrayOf(1)); 1 })
                val selected = aggregate.reportable(V16ServiceScopeFilter(customerId = c.allCustomers().single { it.reference == "CU-DEVICE" }.id))
                aggregate.generate(V16ServiceScopeFilter(customerId = selected.single().customerId), listOf(selected.single().key))
                assertEquals(1, rendered.single().lines.sumOf { it.photos.size })
            } finally { workspaceC.close(); rootC.deleteRecursively() }

            val workspaceD = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
            val rootD = File(context.cacheDir, "b049-transfer-d-device-${System.nanoTime()}").apply { mkdirs() }
            try {
                V16ServiceDatabase.configureStage4Tracking(workspaceD.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(workspaceD.openHelper.writableDatabase)
                V16ServicePeerTrustStore(workspaceD).add(sourceId, "Workspace A")
                ...
                assertTrue(workspaceD.v16ServiceDao().allTransferredEvidence().isEmpty())
                ...
                val d = workspaceD.v16ServiceDao()
                ...
                d.upsertBusinessProfile(BusinessProfileEntity(businessName = "Receiver", technicianName = "Coordinator",
                    phone = null, email = null, postalAddress = null, zoneId = "UTC", modifiedAtEpochMillis = 1))
                var renderedD: List<com.v16studio.v16service.domain.PublicReportModel> = emptyList()
                val aggregateD = AggregateReportService(workspaceD,
                    RoomV16ServiceRepository(workspaceD, ClockBusinessTime(zoneId = ZoneId.of("UTC")), attachmentRoot = rootD), rootD,
                    AggregateReportWriter { models, _, _, _, target, _ -> renderedD = models; target.writeBytes(byteArrayOf(1)); 1 })
                val reportableD = aggregateD.reportable(V16ServiceScopeFilter(customerId = customer.id))
                aggregateD.generate(V16ServiceScopeFilter(customerId = customer.id), listOf(reportableD.single().key))
