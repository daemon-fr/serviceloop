...
class OwnerVisualRuntimeTest {
    ...
    fun finalRecordPdfAndTextOpenFromTestOwnedRecord() {
        ...
        try {
            ...
            composeRule.activity.setContent { V16ServiceTheme { V16ServiceApp(viewModel, "record/r") } }
            ...
            if (InstrumentationRegistry.getArguments().getString("systemHandoff") == "true") {
                ...
                val instrumentation = InstrumentationRegistry.getInstrumentation()
                var external = false
                repeat(30) { if (instrumentation.uiAutomation.rootInActiveWindow?.packageName?.toString() != "com.v16studio.v16service") external = true else Thread.sleep(100) }
                assertTrue("Sharesheet did not open", external)
                if (instrumentation.uiAutomation.rootInActiveWindow?.packageName?.toString() != "com.v16studio.v16service") instrumentation.uiAutomation.performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK)
            }
        } finally {
    ...
    fun inspectionResponseDraftsSwitchWithoutWarningAndRestoreSavedAndLocalBuffers() {
        val database = Room.inMemoryDatabaseBuilder(composeRule.activity, V16ServiceDatabase::class.java).allowMainThreadQueries().build()
        val dao = database.v16ServiceDao()
        runBlocking {
            dao.insertCustomers(listOf(CustomerEntity("c", "CU-1", "Customer")))
        ...
        composeRule.activity.setContent { V16ServiceTheme { V16ServiceApp(viewModel) } }
        ...
        composeRule.waitUntil(5_000) {
            runBlocking { database.v16ServiceDao().responses("w").firstOrNull { it.checklistItemSnapshotId == "check-note" }?.disposition == ResponseDisposition.NOT_APPLICABLE.name }
        }
        composeRule.waitUntil(5_000) {
