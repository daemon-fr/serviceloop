...
class B026LocalFlexibleWorkUiTest {
    ...
    @Test fun searchFindsOneTimeRecordWithoutRegisterReveal() {
        ...
        val viewModel = V16ServiceViewModel(FakeRepository()) {}
    ...
    @Test fun navigationOnlyButtonShowsDisclosureButCommandsDoNot() {
            ...
            compose.activity.setContent {
                V16ServiceTheme {
                    androidx.compose.foundation.layout.Column {
                        com.v16studio.v16service.ui.designsystem.V16ServiceNavigationButton("Customer", { navigated = true }, androidx.compose.ui.Modifier.testTag("relationship-navigation"))
                        com.v16studio.v16service.ui.designsystem.V16ServiceSecondaryButton("Edit", {}, androidx.compose.ui.Modifier.testTag("command-edit"))
                        com.v16studio.v16service.ui.designsystem.V16ServicePrimaryButton("Create visit", {}, androidx.compose.ui.Modifier.testTag("command-create"))
                    }
                }
            }
        }
        compose.onNodeWithTag("v16-service-disclosure-icon", useUnmergedTree = true).assertIsDisplayed()
        compose.onAllNodesWithTag("v16-service-disclosure-icon", useUnmergedTree = true).assertCountEquals(1)
        ...
        val disclosureBounds = compose.onNodeWithTag("v16-service-disclosure-icon", useUnmergedTree = true).fetchSemanticsNode().boundsInRoot
    ...
    @Test fun siteActionsGiveLongerNavigationLabelsMoreWidth() {
        ...
        val viewModel = V16ServiceViewModel(FakeRepository(site = detail)) {}
        compose.runOnUiThread {
            compose.activity.setContent {
                V16ServiceTheme { SiteDetailScreen(detail, PaddingValues(), rememberNavController(), viewModel) }
            }
        }
    ...
    private fun renderServiceAndLink(repository: FakeRepository, viewModel: V16ServiceViewModel) {
        compose.runOnUiThread {
            compose.activity.setContent {
                V16ServiceTheme {
                    val nav = rememberNavController()
    ...
    @Composable
    private fun TestServiceRoute(repository: FakeRepository, viewModel: V16ServiceViewModel, nav: androidx.navigation.NavHostController) {
        val state by viewModel.state.collectAsState()
