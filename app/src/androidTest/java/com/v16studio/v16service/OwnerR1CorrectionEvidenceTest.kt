...
class OwnerR1CorrectionEvidenceTest {
    ...
    private data class Proof(val number: String, val name: String, val route: String?, val prepareCapture: () -> Unit = {}, val assertReady: (V16ServiceViewModel) -> Unit)
    ...
    private fun render(dark: Boolean) {
        ...
        compose.activity.setContent { V16ServiceTheme(darkTheme = dark) { key(route.value) { V16ServiceApp(viewModel, route.value) } } }

        val proofs = listOf(
            Proof("01", "home-brand", null) {
                compose.onNodeWithTag("root-home").assertIsDisplayed()
                compose.onAllNodesWithText("V16 Service")[0].assertIsDisplayed()
            },
            ...
            Proof("08a", "expanded-editor-chrome", "inspection/${FixtureIds.OWNER_REVIEW_WORK}", prepareCapture = {
                ...
                assertTrue(compose.onAllNodesWithText("V16 Service").fetchSemanticsNodes().size >= 2)
            }) { vm ->
                assertEquals(FixtureIds.OWNER_REVIEW_WORK, vm.state.value.inspection?.workItemId)
