...
class B013R31SelectionAndActionStackUiTest {
    ...
    fun selectionGlyphIsTwentyFourDpAndClearsShortCardContext() {
        compose.runOnUiThread { compose.activity.setContent { V16ServiceTheme {
            V16ServiceEntityRecord("P-001 · Short service","EQ-001 · Compressor",selectionChecked=false,onSelectionChange={},onClick={})
        } } }
        compose.waitForIdle()
                ...
                V16ServiceTheme {
                    Column(Modifier.fillMaxWidth().padding(16.dp)) {
                        V16ServiceActionStack {
                            V16ServiceSecondaryButton("First", {}, Modifier.fillMaxWidth().testTag("action-first"))
                            V16ServiceSecondaryButton("Second", {}, Modifier.fillMaxWidth().testTag("action-second"))
                        }
                    }
                }
            }
    ...
    fun brandWordmarkIsCenteredBetweenEqualLineRegions() {
        compose.runOnUiThread { compose.activity.setContent { V16ServiceTheme { V16ServiceBrandStrip(Modifier.width(360.dp).testTag("brand-strip")) } } }
        compose.waitForIdle()
        val strip=compose.onNodeWithTag("brand-strip",true).fetchSemanticsNode().boundsInRoot
        val wordmark=compose.onNodeWithTag("v16service-wordmark",true).fetchSemanticsNode().boundsInRoot
        ...
        compose.onNodeWithText("V16 Service").assertIsDisplayed()
    }
}
...
class B013R31OwnerSurfaceRenderTest {
    ...
    fun restoreOwnerReviewPreferences() {
        ...
        compose.activity.setTeamRole(com.v16studio.v16service.ui.TeamRole.SOLO)
    }
    ...
    fun rendersDueReminderAndTechnicianSurfacesOnCanonicalDevice() {
        ...
        (compose.activity.application as V16ServiceApplication).container.appearancePreferences.setMode(AppearanceMode.LIGHT)
        compose.waitForIdle()
        capture("home.png")
    ...
    fun rendersDarkRootWorkAppearanceAndReminderSurfacesOnCanonicalDevice() {
        compose.waitUntil(5_000) { compose.onAllNodesWithTag("root-home").fetchSemanticsNodes().isNotEmpty() }
        (compose.activity.application as V16ServiceApplication).container.appearancePreferences.setMode(AppearanceMode.DARK)
        compose.waitForIdle()
