...
class B049WorkResultEndToEndInstrumentedTest {
    @Test fun exporterProducedV1PhotoResultIsAppliedAndReplayedWithoutEffects() = runBlocking {
        ...
        val source = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).build()
        val receiver = Room.inMemoryDatabaseBuilder(context, V16ServiceDatabase::class.java).build()
        ...
        try {
            V16ServiceDatabase.configureStage4Tracking(source.openHelper.writableDatabase); V16ServiceDatabase.configureReminderDefaults(source.openHelper.writableDatabase)
            ...
            V16ServicePeerTrustStore(source).add(issuer, "Coordinator")
            ...
            receiverDispatch.insertTrustedV16ServiceId(TrustedV16ServiceIdEntity(technician, "Field technician", 1, 1))
            ...
            Log.i("V16ServiceWorkload", "12-photo result bytes=${bytes.size} exportMs=$exportMillis importMs=$importMillis " +
    ...
    @Test fun partialThenCompleteResultsConvergeOnOneCanonicalReportableVisit() = runBlocking {
        ...
        try {
            ...
            val reports = AggregateReportService(database, RoomV16ServiceRepository(database, time, attachmentRoot = root), root)
            dao.upsertBusinessProfile(BusinessProfileEntity("primary", "Coordinator Business", "Coordinator", null, null, null, "UTC", 1))
            val visits = reports.reportable(V16ServiceScopeFilter(customerId = "c"))
            ...
            val report = reports.generate(V16ServiceScopeFilter(customerId = "c"), listOf(visits.single().key))
            ...
            val runtime = Runtime.getRuntime()
            Log.i("V16ServiceWorkload", "corrected imported history reportMs=$reportMs nativeMs=$nativeMs readableMs=$readableMs " +
