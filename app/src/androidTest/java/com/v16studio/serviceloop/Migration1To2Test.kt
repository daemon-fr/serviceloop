...
class Migration1To2Test {
    ...
    @Test fun migrationThirteenToFourteenAddsBuffersAndNormalizesCompletionDecision() {
        ...
        SQLiteDatabase.openOrCreateDatabase(context.getDatabasePath(dbName), null).use { old ->
            ...
            old.execSQL("CREATE TABLE IF NOT EXISTS room_master_table (id INTEGER PRIMARY KEY,identity_hash TEXT)")
            old.execSQL("INSERT OR REPLACE INTO room_master_table (id,identity_hash) VALUES(42, '${schema.getString("identityHash")}')")
            old.execSQL("INSERT INTO customers(id,reference,name) VALUES('c','CU-14','Customer')")
            old.execSQL("INSERT INTO sites(id,customerId,reference,name) VALUES('s','c','ST-14','Site')")
            old.execSQL("INSERT INTO equipment(id,siteId,reference,name) VALUES('e','s','EQ-14','Equipment')")
            old.execSQL("INSERT INTO service_plans(id,equipmentId,reference,name,intervalCount,intervalUnit,currentDueDate,state,currentObligationId) VALUES('p','e','P-14','Plan',3,'MONTHS','2026-09-01','ACTIVE','o')")
            old.execSQL("INSERT INTO service_obligations(id,planId,sequence,dueDate,createdAtEpochMillis) VALUES('o','p',1,'2026-09-01',1)")
            old.execSQL("INSERT INTO working_visits(id,reference,customerId,siteId,actualServiceDate,customerNameSnapshot,siteNameSnapshot,state,modifiedAtEpochMillis) VALUES('v','V-14','c','s','2026-09-05','Customer','Site','WORKING',1)")
        ...
        val migrated = Room.databaseBuilder(context, ServiceLoopDatabase::class.java, dbName).addMigrations(ServiceLoopDatabase.MIGRATION_13_14, ServiceLoopDatabase.MIGRATION_14_15, ServiceLoopDatabase.MIGRATION_15_16, ServiceLoopDatabase.MIGRATION_16_17).build()
        try {
            kotlinx.coroutines.runBlocking {
                val dao = migrated.serviceLoopDao()
                listOf("w-null", "w-zero", "w-part", "w-not").forEach { id ->
                    val item = dao.workItem(id)!!
                    assertEquals(if (id == "w-part" || id == "w-not") false else null, item.fulfillsCurrentObligation)
                    assertNull(item.confirmedNextDueDate); assertNull(item.nextDueDateCalculated); assertNull(item.nextDueOverrideReason)
                }
                val retained = dao.workItem("w-true")!!
                assertEquals(true, retained.fulfillsCurrentObligation); assertEquals("2026-12-05", retained.confirmedNextDueDate); assertEquals(true, retained.nextDueDateCalculated)
                assertTrue(dao.workingInputBuffers("w-null").isEmpty())
            }
            migrated.openHelper.readableDatabase.query("PRAGMA table_info(working_input_buffers)").use { cursor -> assertTrue(cursor.count == 4) }
            migrated.openHelper.readableDatabase.query("PRAGMA foreign_key_check").use { assertEquals(0, it.count) }
        } finally { migrated.close(); context.deleteDatabase(dbName) }
    }
    ...
    @Test fun migrationFourteenToFifteenPreservesMandatoryServicePlanEquipmentAndB026Defaults() {
        ...
        val migrated = Room.databaseBuilder(context, ServiceLoopDatabase::class.java, dbName).addMigrations(ServiceLoopDatabase.MIGRATION_14_15, ServiceLoopDatabase.MIGRATION_15_16, ServiceLoopDatabase.MIGRATION_16_17).build()
        try {
            kotlinx.coroutines.runBlocking {
    ...
    @Test fun migrationElevenToTwelveNormalizesVisitLifecycleAndAddsCancellationColumns() {
        ...
        val migrated=Room.databaseBuilder(context,ServiceLoopDatabase::class.java,dbName).addMigrations(ServiceLoopDatabase.MIGRATION_11_12,ServiceLoopDatabase.MIGRATION_12_13,ServiceLoopDatabase.MIGRATION_13_14,ServiceLoopDatabase.MIGRATION_14_15,ServiceLoopDatabase.MIGRATION_15_16, ServiceLoopDatabase.MIGRATION_16_17).build()
