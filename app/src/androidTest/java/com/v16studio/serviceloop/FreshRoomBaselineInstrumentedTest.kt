...
class FreshRoomBaselineInstrumentedTest {
    ...
    @Test fun freshDatabaseUsesCurrentSchemaAndEnforcesSnapshotColumns() = runBlocking {
        ...
        val database = Room.databaseBuilder(context, ServiceLoopDatabase::class.java, name).build()
        try {
            val sql = database.openHelper.writableDatabase
            assertEquals(1, sql.version)
            ...
            database.serviceLoopDao().insertCustomers(listOf(customer))
            assertEquals(customer, database.serviceLoopDao().customer(customer.id))
            sql.query("PRAGMA integrity_check").use { cursor ->
                assertTrue(cursor.moveToFirst())
                assertEquals("ok", cursor.getString(0))
            }
