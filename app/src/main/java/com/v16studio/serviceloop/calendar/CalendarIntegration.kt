...
class CalendarCoordinator(private val context:Context, private val database:ServiceLoopDatabase, private val gateway:CalendarGateway, private val store:CalendarDeviceStore, private val scope:CoroutineScope) {
    ...
    suspend fun reconcile()=withContext(Dispatchers.IO){mutex.withLock{val(s,visits)=current();if(s.storeProblem||!s.enabled||!gateway.hasPermissions())return@withLock;var next=s;for(l in s.links.values){val v=visits[l.visitId];if(v==null||v.state=="CANCELLED"){val deleted=runCatching{gateway.delete(l.calendarId,l.eventId)}.getOrDefault(false);next=if(deleted)next.copy(links=next.links-l.visitId)else next.copy(links=next.links+(l.visitId to l.copy(state=CalendarLinkState.DELETE_PENDING)));continue};if(v.state!="BOOKED")continue;val exists=runCatching{gateway.eventExists(l.calendarId,l.eventId)}.getOrDefault(false);if(!exists){next=next.copy(links=next.links+(l.visitId to l.copy(state=CalendarLinkState.MISSING)));continue};val event=event(v);val fp=fingerprint(event);if(l.state==CalendarLinkState.SYNCED&&l.fingerprint!=fp&&runCatching{gateway.update(l.calendarId,l.eventId,event)}.getOrDefault(false))next=next.copy(links=next.links+(l.visitId to l.copy(fingerprint=fp,lastConfirmedAt=System.currentTimeMillis())))};store.write(next);val writable=runCatching{gateway.writableCalendars().any{it.id==next.selectedCalendarId}}.getOrDefault(false);if(writable)visits.values.filter{it.state=="BOOKED"&&it.scheduledAtEpochMillis!=null&&it.id !in next.links&&it.id !in next.suppressedVisitIds}.forEach{v->next=create(next,v)};store.write(next)}}
    ...
    private suspend fun current():Pair<CalendarDeviceState,Map<String,WorkingVisitEntity>>{val dao=database.serviceLoopDao();val dataset=dao.recoveryMetadata()?.datasetId?:"uninitialized";return store.read(dataset) to dao.allVisits().associateBy{it.id}}
