...
class CalendarCoordinator(private val context:Context, private val database:ServiceLoopDatabase, private val gateway:CalendarGateway, private val store:CalendarDeviceStore, private val scope:CoroutineScope) {
    ...
    private suspend fun current():Pair<CalendarDeviceState,Map<String,WorkingVisitEntity>>{val dao=database.serviceLoopDao();val metadata=dao.recoveryMetadata();val dataset=metadata?.datasetId?:"uninitialized";val customers=dao.allCustomers().associateBy{it.id};val sites=dao.allSites().associateBy{it.id};val visits=dao.allVisits().associateBy({it.id}){v->val customer=customers[v.customerId];val site=sites[v.siteId];v.copy(customerNameSnapshot=customer?.name?:v.customerNameSnapshot,siteNameSnapshot=site?.name?:v.siteNameSnapshot,siteAddressSnapshot=if(site!=null)site.address else v.siteAddressSnapshot)};return store.read(dataset,metadata?.adoptionToken.orEmpty()) to visits}
