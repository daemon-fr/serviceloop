...
fun ServiceLoopApp(viewModel: ServiceLoopViewModel, notificationRoute: String? = null) {
    ...
    NavHost(
        ...
        composable("dispatch/create") { DetailScaffold("Outbox",nav,topAction={ Button({nav.navigate("dispatch/visit/new")},Modifier.heightIn(min=52.dp).testTag("dispatch-new-visit")){Text("+  New visit")} }){DispatchOutboxScreen(it,nav,state.businessDate,showEmbeddedTopAction=false)} }
