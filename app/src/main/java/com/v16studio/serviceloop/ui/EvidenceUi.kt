...
private fun ServicePhotoCard(workItemId: String, photo: PhotoEntry, context: Context, viewModel: ServiceLoopViewModel, editingEnabled: Boolean, state: UiState) {
    ...
    if (editingEnabled) {
        ...
        Row(verticalAlignment = Alignment.CenterVertically) {
            com.v16studio.serviceloop.ui.designsystem.ServiceLoopCheckbox(include, { selected -> include = selected; viewModel.setPhotoReportInclusion(workItemId, photo.id, selected) }, modifier = Modifier.testTag("photo-report-${photo.id}"), contentDescription = "Include ${photo.caption ?: "photo"} in customer report")
