...
internal fun CreateDispatchPackageScreen(padding:PaddingValues) {
    ...
    var customers by remember{mutableStateOf<List<CustomerEntity>>(emptyList())}; var sites by remember{mutableStateOf<List<SiteEntity>>(emptyList())}; var equipment by remember{mutableStateOf<List<EquipmentEntity>>(emptyList())};var plans by remember{mutableStateOf<List<ServicePlanEntity>>(emptyList())}; var selectedSite by remember{mutableStateOf<SiteEntity?>(null)}; var selectedEquipment by remember{mutableStateOf<EquipmentEntity?>(null)};var selectedPlan by remember{mutableStateOf<ServicePlanEntity?>(null)}
    var sender by rememberSaveable{mutableStateOf("")};var recipient by rememberSaveable{mutableStateOf("")};var date by rememberSaveable{mutableStateOf(LocalDate.now().toString())};var appointment by rememberSaveable{mutableStateOf("")};var task by rememberSaveable{mutableStateOf("")};var instructions by rememberSaveable{mutableStateOf("")};var drafts by remember{mutableStateOf<List<DispatchVisit>>(emptyList())};var error by remember{mutableStateOf<String?>(null)}
    LaunchedEffect(Unit){withContext(Dispatchers.IO){customers=dao.allCustomers().filter{it.state=="ACTIVE"};sites=dao.allSites().filter{it.state=="ACTIVE"};equipment=dao.allEquipment().filter{it.state=="ACTIVE"};plans=dao.allPlans().filter{it.state=="ACTIVE"}}}
    LazyColumn(Modifier.padding(padding).testTag("dispatch-composer"),contentPadding=PaddingValues(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)) {
        ...
        item { Text("Visit draft",style=MaterialTheme.typography.titleMedium);OutlinedTextField(date,{date=it},label={Text("Service date · YYYY-MM-DD")},modifier=Modifier.fillMaxWidth());OutlinedTextField(appointment,{appointment=it},label={Text("Optional appointment time · HH:mm")},modifier=Modifier.fillMaxWidth());Text("Choose site");sites.forEach{s->OutlinedButton(onClick={selectedSite=s;selectedEquipment=null;selectedPlan=null},modifier=Modifier.fillMaxWidth().testTag("dispatch-site-${s.id}")){Text("${if(selectedSite?.id==s.id) "✓ " else ""}${s.reference} · ${s.name}")}} }
        item { Text("Choose equipment");equipment.filter{it.siteId==selectedSite?.id}.forEach{e->OutlinedButton(onClick={selectedEquipment=e;selectedPlan=null},modifier=Modifier.fillMaxWidth().testTag("dispatch-equipment-${e.id}")){Text("${if(selectedEquipment?.id==e.id) "✓ " else ""}${e.reference} · ${e.name}")}};Text("Optional existing service plan");OutlinedButton(onClick={selectedPlan=null},modifier=Modifier.fillMaxWidth()){Text("${if(selectedPlan==null)"✓ " else ""}One-off work")};plans.filter{it.equipmentId==selectedEquipment?.id}.forEach{p->OutlinedButton(onClick={selectedPlan=p},modifier=Modifier.fillMaxWidth()){Text("${if(selectedPlan?.id==p.id)"✓ " else ""}${p.reference} · ${p.name} · Due ${p.currentDueDate}")}};OutlinedTextField(task,{task=it},label={Text("Task name · Required")},modifier=Modifier.fillMaxWidth().testTag("dispatch-task"));OutlinedTextField(instructions,{instructions=it},label={Text("Instructions")},modifier=Modifier.fillMaxWidth()) }
        item { Button(onClick={runCatching{val day=LocalDate.parse(date);val scheduled=appointment.trim().takeIf{it.isNotEmpty()}?.let{day.atTime(LocalTime.parse(it)).atZone(ZoneId.systemDefault()).toInstant().toEpochMilli()};val s=checkNotNull(selectedSite);val e=checkNotNull(selectedEquipment);drafts= drafts+DispatchVisit(UUID.randomUUID().toString(),null,date,scheduled,s.reference,instructions.takeIf{it.isNotBlank()},listOf(DispatchWork(e.reference,task,selectedPlan?.reference,selectedPlan?.currentDueDate)));task="";instructions="";appointment="";selectedPlan=null}.onFailure{error=it.message}.let{{}}},enabled=selectedSite!=null&&selectedEquipment!=null&&task.isNotBlank(),modifier=Modifier.fillMaxWidth().testTag("add-dispatch-visit")){Text("Add visit")};Text("${drafts.size} visit${if(drafts.size==1)"" else "s"} in package");error?.let{Text(it,color=MaterialTheme.colorScheme.error)} }
