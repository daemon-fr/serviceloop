...
internal fun ServiceLoopNavGraph(
    ...
    NavHost(
        ...
        composable("team/id") { WorkspaceGate(teamRole != TeamRole.SOLO, "ServiceLoop ID", nav) { DetailScaffold(teamRole.idHeading(), nav) { TeamWorkspaceScreen(nav, Modifier.padding(it), TeamWorkspaceContent.ID) } } }
        ...
        composable("import") { WorkspaceGate(teamRole != TeamRole.SOLO, "Import shared data", nav) { DetailScaffold("Import shared data", nav) { padding -> ServiceLoopSyncScreen(state, padding, viewModel, nav, incomingServiceLoopSync) } } }
