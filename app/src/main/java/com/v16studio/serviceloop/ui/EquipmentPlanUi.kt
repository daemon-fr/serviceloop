...
internal fun PlanDetailScreen(plan: PlanDetail?, padding: PaddingValues, nav: NavHostController) {
    ...
    LazyColumn(Modifier.padding(padding), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(ServiceLoopUiTokens.Space.section)) { item { Text("${plan.reference} · ${plan.name}", style = MaterialTheme.typography.headlineSmall); Text(plan.equipmentName); Text("Every ${plan.intervalCount} ${plan.intervalUnit.lowercase()} · Due ${plan.dueDate}"); Text("Current obligation remains separate from bookings and contact."); if(capabilities.canManageRegister || capabilities.canCreateLocalWork){ Spacer(Modifier.height(ServiceLoopUiTokens.Space.large)); ServiceLoopActionStack { if(capabilities.canManageRegister) ServiceLoopPrimaryButton("Edit plan",{ nav.navigate("plan/edit/${plan.id}") }, Modifier.fillMaxWidth(), enabled=plan.state!="ENDED"); if(capabilities.canCreateLocalWork) ServiceLoopSecondaryButton("Create visit",{ nav.navigate("visit/new/${plan.id}") }, Modifier.fillMaxWidth(), enabled=plan.state=="ACTIVE"); if(capabilities.canManageRegister && plan.state=="ACTIVE") ServiceLoopSecondaryButton("Pause plan",{nav.navigate("lifecycle/PLAN/${plan.id}/PAUSE")},Modifier.fillMaxWidth()) else if(capabilities.canManageRegister && plan.state=="PAUSED") ServiceLoopSecondaryButton("Resume plan",{nav.navigate("lifecycle/PLAN/${plan.id}/RESUME")},Modifier.fillMaxWidth()); if(capabilities.canManageRegister && plan.state!="ENDED") ServiceLoopDangerTonalButton("End plan",{nav.navigate("lifecycle/PLAN/${plan.id}/END")},Modifier.fillMaxWidth()) } } } }
}
...
internal fun PlanEditorScreen(equipmentId: String?, existing: PlanDetail?, templates: List<TemplateSummary>, padding: PaddingValues, state: UiState, viewModel: ServiceLoopViewModel, nav: NavHostController) {
    ...
    EditorColumn(padding, state, "plan-editor") {
        item { DailyHeading(if (existing == null) "Add recurring service plan" else "Edit ${existing.reference}"); DailyField(name, { name = it }, "Plan name · Required"); DailyField(count, { count = it }, "Positive interval", bottomPadding = ServiceLoopUiTokens.Space.none) }
        item { ServiceLoopPresetChoiceGroup(listOf("DAYS","WEEKS","MONTHS","YEARS").map{it to it.lowercase().replaceFirstChar(Char::uppercase)},unit,{unit=it}, modifier = Modifier.testTag("plan-interval-unit-row"), singleRow = true) }
        item { DailyField(due, { due = it }, "Next due date · YYYY-MM-DD", bottomPadding = ServiceLoopUiTokens.Space.none); if(existing!=null&&due!=existing.dueDate) LongTextEditor(dueReason,{dueReason=it},"Due-date change reason · Required",false) }
