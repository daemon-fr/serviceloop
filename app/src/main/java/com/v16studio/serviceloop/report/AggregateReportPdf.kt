    ...
    fun render(models: List<PublicReportModel>, businessName: String, businessContact: String, reportId: String, target: File, filesRoot: File): Int {
        ...
        try {
            pages.forEachIndexed { index, item ->
                ...
                val footer = "ServiceLoop · ${item.let { when (it) { is Page.Lines -> it.source.visitReference; is Page.Photo -> it.source.visitReference } }} · ${reportId.take(8)} · Page $number of ${pages.size}"
