        ...
        val results = revisionIds.distinct().flatMap { revisionId ->
            ...
            dao.finalWorkItems(revisionId).mapNotNull { item ->
                ...
                val value = JSONObject()
                    ...
                    .put("followUps", JSONArray().also { array -> dao.followUpsForSourceWorkItem(item.sourceWorkItemId).forEach { followUp -> array.put(JSONObject()
                        .put("sourceId", followUp.id).put("type", followUp.type).put("title", followUp.title).put("dueDate", followUp.dueDate)
                        .put("privatePlanningNote", followUp.privatePlanningNote)) } })
