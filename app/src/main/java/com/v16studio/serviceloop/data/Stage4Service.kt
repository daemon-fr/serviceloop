...
class Stage4Service(
    ...
    suspend fun recordsCsvPackage(includeInactive: Boolean, includePrivate: Boolean, includePreviousRevisions: Boolean, customerId: String? = null): ByteArray {
        ...
        csvEntries["customers.csv"] = sheet(listOf("customer_id","reference","name","contact_name","phone","email","state","private_note"), customers.map { listOf(it.id,it.reference,it.name,it.contactName.orEmpty(),it.phone.orEmpty(),it.email.orEmpty(),it.state,if(includePrivate) it.privateNote.orEmpty() else "") })
