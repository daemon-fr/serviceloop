...
class Stage4Service(
    ...
    suspend fun applyLifecycle(subjectType: String, id: String, action: String, reason: String): Boolean = database.withTransaction {
        ...
        val changed = when (subjectType) {
            ...
            "PLAN" -> { val plan = dao.plan(id)!!; when (action) { "PAUSE" -> dao.changePlanState(id, "ACTIVE", "PAUSED", plan.currentObligationId); "RESUME" -> { require(plan.state == "PAUSED"); val equipment = dao.equipment(plan.equipmentId ?: error("Recurring service requires an Equipment"))!!; val site = dao.site(equipment.siteId)!!; require(equipment.state == "ACTIVE" && site.state == "ACTIVE" && dao.customer(site.customerId)?.state == "ACTIVE"); dao.changePlanState(id, "PAUSED", "ACTIVE", plan.currentObligationId) }; "END" -> dao.changePlanState(id, plan.state, "ENDED", null); else -> 0 } }
            else -> 0
        }
    ...
    suspend fun moveReview(equipmentId: String): MoveReview {
        ...
        val plans = dao.plansForEquipment(equipmentId).map { PlanDetail(it.id, it.equipmentId ?: error("Recurring service requires an Equipment"), equipment.name, it.reference, it.name, it.intervalCount, it.intervalUnit, it.currentDueDate, it.state, it.reusableTemplateId) }
    ...
    suspend fun recordsCsvPackage(includeInactive: Boolean, includePrivate: Boolean, includePreviousRevisions: Boolean, customerId: String? = null): ByteArray {
        ...
        csvEntries["service_plans.csv"] = sheet(listOf("plan_id","equipment_id","reference","name","interval_count","interval_unit","current_due","state","current_obligation_id","last_counted_revision_id"), dao.allPlans().filter { it.equipmentId in equipmentIds }.map { listOf(it.id,it.equipmentId ?: error("Recurring service requires an Equipment"),it.reference,it.name,it.intervalCount.toString(),it.intervalUnit,it.currentDueDate,it.state,it.currentObligationId.orEmpty(),it.lastCountedRevisionId.orEmpty()) })
