...
enum class DispatchVisitClassification { NEW_VISIT, UPDATE, ASSIGNMENT_REMOVED, ALREADY_CURRENT, OLDER_GENERATION, CONFLICT, LOCAL_CONFLICT, UPDATE_BLOCKED, NOT_ASSIGNED }
...
data class DispatchVisitPreview(val dispatchVisitId:String,val generation:Int,val classification:DispatchVisitClassification,val reasons:List<String>,val localVisitId:String?=null,val items:List<DispatchItemPreview> = emptyList(),val changes:List<DispatchFieldChange> = emptyList())
data class DispatchPreview(val value:DispatchPackage,val identity:TechnicianIdentity,val directory:List<DispatchPreviewLine>,val visits:List<DispatchVisitPreview>){val canImport get()=directory.none{it.classification==DispatchClassification.CONFLICT||(it.classification==DispatchClassification.POSSIBLE_DUPLICATE&&it.duplicateDecision==null)}&&visits.none{it.classification in setOf(DispatchVisitClassification.CONFLICT,DispatchVisitClassification.LOCAL_CONFLICT)}&&visits.any{it.classification in setOf(DispatchVisitClassification.NEW_VISIT,DispatchVisitClassification.UPDATE,DispatchVisitClassification.ASSIGNMENT_REMOVED)}}
...
class DispatchPackageService(
    ...
    suspend fun updateOutboxItem(itemId:String,equipmentId:String,task:String,plan:String?,due:String?,assigned:List<String>)=database.withTransaction{require(task.trim().isNotEmpty());val old=dispatch.outboxItems().find{it.dispatchItemId==itemId}?:error("Outbox item missing");val visit=dispatch.outboxVisit(old.dispatchVisitId)?:error("Outbox Visit missing");val equipment=dao.equipment(equipmentId)?:error("Equipment missing");require(equipment.siteId==visit.siteId);val participants=expand(visit.dispatchVisitId).first.map{it.technicianId}.toSet();require(assigned.all{it in participants}){"Assignee is outside selected Teams"};dispatch.clearOutboxItemAssignees(itemId);dispatch.insertOutboxItemAssignees(assigned.distinct().map{DispatchOutboxItemAssigneeEntity(itemId,it)});dispatch.updateOutboxItem(old.copy(equipmentId=equipmentId,taskName=task.trim(),servicePlanReference=plan?.trim()?.takeIf{it.isNotEmpty()},dueDateSnapshot=due?.trim()?.takeIf{it.isNotEmpty()}))}
    ...
    suspend fun preview(value:DispatchPackage):DispatchPreview {
        ...
        val visits=value.visits.map{v->
            ...
            val c=when{
                ...
                r.isEmpty()&&local?.state=="BOOKED"&&fingerprint(b.localVisitId)==b.controlledFingerprint->DispatchVisitClassification.ASSIGNMENT_REMOVED
            ...
            val reasons=when(c){
                ...
                DispatchVisitClassification.ASSIGNMENT_REMOVED->listOf("This generation removes this Technician assignment; the untouched local Visit will be retained as withdrawn history")
            ...
            DispatchVisitPreview(v.dispatchVisitId,v.generation,c,reasons,b?.localVisitId,itemPreviews,changes)
        }
    ...
    suspend fun import(p:DispatchPreview):DispatchImportResult {
        require(p.canImport);val created=mutableListOf<String>();val updated=mutableListOf<String>();val unchanged=mutableListOf<String>()
        database.withTransaction {
            ...
            val actionable=fresh.visits.filter{it.classification in setOf(DispatchVisitClassification.NEW_VISIT,DispatchVisitClassification.UPDATE,DispatchVisitClassification.ASSIGNMENT_REMOVED)}.associateBy{it.dispatchVisitId}
            val projected=p.value.visits.associateWith{v->applicable(v,fresh.identity).filterNot{it.first.equipmentReference in skip}}
            val activeVisits=p.value.visits.filter{v->v.dispatchVisitId in actionable&&v.siteReference !in skip&&(actionable[v.dispatchVisitId]!!.classification==DispatchVisitClassification.ASSIGNMENT_REMOVED||projected.getValue(v).isNotEmpty())}
            ...
            activeVisits.forEach{v->val pv=actionable.getValue(v.dispatchVisitId);val r=projected.getValue(v);when(pv.classification){
                ...
                DispatchVisitClassification.ASSIGNMENT_REMOVED->{applyAssignmentRemoval(v,p.value,pv.localVisitId!!,now);updated+=pv.localVisitId}
                else->Unit
            }}
        ...
        return DispatchImportResult(created,updated,unchanged.distinct())
    }
    ...
    private suspend fun applyAssignmentRemoval(v:DispatchVisit,p:DispatchPackage,local:String,now:Long){
        val visit=dao.visit(local)?:error("Visit missing");require(visit.state=="BOOKED"){"Assignment removal is only automatic for an untouched Booked Visit"}
        val binding=dispatch.visitBinding(v.dispatchVisitId)?:error("Dispatch binding missing");require(fingerprint(local)==binding.controlledFingerprint){"Local changes prevent automatic assignment removal"}
        val items=dispatch.itemBindings(v.dispatchVisitId);require(dao.visitWorkItems(local).map{it.id}.toSet()==items.mapNotNull{it.localWorkItemId}.toSet()){"Local work prevents automatic assignment removal"}
        items.forEach{item->item.localWorkItemId?.let{workId->require(!substantive(workId,binding));dispatch.deleteWorkItem(workId)};val incoming=v.work.find{it.dispatchItemId==item.dispatchItemId};dispatch.updateItemBinding(item.copy(localWorkItemId=null,assignedTechniciansJson=incoming?.let{techJson(it.assignedTechnicians)}?:item.assignedTechniciansJson,assignmentMeaning=incoming?.let{if(it.assignedTechnicians.isEmpty())"EVERYONE" else "EXPLICIT"}?:item.assignmentMeaning,localRole="WITHDRAWN",documentationDisposition="DEFERRED",deferredToTechnicianId=null,deferredToName=null))}
        check(dispatch.withdrawDispatchVisit(local,now)==1);dao.releaseVisitClaims(local)
        dispatch.updateVisitBinding(binding.copy(appliedGeneration=v.generation,packageId=p.packageId,senderLabel=p.senderLabel,managerReference=v.managerReference,instructionsSnapshot=v.instructions,participantSnapshotJson=techJson(v.participants),leaderIdsJson=idsJson(v.leaderTechnicianIds),teamSnapshotJson=teamsJson(v.teams),appliedMaterialHash=DispatchPackageCodec.materialHash(v),controlledFingerprint="withdrawn",updatedAtEpochMillis=now))
        appendEvent(visit,binding,"DISPATCH_ASSIGNMENT_WITHDRAWN","Generation ${v.generation} removed this Technician assignment; the central Visit was not marked cancelled",null,now)
    }
