...
enum class DispatchOutboxStatus { DRAFT, DISPATCHED, CONCLUDED }
...
data class DispatchPreview(val value:DispatchPackage,val identity:TechnicianIdentity,val directory:List<DispatchPreviewLine>,val visits:List<DispatchVisitPreview>){
    private val actionableClasses=setOf(DispatchVisitClassification.NEW_VISIT,DispatchVisitClassification.UPDATE,DispatchVisitClassification.ASSIGNMENT_REMOVED)
...
data class DispatchImportResult(val createdVisitIds:List<String>,val updatedVisitIds:List<String>,val unchangedVisitIds:List<String>,val withdrawnVisitIds:List<String> = emptyList())
...
class DispatchPackageService(
    ...
    suspend fun preview(value:DispatchPackage):DispatchPreview {
        ...
        val visits=value.visits.map{v->
            ...
            val site=value.sites.single{it.reference==v.siteReference};val dependencies=if(c==DispatchVisitClassification.ASSIGNMENT_REMOVED) emptySet() else (setOf(v.siteReference,site.customerReference)+r.map{it.first.equipmentReference}).toSet()
    ...
    suspend fun import(p:DispatchPreview):DispatchImportResult {
        require(p.canImport);val created=mutableListOf<String>();val updated=mutableListOf<String>();val unchanged=mutableListOf<String>();val withdrawn=mutableListOf<String>()
        ...
        return DispatchImportResult(created,updated,unchanged.distinct(),withdrawn)
    }
