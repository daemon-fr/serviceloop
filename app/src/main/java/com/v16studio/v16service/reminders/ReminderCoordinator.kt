package com.v16studio.v16service.reminders
...
import com.v16studio.v16service.MainActivity
...
import com.v16studio.v16service.data.toDomain
import com.v16studio.v16service.domain.BusinessTime
import com.v16studio.v16service.domain.DailySummaryCounts
import com.v16studio.v16service.domain.DailySummaryText
import com.v16studio.v16service.domain.ReminderPreferences
import com.v16studio.v16service.domain.ReminderRuntimeState
import com.v16studio.v16service.domain.ReminderScheduleRules
import com.v16studio.v16service.domain.AppointmentReminderRules
import com.v16studio.v16service.domain.DailySummaryGate
...
class ReminderCoordinator(
    ...
    suspend fun executeSummary(expectedDataset: String, expectedDate: String) = withContext(Dispatchers.IO) {
        try {
            val dao = database.v16ServiceDao(); val metadata = dao.recoveryMetadata() ?: return@withContext
            ...
            post(NOTIFICATION_SUMMARY, CHANNEL_SUMMARIES, "V16 Service", text, homeIntent(expectedDataset))
    ...
    suspend fun executeAppointment(expectedDataset: String, visitId: String) = withContext(Dispatchers.IO) {
        ...
        post(appointmentNotificationId(visit.id), CHANNEL_APPOINTMENTS, "V16 Service appointment", "Visit scheduled for $time", visitIntent(expectedDataset, visit.id))
    }
    ...
    fun sendTestNotification(): Boolean {
        ...
        ensureChannels()
        post(NOTIFICATION_TEST, CHANNEL_SUMMARIES, "V16 Service test", "Test notification requested from this device. Delivery may be delayed or blocked by Android.", homeIntent(null))
        return true
    }
    ...
    private fun cancelAllAndWithdraw() {
        ...
        scope.launch(Dispatchers.IO) { database.v16ServiceDao().recoveryMetadata()?.let { metadata -> val visits=database.v16ServiceDao().reminderVisits(); cancelScheduled(visits, metadata.datasetId); visits.forEach { NotificationManagerCompat.from(context).cancel(appointmentNotificationId(it.id)) } } }
    }

    private fun receiverIntent(type: String, datasetId: String, item: String) = Intent(context, ReminderReceiver::class.java).apply {
        data = Uri.Builder().scheme("v16service").authority("reminder").appendPath(datasetId).appendPath(type).appendPath(item).build()
    }
    ...
    private fun post(id: Int, channel: String, title: String, text: String, content: PendingIntent) {
        ...
        val notification = NotificationCompat.Builder(context, channel).setSmallIcon(com.v16studio.v16service.R.mipmap.ic_launcher).setContentTitle(title).setContentText(text).setStyle(NotificationCompat.BigTextStyle().bigText(text)).setContentIntent(content).setAutoCancel(true).setPriority(NotificationCompat.PRIORITY_DEFAULT).build()
    ...
    private fun ensureChannels() {
        ...
        notificationManager.createNotificationChannels(listOf(
            NotificationChannel(CHANNEL_SUMMARIES, "Work summaries", NotificationManager.IMPORTANCE_DEFAULT).apply { description = "Approximate daily V16 Service work summaries" },
            NotificationChannel(CHANNEL_APPOINTMENTS, "Appointment reminders", NotificationManager.IMPORTANCE_DEFAULT).apply { description = "Approximate V16 Service appointment reminders" },
        ))
    }
    ...
    companion object {
        const val CHANNEL_SUMMARIES = "v16service_work_summaries"
        const val CHANNEL_APPOINTMENTS = "v16service_appointment_reminders"
        const val EXTRA_ROUTE = "v16service.reminder.ROUTE"
        const val EXTRA_DATASET = "v16service.reminder.DATASET"
        private const val DEVICE_PREFS = "v16service_reminder_device"
...
class ReminderReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val pending = goAsync()
        val app = context.applicationContext as V16ServiceApplication
