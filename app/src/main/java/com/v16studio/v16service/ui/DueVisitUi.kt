...
import com.v16studio.v16service.ui.designsystem.V16ServiceWorkItemRow
    ...
    if (!state.dueServicesReady) {
        Column(modifier.fillMaxSize().padding(padding)) {
            Box(Modifier.weight(1f).fillMaxWidth(), contentAlignment = Alignment.Center) {
                Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.md)) {
                    Text(state.dueServicesError?.let { "Unable to read due services — $it" } ?: "Reading due services")
                    state.dueServicesError?.let { V16ServicePrimaryButton("Retry", { viewModel.retryDueServices() }, Modifier.testTag("retry-due-services")) }
                }
            }
        }
        return
            ...
            item {
                ...
                if (initialOperationalState != null) {
                    ...
                    V16ServiceFilterSelectorRow(
                        first = {
                            V16ServiceFilterSelector("Due date", dateFilter, DueServiceDateFilter.entries.map { it to it.label }, { dateFilter = it; if (remembersFilters) filterPreferences.saveDueDate(it) }, testTag = "due-date-selector")
                        },
                        second = {
                            V16ServiceFilterSelector("Visit", visitFilter, DueServiceVisitFilter.entries.map { it to it.label }, { visitFilter = it; if (remembersFilters) filterPreferences.saveDueVisit(it) }, testTag = "due-visit-selector")
                        },
                    )
                }
...
@Composable
private fun DueServiceSelectionActions(selected: List<String>, enabled: Boolean, canPerformFieldWork: Boolean, state: UiState, viewModel: V16ServiceViewModel, nav: NavHostController, onClear: () -> Unit) {
    Column(
        ...
        V16ServiceAdaptiveActionRow(
            actions = listOf(
                { V16ServiceSecondaryButton("Book selected", { nav.currentBackStackEntry?.savedStateHandle?.set("visit-setup-plan-ids", ArrayList(selected)); nav.navigate("visit/new") }, enabled = enabled, modifier = Modifier.testTag("book-selected-services")) },
                { if (canPerformFieldWork) V16ServicePrimaryButton("Start selected", { viewModel.createVisit(selected, "WORKING", state.businessDate.toString(), null) { nav.navigate("visit/$it") } }, enabled = enabled, modifier = Modifier.testTag("start-selected-services")) },
                { V16ServiceSecondaryButton("Clear", onClear, modifier = Modifier.testTag("clear-selected-services")) },
            ),
    ...
    Card {
        Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Row(
                ...
                V16ServiceIcon(V16ServiceIcons.CaretDown, null, Modifier.size(V16ServiceUiTokens.Size.icon).graphicsLayer { rotationZ = if (expanded) 180f else 0f })
            }
            if (expanded) {
            DailyField(taskName, { taskName = it }, "Task name · Required")
            Text("Subject", fontWeight = FontWeight.Medium)
            V16ServiceChoicePair(listOf(WorkSubjectType.SITE to "Site", WorkSubjectType.EQUIPMENT to "Equipment"), subjectType, { value -> subjectType = value; if (value == WorkSubjectType.SITE) { equipmentId = null; equipmentDescription = "" } }, testTagPrefix = "visit-task-subject")
            if (subjectType == WorkSubjectType.EQUIPMENT) {
                ...
                else V16ServiceChoiceGroup(listOf<Pair<String?, String>>(null to "No specific equipment yet") + equipment.map { it.id to "${it.name} · ${it.reference}" }, equipmentId, { equipmentId = it; equipmentDescription = "" }, testTagPrefix = "visit-task-equipment")
...
private fun VisitDateLandmark(state: String, persistedDate: String, scheduledAtEpochMillis: Long?, appointmentZoneId: String?, businessZoneId: String) {
    ...
    Column(Modifier.fillMaxWidth().testTag("visit-date-landmark"), verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.xs)) {
        Text(if (state == "BOOKED") "APPOINTMENT" else "SERVICE DATE", style = V16ServiceUiTokens.Type.dayHeading, color = colors.action, modifier = Modifier.testTag("visit-date-landmark-label"))
        val zone = runCatching { java.time.ZoneId.of(appointmentZoneId ?: businessZoneId) }.getOrDefault(java.time.ZoneId.systemDefault())
        Text(listOfNotNull(formatV16ServiceDate(persistedDate), formatAppointmentTime(scheduledAtEpochMillis, zone)).joinToString(" · "), style = V16ServiceUiTokens.Type.screenTitle, modifier = Modifier.testTag("visit-date-landmark-value"))
    }
}
...
@Composable
internal fun VisitDetailScreen(detail: VisitDetail?, padding: PaddingValues, state: UiState, viewModel: V16ServiceViewModel, nav: NavHostController) {
    ...
    LazyColumn(Modifier.padding(padding).testTag("visit-detail-list"), contentPadding = PaddingValues(16.dp, 8.dp, 16.dp, 32.dp), verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.section)) {
         ...
         item {
             V16ServiceAdaptiveActionRow(
                 actions = listOf(
                     { V16ServiceNavigationButton("Customer", { nav.navigate("customer/${detail.customerId}") }, Modifier.testTag("visit-customer-link")) },
                     { V16ServiceNavigationButton("Site", { nav.navigate("site/${detail.siteId}") }, Modifier.testTag("visit-site-link")) },
                     { V16ServiceSecondaryButton("Maps", { handoffStatus = handoff(context, visitMapsIntent(detail.siteAddress), "maps", "Opened maps.", "No compatible maps app is available.") }, Modifier.testTag("visit-maps-link"), enabled = detail.siteAddress.isNotBlank()) },
                 ),
        ...
        item { DispatchVisitPanel(detail) }
         item { val calendar=state.visitCalendarState;Card(Modifier.fillMaxWidth().testTag("visit-calendar")){Column(Modifier.padding(12.dp)){Text("Calendar",fontWeight=FontWeight.Bold);Text(calendar?.label?:"Checking Calendar status", modifier = Modifier.testTag("visit-calendar-status"));if(calendar?.label=="Calendar integration is off") V16ServiceNavigationButton("Calendar settings",{nav.navigate("calendar")},Modifier.fillMaxWidth().testTag("visit-calendar-settings"));when(calendar?.action){"Add to Calendar","Recreate event"->OutlinedButton({viewModel.addVisitToCalendar(detail.id)},Modifier.fillMaxWidth().testTag("visit-calendar-add")){Text(calendar.action)};"Remove from Calendar"->OutlinedButton({viewModel.removeVisitFromCalendar(detail.id)},Modifier.fillMaxWidth().testTag("visit-calendar-remove")){Text(calendar.action)}};calendar?.eventId?.let{id->TextButton({viewModel.calendarEventIntent(id)?.let(context::startActivity)}){Text("Open Calendar event")}}}} }
        val serviceProgress = state.serviceProgress
         if (capabilities.canPerformFieldWork && serviceProgress?.visitId == detail.id) item {
            VisitServiceProgressOverview(serviceProgress, onSelect = { item ->
             ...
             if (!capabilities.canPerformFieldWork && detail.state == "WORKING") item { V16ServiceNotice("Field work is performed by technicians", "This role can inspect and coordinate this Visit, but cannot execute its Services.", V16ServiceNoticeKind.Info) }
             items(detail.lines) { line -> V16ServiceWorkItemRow(v16ServiceSubjectLabel(line.subjectType, line.equipmentName, line.equipmentReference, line.equipmentDescription),line.serviceName,"Due ${line.dueDate ?: "one-off"} · ${line.outcome?.lowercase()?.replace('_',' ') ?: detail.state.lowercase().replaceFirstChar(Char::uppercase)}",capabilities.canPerformFieldWork && detail.state=="WORKING",Modifier.testTag("visit-line-${line.workItemId}")){if(capabilities.canPerformFieldWork) nav.navigate("inspection/${line.workItemId}")} }
        }
        ...
        if (detail.state == "BOOKED") item {
            ...
            Spacer(Modifier.height(V16ServiceUiTokens.Space.md))
            ...
            Column(verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.md)) {
                ...
                if (rescheduleSaved) Row(Modifier.align(Alignment.End).testTag("reschedule-saved"), verticalAlignment = Alignment.CenterVertically) {
                    V16ServiceIcon(V16ServiceIcons.LocalSaved, null, Modifier.size(V16ServiceUiTokens.Size.iconSmall), MaterialTheme.colorScheme.primary)
        ...
        if (capabilities.canPerformFieldWork && detail.state == "WORKING") item {
            ...
            V16ServicePrimaryButton("Resume service", { if (target != null) nav.navigate("inspection/$target") }, Modifier.fillMaxWidth().testTag("resume-service"), enabled = target != null)
            V16ServiceSecondaryButton("Review visit", {
                scope.launch {
            ...
            }, Modifier.fillMaxWidth().testTag("visit-review"), enabled = state.serviceProgress?.let { progress -> progress.actionableItems.isNotEmpty() && progress.actionableItems.all { it.status == com.v16studio.v16service.domain.ServiceEntryStatus.READY } } == true)
            }
