...
@Composable
internal fun V16ServiceNavGraph(
    nav: NavHostController,
    state: UiState,
    viewModel: V16ServiceViewModel,
    appearanceController: AppearancePreferences,
    incomingV16ServiceSync: String?,
    incomingV16ServiceSyncEvent: Int,
) {
    val context = LocalContext.current
    val teamRole by rememberTeamRoleState(context)
    val capabilities = teamRole.workspaceCapabilities
    LaunchedEffect(incomingV16ServiceSync, incomingV16ServiceSyncEvent, state.restrictedRecoveryState) {
        if (!state.restrictedRecoveryState && incomingV16ServiceSync != null) nav.navigate("import") { launchSingleTop = true }
    }
    ...
    NavHost(
            ...
            DetailScaffold(customer?.name ?: "Customer", nav) { padding ->
                ...
                if (projection == null) {
                    ...
                    androidx.compose.foundation.lazy.LazyColumn(
                        ...
                        verticalArrangement = androidx.compose.foundation.layout.Arrangement.spacedBy(V16ServiceUiTokens.Space.md),
                    ) {
                        item {
        ...
        composable("customer/contacts/{id}") { entry -> WorkspaceGate(capabilities.canManageRegister, "Customer management", nav) { val id=entry.arguments?.getString("id").orEmpty(); LaunchedEffect(id){viewModel.loadCustomer(id)}; DetailScaffold("List contacts",nav){CustomerContactsScreen(state.customer,it,state,viewModel)} } }
        ...
        composable("visit/new") { entry -> WorkspaceGate(capabilities.canCreateVisits, "Local Visit creation", nav) { val ids=remember(entry){nav.previousBackStackEntry?.savedStateHandle?.remove<ArrayList<String>>("visit-setup-plan-ids")?.toList().orEmpty()}; LaunchedEffect(Unit){viewModel.loadVisitSetup()}; DetailScaffold("Create visit",nav){NewVisitScreen(state.visitSites,state.dueServices,it,state,viewModel,nav,ids)} } }
        composable("visit/new/{planId}") { entry -> WorkspaceGate(capabilities.canCreateVisits, "Local Visit creation", nav) { val id=entry.arguments?.getString("planId").orEmpty(); LaunchedEffect(id){viewModel.loadVisitSetup()}; DetailScaffold("Create visit",nav){NewVisitScreen(state.visitSites,state.dueServices,it,state,viewModel,nav,listOf(id))} } }
        ...
        composable("inspection/{id}") { entry -> WorkspaceGate(capabilities.canPerformFieldWork, "Technician field work", nav) {
            ...
            DetailScaffold("Service", nav) { padding ->
                ScreenState(state.loading, state.error, padding) { state.inspection?.let { draft ->
                    ServiceScreen(draft, state.serviceProgress, state.saveStatus, state.inspectionFocus, viewModel, nav, PaddingValues(), LocalDetailBackInterceptor.current, scrollHandle = entry.savedStateHandle)
                } }
            }
        } }
        ...
        composable("team/id") { WorkspaceGate(teamRole != TeamRole.SOLO, "V16 Service ID", nav) { DetailScaffold(teamRole.idHeading(), nav) { TeamWorkspaceScreen(nav, Modifier.padding(it), TeamWorkspaceContent.ID) } } }
        composable("team/trusted-ids") { WorkspaceGate(teamRole != TeamRole.SOLO, "Trusted IDs", nav) { DetailScaffold("Trusted IDs", nav) { TeamWorkspaceScreen(nav, Modifier.padding(it), TeamWorkspaceContent.TRUSTED_IDS) } } }
        ...
        composable("dispatch/visit/new") { WorkspaceGate(capabilities.canCreateVisits, "Local Visit creation", nav) { LaunchedEffect(Unit) { viewModel.loadVisitSetup() }; DetailScaffold("Create visit",nav){NewVisitScreen(state.visitSites,state.dueServices,it,state,viewModel,nav)} } }
        composable("dispatch/visit/{id}") { entry -> WorkspaceGate(capabilities.canUseCoordinatorTools, "Coordinator tools", nav) { val id=entry.arguments?.getString("id").orEmpty(); LaunchedEffect(id) { viewModel.loadVisitSetup() }; DetailScaffold("Dispatch visit",nav){DispatchVisitEditorScreen(it,nav,id,state.businessDate,canConcludeDelegatedWork=capabilities.canConcludeDelegatedWork,sitesOverride=state.visitSites,dueServicesOverride=state.dueServices,templatesOverride=state.templates,businessZoneId=state.businessZoneId,plannedWorkState=state.dueServicesProjection,onRetryDueServices=viewModel::retryDueServices,onRefreshTemplates=viewModel::loadTemplates)} } }
         ...
         composable("dispatch/import") { WorkspaceGate(capabilities.canReceiveAssignedWork, "Assigned work import", nav) { DetailScaffold("Import assigned work", nav) { padding -> ImportDispatchPackageScreen(padding, nav, viewModel) } } }
         composable("dispatch/import/pick") { WorkspaceGate(capabilities.canReceiveAssignedWork, "Assigned work import", nav) { DetailScaffold("Import assigned work", nav) { padding -> ImportDispatchPackageScreen(padding, nav, viewModel, openPickerImmediately = true) } } }
        composable("business-profile") { WorkspaceGate(teamRole != TeamRole.EMPLOYEE, "Business and report identity", nav) { LaunchedEffect(Unit) { viewModel.loadBusinessProfile() }; DetailScaffold("Business and report identity", nav) { padding -> BusinessProfileScreen(state.businessProfile, state.businessProfileSaveStatus, padding, viewModel, nav) } } }
        ...
        composable("history/global") { DetailScaffold("History", nav) { padding -> HistoryScreen(com.v16studio.v16service.domain.HistoryScope(com.v16studio.v16service.domain.HistoryScopeType.GLOBAL), state, padding, viewModel, nav) } }
        composable("history/{scope}/{id}") { entry -> val scope = runCatching { com.v16studio.v16service.domain.HistoryScopeType.valueOf(entry.arguments?.getString("scope").orEmpty()) }.getOrDefault(com.v16studio.v16service.domain.HistoryScopeType.GLOBAL); val id = entry.arguments?.getString("id"); DetailScaffold("History", nav) { padding -> HistoryScreen(com.v16studio.v16service.domain.HistoryScope(scope, id, "${scope.name.lowercase()} history"), state, padding, viewModel, nav) } }
        ...
        composable("csv/export") { WorkspaceGate(teamRole in setOf(TeamRole.SUBCONTRACTOR, TeamRole.TEAM_LEADER, TeamRole.COORDINATOR), "Data export", nav) { DetailScaffold("Export readable CSV", nav) { padding -> CsvExportScreen(state, padding, viewModel) } } }
        ...
        composable("import") { WorkspaceGate(teamRole != TeamRole.SOLO, "Import shared data", nav) { DetailScaffold("Import shared data", nav) { padding -> V16ServiceSyncScreen(state, padding, viewModel, nav, incomingV16ServiceSync) } } }
        composable("work-results/export") { WorkspaceGate(capabilities.canReceiveAssignedWork, "Work result export", nav) { DetailScaffold("Export work results", nav) { padding -> WorkResultExportScreen(padding) } } }
        composable("aggregate-report/new") { WorkspaceGate(capabilities.canUseCoordinatorTools, "Aggregate customer report", nav) { DetailScaffold("Generate report", nav) { padding -> AggregateReportScreen(padding) } } }
        composable("image-cleanup") { DetailScaffold("Image cleanup", nav) { padding -> ImageCleanupScreen(padding) } }
        composable("data-transfer") { WorkspaceGate(teamRole != TeamRole.SOLO, "Data transfer", nav) { DetailScaffold("Import / export data", nav) { padding -> DataTransferScreen(padding, nav) } } }
        composable("data-transfer/export") { WorkspaceGate(teamRole in setOf(TeamRole.SUBCONTRACTOR, TeamRole.TEAM_LEADER, TeamRole.COORDINATOR), "Data export", nav) { DetailScaffold("Export data", nav) { padding -> DataExportScreen(state, padding, nav) } } }
        composable("data-transfer/templates") { WorkspaceGate(teamRole in setOf(TeamRole.SUBCONTRACTOR, TeamRole.TEAM_LEADER, TeamRole.COORDINATOR), "Template export", nav) { DetailScaffold("Export inspection templates", nav) { padding -> TemplateExportScreen(state.templates, padding, nav) } } }
