...
internal fun ServiceScreen(
    ...
    var leaveFlushResult by remember { mutableStateOf<com.v16studio.v16service.ui.service.ServiceDraftFlushResult?>(null) }
    ...
    BoxWithConstraints(Modifier.fillMaxSize().padding(padding)) {
        ...
        val movePrimaryActionIntoList = maxHeight < V16ServiceUiTokens.Size.compactHeightThreshold || imeVisible

        @Composable
        fun PrimaryServiceAction(modifier: Modifier = Modifier) {
            if (next != null) {
                V16ServicePrimaryButton("Next service", { flushAndThen { replaceServiceDestination(nav, next.workItemId) } }, modifier.testTag("next-service"))
            } else {
        ...
        LazyColumn(
            ...
            navigationMessage?.let { message -> item { Box(Modifier.fillMaxWidth().padding(horizontal = 16.dp)) { V16ServiceNotice("Resolve unsaved service edits before continuing.", message, V16ServiceNoticeKind.Error) } } }
            item {
            ...
            item {
                Box(Modifier.fillMaxWidth().padding(horizontal = 16.dp)) { V16ServiceSurfaceCard(modifier = Modifier.testTag("work-performed-section")) {
                    Text(if (editingEnabled && com.v16studio.v16service.data.ServiceWorkEvaluator.requiresPublicWork(completion?.outcome)) "Work performed · Required" else "Work performed", style = MaterialTheme.typography.titleLarge, modifier = Modifier.testTag("work-performed-heading"))
                     Text("Included in customer report", style = MaterialTheme.typography.bodySmall, color = LocalV16ServiceTokens.current.textSecondary)
            ...
            item {
                ...
                if (!privateExpanded) {
                    Box(Modifier.fillMaxWidth().padding(horizontal = 16.dp)) { V16ServiceTextAction("+ Private note", { privateExpanded = true }, Modifier.testTag("add-private-note"), enabled = editingEnabled) }
                } else {
                    Box(Modifier.fillMaxWidth().padding(horizontal = 16.dp)) { V16ServiceSurfaceCard(modifier = Modifier.testTag("private-work-note")) {
                        V16ServicePrivateLabel("Private note", style = MaterialTheme.typography.titleMedium)
                        V16ServiceLongTextEditor(
                            value = privateNote,
        ...
        if (!movePrimaryActionIntoList) {
            V16ServicePinnedBar { PrimaryServiceAction(Modifier.fillMaxWidth()) }
        }
        }
    }
}
...
@Composable
internal fun InspectionScreen(draft: InspectionDraft, saveStatus: SaveStatus, focus: InspectionFocus?, viewModel: V16ServiceViewModel, nav: NavHostController) {
