...
import com.v16studio.v16service.domain.SearchTarget
...
import com.v16studio.v16service.ui.designsystem.V16ServiceTextFieldAdapter
    ...
    LazyColumn(
        ...
        item {
            V16ServiceTextFieldAdapter(
                ...
                singleLine = true,
                leadingIcon = { V16ServiceIcon(V16ServiceIcons.Search, null, Modifier.size(V16ServiceUiTokens.Size.icon), LocalV16ServiceTokens.current.icon) },
            )
...
private fun SearchCategoryStrip(
    ...
    Row(
        Modifier.fillMaxWidth()
            ...
            .padding(vertical = V16ServiceUiTokens.Space.sm),
...
private fun SearchRecentPanel(
    ...
    Surface(
        ...
        shadowElevation = V16ServiceUiTokens.Elevation.menu,
        shape = V16ServiceUiTokens.Shapes.small,
        border = androidx.compose.foundation.BorderStroke(V16ServiceUiTokens.Stroke.outline, colors.outlineDecorative),
    ) {
        Column(
            Modifier.fillMaxWidth().heightIn(max = 320.dp).verticalScroll(rememberScrollState()).padding(vertical = V16ServiceUiTokens.Space.sm),
        ) {
            Text("Recent searches", style = MaterialTheme.typography.labelLarge, color = colors.textSecondary, modifier = Modifier.padding(horizontal = V16ServiceUiTokens.Space.md, vertical = V16ServiceUiTokens.Space.xs))
            queries.forEachIndexed { index, query ->
                Row(
                    Modifier.fillMaxWidth().testTag("search-recent-$index").clickable(onClick = { onSelect(query) }).padding(horizontal = V16ServiceUiTokens.Space.md, vertical = V16ServiceUiTokens.Space.sm),
                    verticalAlignment = Alignment.CenterVertically,
                    horizontalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.sm),
                ) {
                    V16ServiceIcon(V16ServiceIcons.History, null, Modifier.size(V16ServiceUiTokens.Size.iconSmall), colors.icon)
            ...
            Text(
                ...
                color = colors.action,
                modifier = Modifier.fillMaxWidth().testTag("clear-recent-searches").clickable(onClick = onClear).padding(horizontal = V16ServiceUiTokens.Space.md, vertical = V16ServiceUiTokens.Space.sm),
            )
        }
    }
}
