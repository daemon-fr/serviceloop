...
import com.v16studio.v16service.ui.designsystem.V16ServiceCheckbox
import com.v16studio.v16service.ui.designsystem.V16ServicePrimaryButton
import java.io.File

@Composable
internal fun ServicePhotoManagementScreen(workItemId: String, state: UiState, padding: PaddingValues, viewModel: V16ServiceViewModel) {
    ...
    Column(Modifier.fillMaxSize().padding(padding).testTag("service-photo-management")) {
        Column(Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 8.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (selected.isNotEmpty()) {
                ...
                Text("Tap a photo to view it. Select photos to manage report visibility or remove V16 Service’s stored copies.", style = MaterialTheme.typography.bodyMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
    ...
    if (confirmDelete) AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                Text("${selected.size} v16service-stored ${if (selected.size == 1) "copy will" else "copies will"} be deleted from this Service. The original photos in your source gallery will not be affected.")
...
@Composable
private fun ServicePhotoViewer(photos: List<PhotoEntry>, initialIndex: Int, context: android.content.Context, viewModel: V16ServiceViewModel, workItemId: String, onClose: (Int?) -> Unit) {
        ...
        Surface(Modifier.fillMaxSize().testTag("service-photo-viewer"), color = MaterialTheme.colorScheme.surface) {
            Column(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    V16ServiceCheckbox(photo.includedInReport, { include -> viewModel.setPhotoReportInclusion(workItemId, photo.id, include) }, modifier = Modifier.testTag("viewer-photo-report-toggle"), contentDescription = "Include photo in customer report")
