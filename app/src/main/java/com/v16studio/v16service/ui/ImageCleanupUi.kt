...
import com.v16studio.v16service.data.ImageRetention
...
internal fun ImageCleanupScreen(padding: PaddingValues) {
    ...
    LazyColumn(Modifier.padding(padding).testTag("image-cleanup"), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        item {
            ...
            Text("V16 Service keeps a report-quality copy for finalized work. Working photos are never automatically removed.")
        ...
        ImageRetention.entries.forEach { option -> item { V16ServiceSelectionOption(choice == option, { choice = option }, option.title, Modifier.testTag("image-retention-${option.name.lowercase()}")) } }
        item {
            V16ServiceActionStack {
                V16ServicePrimaryButton("Save image cleanup setting", { scope.launch {
                ...
                V16ServicePrimaryButton("Run cleanup now", { scope.launch {
                    busy = true; message = null
