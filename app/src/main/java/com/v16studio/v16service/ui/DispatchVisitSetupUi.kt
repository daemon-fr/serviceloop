...
import com.v16studio.v16service.data.DispatchOutboxEditorDraft
import com.v16studio.v16service.data.DispatchOutboxItemDraft
import com.v16studio.v16service.data.DispatchOutboxStatus
...
private data class DispatchLoadedSetup(
    ...
    val visit: com.v16studio.v16service.data.DispatchOutboxVisitEntity?,
...
internal fun DispatchVisitEditorScreen(
    ...
    databaseOverride: V16ServiceDatabase? = null,
    ...
    val database = databaseOverride ?: (context.applicationContext as V16ServiceApplication).container.database
    ...
    var loadedVisit by remember { mutableStateOf<com.v16studio.v16service.data.DispatchOutboxVisitEntity?>(null) }
    ...
    VisitSetupForm(
        ...
        preludeItems = {
            if (loadedVisit != null) {
                item {
                    ...
                    V16ServiceStatusBadge(visit.outboxStatus.name)
                    if (visit.outboxStatus == DispatchOutboxStatus.DISPATCHED) V16ServiceNotice("Dispatched", "Material changes create a later version only when this visit is exported again.", V16ServiceNoticeKind.Info)
                    if (loadedVisit?.localVisitId != null) V16ServiceNavigationButton("Open canonical Visit", { nav.navigate("visit/${loadedVisit?.localVisitId}") }, Modifier.fillMaxWidth())
                    if (readOnly) V16ServiceNotice("Read-only", if (loadedVisit?.localVisitId != null) "Edit this Visit from its canonical record." else "Canceled and Concluded visits cannot be edited.", V16ServiceNoticeKind.Info)
                }
            }
        },
        extensionItems = {
            item {
                Column(Modifier.fillMaxWidth().padding(horizontal = V16ServiceUiTokens.Layout.pageInsetCompact), verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.sm)) {
                    V16ServiceSectionHeading("Assignment")
                    val teamLabel = teams.filter { it.team.id in selectedTeams }.joinToString(" · ") { it.team.name }.ifBlank { "Choose teams" }
                    V16ServiceSecondaryButton(teamLabel, { showTeams = true }, Modifier.fillMaxWidth().testTag("dispatch-choose-teams"), enabled = !readOnly && !busy)
                    ...
                    if (unmatchedItems.isNotEmpty()) {
                        V16ServiceNotice("Planned work preserved", "Some saved plan items are not in the current due-work projection. They remain unchanged and will not be silently converted.", V16ServiceNoticeKind.Warning)
                    }
                }
            }
            item {
                var expanded by rememberSaveable(visitId) { mutableStateOf(false) }
                Column(Modifier.fillMaxWidth().padding(horizontal = V16ServiceUiTokens.Layout.pageInsetCompact).clickable(enabled = !readOnly) { expanded = !expanded }.padding(vertical = V16ServiceUiTokens.Space.md), verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.xs)) {
                    ...
                    if (expanded) {
                        ...
                        V16ServiceTextField(manager, { manager = it }, "Reference", enabled = !readOnly, modifier = Modifier.testTag("dispatch-manager-reference"))
                        V16ServiceLongTextEditor(instructions, { instructions = it }, "Instructions", private = false, enabled = !readOnly)
                    }
                }
            }
        },
        actionItems = {
            item {
                V16ServicePinnedBar {
                    if (!readOnly) V16ServicePrimaryButton(if (visitId == null) "Save draft" else "Save changes", ::save, Modifier.fillMaxWidth().testTag("dispatch-save-visit"), enabled = !busy && commonValid && selectedTeams.isNotEmpty() && participants.isNotEmpty(), busy = busy)
                    if (loadedVisit?.outboxStatus in setOf(DispatchOutboxStatus.DRAFT, DispatchOutboxStatus.DISPATCHED)) V16ServiceSecondaryButton("Cancel visit", { cancelReason = ""; showCancel = true }, Modifier.fillMaxWidth().testTag("dispatch-cancel-visit"), enabled = !busy)
                    if (canConcludeDelegatedWork && loadedVisit?.outboxStatus == DispatchOutboxStatus.DISPATCHED) V16ServiceSecondaryButton("Mark concluded", { scope.launch { runCatching { withContext(Dispatchers.IO) { service.concludeOutboxVisits(listOf(requireNotNull(visitId))) } }.onSuccess { nav.popBackStack() }.onFailure { if (it is CancellationException) throw it else error = it.message } } }, Modifier.fillMaxWidth().testTag("dispatch-conclude-visit"), enabled = !changed && !busy)
                    if (canConcludeDelegatedWork && loadedVisit?.outboxStatus == DispatchOutboxStatus.CONCLUDED) V16ServicePrimaryButton("Reopen", { scope.launch { runCatching { withContext(Dispatchers.IO) { service.reopenOutboxVisits(listOf(requireNotNull(visitId))) } }.onSuccess { load() }.onFailure { if (it is CancellationException) throw it else error = it.message } } }, Modifier.fillMaxWidth().testTag("dispatch-reopen-visit"))
                }
            }
        },
    )
...
internal fun DispatchTeamSelectionDialog(teams: List<DispatchTeamDetail>, selected: Set<String>, onDismiss: () -> Unit, onApply: (Set<String>) -> Unit) {
    ...
    AlertDialog(
        ...
        dismissButton = if (teams.isEmpty()) null else ({ V16ServiceTextAction("Cancel", onDismiss) }),
        confirmButton = if (teams.isEmpty()) ({ V16ServiceTextAction("Close", onDismiss) }) else ({ V16ServicePrimaryButton("Apply", { onApply(staged) }, Modifier.testTag("dispatch-team-apply")) }),
    )
}

@Composable
internal fun DispatchAssigneeDialog(participants: List<com.v16studio.v16service.data.DispatchTechnicianEntity>, selected: Set<String>, onDismiss: () -> Unit, onApply: (Set<String>) -> Unit) {
    var staged by remember(selected) { mutableStateOf(selected) }
    AlertDialog(
        onDismissRequest = onDismiss, title = { Text("Assign work") },
        text = { Column(verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.xs)) { Text("Leave everyone unchecked to assign this work to everyone on the selected Teams.", style = MaterialTheme.typography.bodySmall); participants.forEach { person -> Row(Modifier.fillMaxWidth().heightIn(min = V16ServiceUiTokens.Size.touchMin).clickable { staged = if (person.technicianId in staged) staged - person.technicianId else staged + person.technicianId }.padding(vertical = V16ServiceUiTokens.Space.xs), verticalAlignment = Alignment.CenterVertically) { Checkbox(person.technicianId in staged, null, contentDescription = "Assign ${person.displayName}"); Text(person.displayName) } } } },
        dismissButton = { V16ServiceTextAction("Cancel", onDismiss) },
        confirmButton = { V16ServicePrimaryButton("Apply", { onApply(staged) }) },
    )
}
...
private fun DispatchVisitCancelDialog(reason: String, dispatched: Boolean, onDismiss: () -> Unit, onApply: (String) -> Unit) {
    ...
    AlertDialog(
        ...
        text = { Column(verticalArrangement = Arrangement.spacedBy(V16ServiceUiTokens.Space.sm)) { V16ServiceTextField(staged, { staged = it }, "Cancellation reason", modifier = Modifier.testTag("dispatch-cancel-reason")); Text(if (dispatched) "A newer work package must be created to communicate this cancellation to technicians." else "This Draft stays in local cancellation history and is not exported.", color = LocalV16ServiceTokens.current.textSecondary) } },
        dismissButton = { V16ServiceTextAction("Cancel", onDismiss) },
        confirmButton = { V16ServicePrimaryButton("Cancel visit", { onApply(staged.trim()) }, Modifier.testTag("dispatch-cancel-confirm"), enabled = staged.isNotBlank()) },
    )
}
