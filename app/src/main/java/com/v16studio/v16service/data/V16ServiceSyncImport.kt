...
internal class V16ServiceSyncImporter(
    ...
    private val dao get() = database.v16ServiceDao()
    private val dispatch get() = database.dispatchDao()

    suspend fun apply(value: V16ServiceSyncPackage): SyncImportResult = BusinessFileCoordinator.mutex.withLock {
        ...
        V16ServiceSyncCodec.preview(value).also { validateReferences(value) }
        writeGate.beforeWrite()
        val importedAt = businessTime.instant().toEpochMilli()
        database.withTransaction {
            V16ServicePeerTrustStore(database).requireTrustedInCurrentTransaction(value.manifest.exporterId)
        ...
        SyncImportResult(V16ServiceSyncCodec.preview(value).counts, importedAt, value.business.zoneId)
    }

    private fun validateReferences(value: V16ServiceSyncPackage) {
    ...
    private fun clearPortableBusinessState() {
        val preserved = setOf("technician_identity", "trusted_v16_service_ids", "reminder_preferences", "recovery_metadata")
    ...
    private suspend fun insertBusiness(value: V16ServiceSyncPackage, importedAt: Long) {
    ...
    private suspend fun insertVisits(value: V16ServiceSyncPackage, importedAt: Long) {
    ...
    private suspend fun insertDispatchDrafts(drafts: List<SyncDispatchDraft>, value: V16ServiceSyncPackage, importedAt: Long) {
