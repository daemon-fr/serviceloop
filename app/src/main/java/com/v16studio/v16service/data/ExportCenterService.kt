...
data class ExportCenterSelection(
    val scope: V16ServiceScopeFilter = V16ServiceScopeFilter(),
...
class ExportCenterService(private val database: V16ServiceDatabase, private val filesRoot: File) {
    ...
    private suspend fun exportLocked(selection: ExportCenterSelection): Map<String, ByteArray> {
        ...
        val readme = buildString {
            appendLine("V16 Service Export Center")
    ...
    private fun ownedFile(path: String): File {
        ...
        require(file.path.startsWith(root.path + File.separator) && file.isFile) { "Image is outside V16 Service storage or missing" }
        return file
    }
