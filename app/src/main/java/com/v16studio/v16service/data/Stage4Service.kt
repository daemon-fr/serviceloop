...
class Stage4Service(
    ...
    suspend fun openCorrection(recordId: String): CorrectionDraft = database.withTransaction {
        ...
        dao.insertCorrectionWorkItems(sourceWork.map { item -> CorrectionWorkItemEntity(
            ...
            photosJson = encodePhotos(dao.finalPhotos(item.id).map { CorrectionPhotoDraft(it.sourceAttachmentId, it.storedRelativePath, it.sha256, it.byteSize, it.mimeType, it.caption, true, it.addedInCorrection, it.addedAtEpochMillis, it.includedInCustomerReport, it.visibility) }),
        ) })
    ...
    suspend fun moveReview(equipmentId: String): MoveReview {
        ...
        return MoveReview(equipmentId, "${equipment.reference} · ${equipment.name}", oldSite.id, "${customer.name} · ${oldSite.name}", dao.activeVisitSites().filter { it.id != oldSite.id }.map { VisitSiteOption(it.id, it.reference, it.name, it.customerName, emptyList(), CustomerType.fromCode(it.customerType)) }, plans, blockers)
    }
    ...
    suspend fun moveEquipment(equipmentId: String, destinationSiteId: String, effectiveDate: String, reason: String, acknowledged: Boolean): Boolean = database.withTransaction {
        ...
        if (dao.plansForEquipment(equipmentId).isNotEmpty()) require(CustomerType.fromCode(destinationCustomer.customerType) == CustomerType.STANDARD) { "Recurring service requires a Standard customer" }
    ...
    suspend fun setBackupReminder(days: Int) { require(days in setOf(0, 1, 7, 30, 90)); dao.upsertRecoveryMetadata(ensureMetadata().copy(backupReminderDays = days)) }
    ...
    suspend fun recordsCsvPackage(includeInactive: Boolean, includePrivate: Boolean, includePreviousRevisions: Boolean, customerId: String? = null): ByteArray {
        ...
        val csvEntries = linkedMapOf<String, ByteArray>()
        csvEntries["customers.csv"] = sheet(listOf("customer_id","reference","customer_type","name","contact_name","phone","email","state","private_note"), customers.map { listOf(it.id,it.reference,it.customerType,it.name,it.contactName.orEmpty(),it.phone.orEmpty(),it.email.orEmpty(),it.state,if(includePrivate) it.privateNote.orEmpty() else "") })
        csvEntries["sites.csv"] = sheet(listOf("site_id","customer_id","reference","name","address","state","is_default","private_access_note"), sites.map { listOf(it.id,it.customerId,it.reference,it.name,it.address.orEmpty(),it.state,it.isDefault.toString(),if(includePrivate) it.privateAccessNotes.orEmpty() else "") })
        csvEntries["equipment.csv"] = sheet(listOf("equipment_id","site_id","reference","name","make","model","serial","state","private_note"), equipment.map { listOf(it.id,it.siteId,it.reference,it.name,it.make.orEmpty(),it.model.orEmpty(),it.serialNumber.orEmpty(),it.state,if(includePrivate) it.privateNotes.orEmpty() else "") })
        csvEntries["service_plans.csv"] = sheet(listOf("plan_id","equipment_id","reference","name","interval_count","interval_unit","current_due","state","current_obligation_id","last_counted_revision_id"), dao.allPlans().filter { it.equipmentId in equipmentIds }.map { listOf(it.id,it.equipmentId,it.reference,it.name,it.intervalCount.toString(),it.intervalUnit,it.currentDueDate,it.state,it.currentObligationId.orEmpty(),it.lastCountedRevisionId.orEmpty()) })
        csvEntries["visits.csv"] = sheet(listOf("visit_id","reference","customer_id","site_id","service_date","state","cancellation_reason"), visits.map { listOf(it.id,it.reference,it.customerId,it.siteId,it.actualServiceDate,it.state,it.cancellationReason.orEmpty()) })
        val visitWork = visits.flatMap { visit -> dao.visitWorkItems(visit.id).map { visit to it } }
        csvEntries["work_items.csv"] = sheet(listOf("work_item_id","visit_id","subject_type","equipment_id","equipment_description","plan_id","service_name","outcome","fulfills_obligation","captured_obligation_id"), visitWork.map { (_,it) -> listOf(it.id,it.visitId,it.subjectType,it.equipmentId.orEmpty(),it.equipmentDescriptionSnapshot.orEmpty(),it.servicePlanId.orEmpty(),it.serviceNameSnapshot,it.outcome.orEmpty(),it.fulfillsCurrentObligation?.toString().orEmpty(),it.capturedObligationId.orEmpty()) })
        csvEntries["service_records.csv"] = sheet(listOf("record_id", "revision_id", "revision_number", "visit_reference", "service_date", "recorded_at", "customer", "site", "voided", "correction_reason", "public_note", "private_internal_note"), revisions.map { (record, revision) -> listOf(record.id, revision.id, revision.revisionNumber.toString(), revision.visitReference, revision.actualServiceDate, revision.recordedAtEpochMillis.toString(), revision.customerName, revision.siteName, record.voided.toString(), revision.correctionReason.orEmpty(), revision.publicNote.orEmpty(), if (includePrivate) revision.privateInternalNote.orEmpty() else "") })
        val finalWork = revisions.flatMap { (_, revision) -> dao.finalWorkItems(revision.id).map { revision to it } }
        csvEntries["final_work_items.csv"] = sheet(listOf("revision_id","final_work_item_id","subject_type","equipment_id","equipment_description","equipment_name","equipment_reference","service_name","outcome","fulfilled_obligation"), finalWork.map { (revision,it) -> listOf(revision.id,it.id,it.subjectType,it.equipmentId.orEmpty(),it.equipmentDescription.orEmpty(),it.equipmentName.orEmpty(),it.equipmentReference.orEmpty(),it.serviceName,it.outcome,it.fulfilledObligation.toString()) })
        csvEntries["checklist_answers.csv"] = sheet(listOf("revision_id","final_work_item_id","position","question","response_type","disposition","value","reason"), finalWork.flatMap { (revision,work) -> dao.finalChecklistItems(work.id).map { listOf(revision.id,work.id,it.position.toString(),it.label,it.responseType,it.disposition,it.textValue ?: it.numberValue.orEmpty(),it.reason.orEmpty()) } })
        csvEntries["findings.csv"] = sheet(listOf("revision_id","final_work_item_id","question","public_finding_description"), finalWork.flatMap { (revision,work) -> dao.finalChecklistItems(work.id).filter { it.disposition == "ISSUE_FOUND" }.map { listOf(revision.id,work.id,it.label,it.reason.orEmpty()) } })
        csvEntries["parts.csv"] = sheet(listOf("revision_id","final_work_item_id","description","quantity","unit"), finalWork.flatMap { (revision,work) -> dao.finalParts(work.id).map { listOf(revision.id,work.id,it.description,it.quantity,it.unit) } })
        csvEntries["followups.csv"] = sheet(listOf("followup_id","reference","type","title","due_date","state","customer_id","site_id","equipment_id","private_planning_note"), dao.allFollowUps().filter { it.customerId in customerIds }.map { listOf(it.id,it.reference,it.type,it.title,it.dueDate,it.state,it.customerId,it.siteId.orEmpty(),it.equipmentId.orEmpty(),if(includePrivate) it.privatePlanningNote.orEmpty() else "") })
        csvEntries["contact_notes.csv"] = sheet(listOf("contact_id","reference","customer_id","site_id","equipment_id","channel","occurred_at","outcome","entered_in_error","private_note"), dao.allContactNotes().filter { it.customerId in customerIds }.map { listOf(it.id,it.reference,it.customerId,it.siteId.orEmpty(),it.equipmentId.orEmpty(),it.channel,it.occurredAtEpochMillis.toString(),it.outcome,it.enteredInError.toString(),if(includePrivate) it.privateNote.orEmpty() else "") })
        csvEntries["changes.csv"] = sheet(listOf("change_id","subject_type","subject_id","change_type","event_date","recorded_at","reason","old_value","new_value"), dao.allChangeEntries().filter { it.customerId in customerIds || (customerId == null && it.customerId == null) }.map { listOf(it.id,it.subjectType,it.subjectId,it.changeType,it.eventDate,it.recordedAtEpochMillis.toString(),it.reason,it.oldValue.orEmpty(),it.newValue.orEmpty()) })
        val historyScope = customerId?.let { HistoryScope(HistoryScopeType.CUSTOMER, it) } ?: HistoryScope(HistoryScopeType.GLOBAL)
        csvEntries["history.csv"] = sheet(listOf("event_id", "category", "event_kind", "event_date", "recorded_at", "title", "detail", "customer_id", "site_id", "equipment_id"), history(HistoryQuery(scope = historyScope)).map { listOf(it.id, it.type.name, it.eventKind, it.eventDate, it.recordedAtEpochMillis.toString(), it.title, it.subtitle, it.customerId.orEmpty(), it.siteId.orEmpty(), it.equipmentId.orEmpty()) })
        val revisionIds = revisions.map { it.second.id }.toSet()
        csvEntries["report_index.csv"] = sheet(listOf("rendition_id", "revision_id", "version", "status", "kind", "generated_at", "relative_path", "sha256", "byte_size"), dao.allReportRenditions().filter { it.revisionId in revisionIds }.map { listOf(it.id, it.revisionId, it.versionNumber.toString(), it.status, it.kind, it.generatedAtEpochMillis?.toString().orEmpty(), it.relativePath, it.sha256.orEmpty(), it.byteSize?.toString().orEmpty()) })
        ...
        csvEntries["attachment_index.csv"] = sheet(listOf("attachment_id", "owner_type", "owner_id", "availability", "included_in_report", "relative_path", "sha256", "byte_size", "caption"), dao.allAttachments().filter { it.ownerId in workIds || it.id in historicalAttachmentIds }.map { listOf(it.id, it.ownerType, it.ownerId, it.availability, it.includedInCustomerReport.toString(), it.storedRelativePath, it.sha256, it.byteSize.toString(), if (includePrivate) it.caption.orEmpty() else "") })
        val archiveNames = listOf("README.txt") + csvEntries.keys
        val scope = customerId?.let { id -> customers.singleOrNull()?.reference?.let { "customer $it" } ?: "customer $id" } ?: "all data"
        val readme = buildString {
            appendLine("V16 Service records export")
            appendLine("Generated with V16 Service")
            appendLine("Generated: ${businessTime.instant()}")
            appendLine("Scope: $scope")
            appendLine("Include inactive: ${if (includeInactive) "yes" else "no"}")
            appendLine("Include private data: ${if (includePrivate) "yes" else "no"}")
            appendLine("Include previous revisions: ${if (includePreviousRevisions) "yes" else "no"}")
            appendLine()
            appendLine("Files:")
            archiveNames.forEach { appendLine("- $it") }
            appendLine()
            appendLine("This archive is unencrypted and is not a recovery backup; it contains CSV data and indexes, not PDF or photograph bytes.")
        }.toByteArray(Charsets.UTF_8)
        val entries = linkedMapOf<String, ByteArray>("README.txt" to readme).apply { putAll(csvEntries) }
    ...
    suspend fun validateDirectoryCsv(bytes: ByteArray): CsvImportPreview {
        ...
        val header = rows.first(); require(header.size == CSV_HEADERS.size && header.toSet() == CSV_HEADERS.toSet()) { "CSV must contain exactly the fixed V16 Service headers" }
        ...
        rows.drop(1).forEachIndexed { index, raw ->
            ...
            runCatching { CustomerType.fromCode(values.getValue("customer_type")) }.onFailure { messages += "customer_type must be STANDARD or ONE_TIME" }
            ...
            existingCustomers[values.getValue("customer_ref")]?.let { existing -> if (normalize(existing.name) != normalize(values.getValue("customer_name")) || existing.customerType != values.getValue("customer_type")) messages += "HARD CONFLICT: customer reference has different fields" else if (status != "ERROR") status = "EXISTING_UNCHANGED" }
    ...
    suspend fun importDirectory(preview: CsvImportPreview, createSeparate: Set<String> = emptySet(), skippedBranches: Set<String> = emptySet()): ImportResult = database.withTransaction {
        ...
        rows.groupBy { it.values.getValue("customer_ref") }.forEach { (ref, group) ->
            ...
            val customer = existing ?: CustomerEntity(id = UUID.randomUUID().toString(), reference = ref, name = first.getValue("customer_name"), contactName = clean(first["customer_contact_name"]), phone = clean(first["customer_phone"]), email = clean(first["customer_email"]), privateNote = clean(first["customer_notes_internal"]), state = "ACTIVE", customerType = CustomerType.fromCode(first.getValue("customer_type")).code).also { dao.insertCustomers(listOf(it)); customers[ref] = it; cc++ }
            if (existing != null) { require(normalize(existing.name) == normalize(first.getValue("customer_name")) && existing.customerType == first.getValue("customer_type")) { "Hard conflict for customer reference $ref" }; unchanged++ }
    ...
    private fun encodePhotos(values: List<CorrectionPhotoDraft>) = JSONArray().also { array -> values.forEach { v -> array.put(JSONObject().put("sourceId", v.sourceId).put("storedRelativePath", v.storedRelativePath).put("sha256", v.sha256).put("byteSize", v.byteSize).put("mimeType", v.mimeType).put("caption", v.caption).put("selected", v.selected).put("addedInCorrection", v.addedInCorrection).put("addedAtEpochMillis", v.addedAtEpochMillis).put("includedInCustomerReport", v.includedInCustomerReport).put("visibility", v.visibility)) } }.toString()
    private fun decodePhotos(value: String) = JSONArray(value).let { array -> List(array.length()) { i -> array.getJSONObject(i).let { v -> CorrectionPhotoDraft(v.optNullable("sourceId"), v.getString("storedRelativePath"), v.getString("sha256"), v.getLong("byteSize"), v.getString("mimeType"), v.optNullable("caption"), v.getBoolean("selected"), v.optBoolean("addedInCorrection"), if (v.isNull("addedAtEpochMillis")) null else v.getLong("addedAtEpochMillis"), v.optBoolean("includedInCustomerReport", true), v.optString("visibility", "PUBLIC")) } } }
    ...
    private fun referenceProjection(key: String, values: Map<String, String>): Map<String, String> = when (key) {
        "customer_ref" -> listOf("customer_ref", "customer_type", "customer_name", "customer_contact_name", "customer_phone", "customer_email", "customer_notes_internal")
        "site_ref" -> listOf("customer_ref", "customer_type", "site_ref", "site_name", "site_address", "site_use_customer_contact", "site_contact_name", "site_phone", "site_email", "site_access_notes_internal")
        else -> listOf("site_ref", "equipment_ref", "equipment_name", "equipment_type", "make", "model", "serial_number", "internal_identifier", "equipment_notes_internal")
