...
import com.v16studio.v16service.domain.CustomerWithFirstSiteInput
...
class DispatchPackageService(
    ...
    private val dao=database.v16ServiceDao();private val dispatch=database.dispatchDao()
    ...
    suspend fun importTechnicianManually(v:TechnicianIdentity,confirmRename:Boolean=false){val normalized=TechnicianIdCodec.normalize(v.technicianId)?:throw IllegalArgumentException("Technician ID is not a valid V16 Service Technician ID.");importTechnician(v.copy(technicianId=normalized),confirmRename)}
    ...
    suspend fun saveOutboxVisit(draft:DispatchOutboxEditorDraft):String=database.withTransaction{
        ...
        require(existing?.localVisitId==null) { "Linked Visits must be edited from the canonical Visit" }
    ...
    suspend fun rescheduleOutboxVisit(id:String,date:String,time:String?,zone:String){LocalDate.parse(date);time?.let(LocalTime::parse);ZoneId.of(zone);val value=dispatch.outboxVisit(id)?:error("Outbox visit missing");require(value.localVisitId==null){"Linked Visits must be rescheduled from the canonical Visit"};require(value.outboxStatus!=DispatchOutboxStatus.CONCLUDED){"Reopen this concluded Visit before editing"};dispatch.updateOutboxVisit(value.copy(serviceDate=date,appointmentLocalTime=time,appointmentZoneId=zone,modifiedAtEpochMillis=modifiedAfter(value)))}
    private suspend fun compose(o:DispatchOutboxVisitEntity,g:Int):DispatchVisit{
        val localVisit=o.localVisitId?.let { dao.visit(it) ?: error("Linked Visit missing") }
        val site=dao.site(localVisit?.siteId ?: o.siteId)?:error("Site missing")
        val customer=dao.customer(site.customerId)?:error("Customer missing")
        ...
        val items=dispatch.outboxItems(o.dispatchVisitId).map{i->
            val localWork=i.localWorkItemId?.let { dao.workItem(it) ?: error("Linked work item missing") }
            require(localWork==null || localWork.visitId==localVisit?.id) { "Linked work item belongs to another Visit" }
            val subject=WorkSubjectType.fromCode(localWork?.subjectType ?: i.subjectType)
            val equipment=if(localWork!=null) localWork.equipmentId?.let { dao.equipment(it) ?: error("Linked Equipment missing") }
                else validateOutboxSubject(site,customer,subject,i.equipmentId,i.equipmentDescription.orEmpty(),i.servicePlanReference,i.dueDateSnapshot)
            ...
            DispatchWork(i.dispatchItemId,subject,localWork?.equipmentReferenceSnapshot ?: equipment?.reference,
                (localWork?.equipmentDescriptionSnapshot ?: i.equipmentDescription)?.trim()?.takeIf{equipment==null&&!it.isNullOrBlank()},
                localWork?.serviceNameSnapshot ?: i.taskName,localWork?.planReferenceSnapshot ?: i.servicePlanReference,
                localWork?.dueDateSnapshot ?: i.dueDateSnapshot,assigned,snapshot?.snapshotId)
        }
        require(items.isNotEmpty())
        val zone=localVisit?.appointmentZoneId ?: o.appointmentZoneId
        val time=localVisit?.scheduledAtEpochMillis?.let { Instant.ofEpochMilli(it).atZone(ZoneId.of(zone)).toLocalTime().withSecond(0).withNano(0).toString() }
        return DispatchVisit(o.dispatchVisitId,g,o.managerReference,localVisit?.actualServiceDate ?: o.serviceDate,
            if(localVisit!=null) time else o.appointmentLocalTime,zone,site.reference,o.instructions,teams,participants,
            teams.flatMap{it.leaderIds}.distinct().sorted(),items,if(o.canceledAtEpochMillis!=null)"CANCELED" else "ACTIVE",o.cancellationReason)
    }
    ...
    private suspend fun snapshotForOutboxItem(item:DispatchOutboxItemEntity):DispatchInspectionSnapshot? {
        if(item.localWorkItemId!=null){
            val work=dao.workItem(item.localWorkItemId)?:error("Linked work item missing")
            val frozen=work.templateSnapshotId?.let { dao.templateSnapshot(it) } ?: return null
            val draft=DispatchInspectionSnapshot("",frozen.templateName,null,frozen.revision,
                dao.checklistItems(frozen.id).map { DispatchInspectionItem(it.position,it.label,it.responseType,it.unit,it.required,it.privateGuidance) })
            return draft.copy(snapshotId=DispatchPackageCodec.contentAddressedSnapshotId(draft))
        }
        return item.reusableTemplateId?.let { templateId -> snapshotForTemplateId(templateId) } ?: snapshotForPlanReference(item.servicePlanReference)
    }
    ...
    suspend fun createExportFile(prepared:DispatchExportPreparation,cacheRoot:File):DispatchExportArtifact{val visits=prepared.packageValue.visits;val first=visits.minOf{it.serviceDate};val last=visits.maxOf{it.serviceDate};val directory=File(cacheRoot,"work-packages").apply{check(isDirectory||mkdirs()){"Cannot create work-package cache"}};val suffix=prepared.packageValue.packageId.take(8);val final=File(directory,"v16service-dispatch-${first}_${last}-${visits.size}-visits-$suffix.v16service");val temp=File(directory,".${prepared.packageValue.packageId}.tmp");val exporterId=V16ServicePeerTrustStore(database).localIdentity().technicianId;val envelope=V16ServiceSyncEnvelopeCodec.wrapWorkAssignment(prepared.packageValue,exporterId);try{mutationFault.checkpoint("before_export_file_write");temp.outputStream().use{it.write(envelope);it.fd.sync()};require(temp.isFile&&temp.readBytes().contentEquals(envelope)){"Work-package file verification failed"};mutationFault.checkpoint("after_export_file_write_before_commit");check(temp.renameTo(final)){"Cannot finalize work-package file"};require(final.isFile&&final.readBytes().contentEquals(envelope)){"Work-package file verification failed"};commitPreparedExport(prepared);return DispatchExportArtifact(prepared.packageValue,final)}catch(failure:Throwable){temp.delete();final.delete();if(failure is CancellationException)throw failure;throw failure}}
    suspend fun concludeOutboxVisits(ids:List<String>)=database.withTransaction{
        val selected=ids.distinct();require(selected.isNotEmpty())
        val current=selected.map{dispatch.outboxVisit(it)?:error("Outbox visit missing")}
        require(current.all{it.outboxStatus==DispatchOutboxStatus.DISPATCHED}){"Every selected Visit must be Dispatched"}
        current.forEach { outbox ->
            outbox.localVisitId?.let { localId ->
                val localFinalIds=dao.finalRecordForVisit(localId)?.let { record -> dao.finalWorkItems(record.currentRevisionId).map { it.sourceWorkItemId }.toSet() }.orEmpty()
                val remoteIds=dao.appliedWorkResultReceipts(outbox.dispatchVisitId).map { it.dispatchItemId }.toSet()
                val items=dispatch.outboxItems(outbox.dispatchVisitId)
                require(items.isNotEmpty()&&items.all { it.dispatchItemId in remoteIds || it.localWorkItemId in localFinalIds }) { "Awaiting final results for assigned work" }
                val canonical=dao.visit(localId)?:error("Linked canonical Visit is missing")
                require(canonical.state!="CANCELED") { "Canceled Visit cannot be concluded" }
                if(canonical.state!="COMPLETED") { val now=System.currentTimeMillis();dao.updateVisit(canonical.copy(state="COMPLETED",modifiedAtEpochMillis=now));dao.releaseVisitClaims(localId) }
            }
            val changedAt=modifiedAfter(outbox);dispatch.updateOutboxVisit(outbox.copy(concludedAtEpochMillis=changedAt,modifiedAtEpochMillis=changedAt))
        }
    }
    ...
    suspend fun import(p:DispatchPreview, exporterId:String):DispatchImportResult {
        ...
        database.withTransaction {
            V16ServicePeerTrustStore(database).requireTrustedInCurrentTransaction(exporterId)
    ...
    suspend fun handoff(local:String,itemId:String,target:DispatchTechnicianSnapshot,discard:Boolean=false)=BusinessFileCoordinator.mutex.withLock{
        ...
        try{
            database.withTransaction{
                ...
                work.capturedObligationId?.let{dispatch.releaseItemClaim(local,it)};check(dispatch.unlinkDispatchWork(workId)==1);dispatch.updateItemBinding(item.copy(documentationDisposition="DEFERRED",deferredToTechnicianId=target.technicianId,deferredToName=target.name));val now=System.currentTimeMillis();appendEvent(visit,binding,"DISPATCH_DOCUMENTATION_DEFERRED","Local documentation handed off to ${target.name} (${target.technicianId}); V16 Service cannot confirm acceptance",item,now);mutationFault.checkpoint("before_db_commit")
            }
