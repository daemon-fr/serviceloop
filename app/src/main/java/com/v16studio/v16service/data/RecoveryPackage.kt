...
import com.v16studio.v16service.domain.BackupInspection
import com.v16studio.v16service.domain.BackupResult
...
class RecoveryPackage(
    ...
    fun inspect(protectedBytes: ByteArray, passphrase: CharArray): BackupInspection {
        ...
        require(version <= FORMAT_VERSION) { "This backup needs a newer V16 Service version" }
    ...
    private fun validateDatabase(root: JSONObject) {
        ...
        val trustedRows = tableRows(root, "trusted_v16_service_ids")
        ...
        }) { "Trusted V16 Service IDs are invalid" }
    ...
    private suspend fun eraseUnlocked(newDatasetId: String) {
        ...
        try {
            ...
            database.withTransaction {
                val db = database.openHelper.writableDatabase
                val preserved = setOf("recovery_metadata", "technician_identity", "trusted_v16_service_ids")
                TABLE_ORDER.asReversed().filterNot { it in preserved }.forEach { db.execSQL("DELETE FROM `$it`") }
                database.v16ServiceDao().upsertRecoveryMetadata(
    ...
    private fun unprotect(bytes: ByteArray, passphrase: CharArray): ByteArray {
        require(bytes.size > 52) { "Not a V16 Service backup" }
        val buffer = ByteBuffer.wrap(bytes).order(ByteOrder.BIG_ENDIAN); val magic = ByteArray(4).also(buffer::get)
        require(magic.contentEquals(MAGIC)) { "Not a V16 Service backup" }
        val version = buffer.int; require(version <= FORMAT_VERSION) { "This backup needs a newer V16 Service version" }; require(version == FORMAT_VERSION)
    ...
    companion object {
        ...
        val MAGIC = byteArrayOf('V'.code.toByte(), '1'.code.toByte(), '6'.code.toByte(), 'B'.code.toByte())
        val TABLE_ORDER = listOf(
            ...
            "technician_identity", "trusted_v16_service_ids", "dispatch_technicians", "dispatch_teams", "dispatch_team_members",
        ...
        fun recoverInterrupted(database: V16ServiceDatabase, fileRoot: File): RecoveryResult {
        ...
        private fun markRestricted(database: V16ServiceDatabase) = runCatching { database.openHelper.writableDatabase.execSQL("UPDATE recovery_metadata SET restrictedRecoveryState=1 WHERE id='primary'") }
