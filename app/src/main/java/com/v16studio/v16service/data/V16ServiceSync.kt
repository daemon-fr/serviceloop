...
const val V16_SERVICE_SYNC_MIME = "application/vnd.v16studio.v16service+zip"

data class V16ServiceSyncManifest(
    val syncId: String,
    val title: String,
    val purpose: String,
    val generatedAt: String,
    val generatedWith: String = "V16 Service",
    val sections: List<V16ServiceSyncSectionDeclaration> = emptyList(),
    val exporterId: String,
    val formatVersion: Int = 1,
)

data class V16ServiceSyncSectionDeclaration(
    val name: String,
    val version: Int,
    val path: String,
)

data class V16ServiceSyncEnvelope(
    val manifest: V16ServiceSyncManifest,
    val sections: Map<String, ByteArray>,
) {
    fun section(name: String): ByteArray = sections[name] ?: error("Missing V16 Service sync section: $name")
}
...
data class V16ServiceSyncPackage(
    val manifest: V16ServiceSyncManifest,
...
data class SyncImportPreview(val packageValue: V16ServiceSyncPackage, val counts: SyncImportCounts)
...
fun SyncContentFamily.count(value: V16ServiceSyncPackage): Int = when (this) {
...
fun normalizeSyncContentSelection(value: V16ServiceSyncPackage, requested: Set<SyncContentFamily>): Set<SyncContentFamily> {
...
fun removeSyncContentFamily(value: V16ServiceSyncPackage, selected: Set<SyncContentFamily>, removed: SyncContentFamily): Set<SyncContentFamily> {
...
fun filterFullWorkspacePackage(value: V16ServiceSyncPackage, requested: Set<SyncContentFamily>): V16ServiceSyncPackage {
...
object V16ServiceSyncEnvelopeCodec {
    const val FORMAT = "V16ServiceFile"
    ...
    fun encode(manifest: V16ServiceSyncManifest, sections: Map<String, ByteArray>): ByteArray {
        require(manifest.formatVersion == FORMAT_VERSION) { "New V16 Service files must use envelope v1" }
        ...
        require(manifest.purpose in setOf("FULL_WORKSPACE", "WORK_ASSIGNMENT", "WORK_RESULT", "DATA_TRANSFER"))
        Instant.parse(manifest.generatedAt)
        require(manifest.generatedWith == "V16 Service")
    ...
    fun decode(bytes: ByteArray): V16ServiceSyncEnvelope {
        ...
        return V16ServiceSyncEnvelope(manifest, manifest.sections.associate { it.name to entries.getValue(it.path) })
    }

    fun wrapWorkAssignment(value: DispatchPackage, exporterId: String): ByteArray = encode(
        V16ServiceSyncManifest(value.packageId, "V16 Service work assignment", "WORK_ASSIGNMENT", value.createdAt, sections = listOf(V16ServiceSyncSectionDeclaration("working", DispatchPackageCodec.CURRENT_VERSION, "working.json")), exporterId = exporterId),
    ...
    fun unwrapWorkAssignment(bytes: ByteArray): DispatchPackage {
        ...
        require(envelope.manifest.purpose == "WORK_ASSIGNMENT") { "This V16 Service file is not a work assignment" }
        require(envelope.manifest.sections == listOf(V16ServiceSyncSectionDeclaration("working", DispatchPackageCodec.CURRENT_VERSION, "working.json"))) { "Invalid work-assignment sections" }
        return DispatchPackageCodec.decode(envelope.section("working"))
    }

    fun wrapTemplateTransfer(value: InspectionTemplateTransfer, exporterId: String, sourceWorkspaceId: String = exporterId): ByteArray {
    ...
    fun unwrapTemplateTransfer(bytes: ByteArray): InspectionTemplateTransfer {
        val payload = DataTransferCodec.decode(bytes)
        require(payload.families.keys == setOf(DataTransferFamily.INSPECTION_TEMPLATES)) { "This transfer contains other data" }
        return InspectionTemplateCodec.decode(payload.families.getValue(DataTransferFamily.INSPECTION_TEMPLATES))
    }

    private fun manifestJson(manifest: V16ServiceSyncManifest) = JSONObject()
    ...
    private fun parseManifest(o: JSONObject): V16ServiceSyncManifest {
        require(o.requiredText("format") == FORMAT)
        val formatVersion = o.getInt("formatVersion")
        require(formatVersion == FORMAT_VERSION) { "Unsupported V16 Service sync version" }
        val purpose = o.requiredText("purpose")
        require(purpose in setOf("FULL_WORKSPACE", "WORK_ASSIGNMENT", "WORK_RESULT", "DATA_TRANSFER"))
        val generatedAt = o.requiredText("generatedAt"); Instant.parse(generatedAt)
        require(o.requiredText("generatedWith") == "V16 Service")
        ...
        val sections = (0 until sectionsObject.length()).map { index ->
            ...
            V16ServiceSyncSectionDeclaration(section.requiredText("name"), section.getInt("version"), section.requiredText("path"))
        }
        ...
        val exporterId = TechnicianIdCodec.normalize(o.requiredText("exporterId", 100))
            ?: throw IllegalArgumentException("Invalid exporter ID")
        return V16ServiceSyncManifest(o.requiredText("syncId"), o.requiredText("title"), purpose, generatedAt, "V16 Service", sections, exporterId, formatVersion)
    }
...
object V16ServiceSyncCodec {
    ...
    private val sectionVersions = sectionNames.associateWith { 1 }
    private val expectedEntries = (listOf("manifest.json") + sectionNames.map { "$it.json" }).toSet()

    fun preview(value: V16ServiceSyncPackage): SyncImportPreview = SyncImportPreview(value, SyncImportCounts(
        businessProfiles = 1,
    ...
    fun encode(value: V16ServiceSyncPackage): ByteArray {
        validatePackage(value)
        val sectionDeclarations = sectionNames.map { V16ServiceSyncSectionDeclaration(it, sectionVersions.getValue(it), "$it.json") }
    ...
    fun decode(bytes: ByteArray): V16ServiceSyncPackage {
        val envelope = V16ServiceSyncEnvelopeCodec.decode(bytes)
        require(envelope.manifest.purpose == PURPOSE_FULL_WORKSPACE) { "Unsupported V16 Service sync purpose" }
        val expectedVersions = sectionVersions
        require(envelope.manifest.sections == sectionNames.map { V16ServiceSyncSectionDeclaration(it, expectedVersions.getValue(it), "$it.json") }) { "Sync entries do not match the supported contract" }
        val sections = envelope.sections
        return V16ServiceSyncPackage(
    ...
    private fun parseManifest(o: JSONObject): V16ServiceSyncManifest {
        require(o.requiredText("format") == FORMAT && o.requiredText("purpose") == PURPOSE_FULL_WORKSPACE) { "Unsupported V16 Service sync" }
        val generatedAt = o.requiredText("generatedAt"); Instant.parse(generatedAt)
        require(o.requiredText("generatedWith") == "V16 Service") { "Unsupported sync generator" }
        val formatVersion = o.getInt("formatVersion")
        require(formatVersion == FORMAT_VERSION) { "Unsupported V16 Service sync version" }
        val exporterId = TechnicianIdCodec.normalize(o.requiredText("exporterId", 100)) ?: error("Invalid exporter ID")
        return V16ServiceSyncManifest(o.requiredText("syncId"), o.requiredText("title"), PURPOSE_FULL_WORKSPACE, generatedAt, exporterId = exporterId, formatVersion = formatVersion)
    }
    ...
    private fun parseRegister(o: JSONObject): SyncRegister {
        ...
        val contacts=o.getJSONArray("customerContacts").let{a->List(a.length()){index->val c=a.getJSONObject(index);SyncCustomerContact(c.requiredText("id"),c.requiredText("customerId"),c.nullableText("personName"),c.requiredText("channel"),c.requiredText("value"),c.nullableText("notes"),c.getInt("position"))}}
    ...
    private fun parseTeam(o: JSONObject): SyncTeamDirectory {
        val technicians = o.getJSONArray("technicians").let { array ->
            List(array.length()) { index ->
                val value = array.getJSONObject(index)
                require(value.has("notes")) { "Technician notes field is missing" }
                SyncTechnician(value.requiredText("technicianId"), value.requiredText("displayName"),
                    value.nullableText("designation"), value.nullableText("notes"))
            }
        }
        val teams = o.getJSONArray("teams").let { array ->
            List(array.length()) { index ->
                val value = array.getJSONObject(index)
                SyncTeam(value.requiredText("id"), value.requiredText("name"),
                    stringList(value, "memberIds"), stringList(value, "leaderIds"))
            }
        }
        return SyncTeamDirectory(technicians, teams)
    }
    ...
    private fun validatePackage(value: V16ServiceSyncPackage) {
        ...
        require(value.manifest.formatVersion == FORMAT_VERSION &&
            TechnicianIdCodec.normalize(value.manifest.exporterId.orEmpty()) == value.manifest.exporterId) { "Invalid exporter ID" }
