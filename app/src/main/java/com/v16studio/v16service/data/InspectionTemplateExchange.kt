...
object InspectionTemplateCodec {
    const val FORMAT = "V16ServiceInspectionTemplates"
    ...
    fun encode(value: InspectionTemplateTransfer): ByteArray {
        ...
        val root = JSONObject().put("format", FORMAT).put("formatVersion", CURRENT_VERSION).put("generatedAt", value.generatedAt).put("generatedWith", "V16 Service")
    ...
    fun decode(bytes: ByteArray): InspectionTemplateTransfer {
        ...
        require(root.getString("generatedWith") == "V16 Service") { "Unsupported inspection template generator" }
...
class InspectionTemplateExchangeService(private val database: V16ServiceDatabase) {
    ...
    suspend fun export(templateIds: Set<String>): InspectionTemplateTransfer {
        ...
        val localWorkspaceId = V16ServicePeerTrustStore(database).localIdentity().technicianId
