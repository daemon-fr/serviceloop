...
class WorkResultExchangeService(private val database: V16ServiceDatabase, private val attachmentRoot: File) {
    ...
    private suspend fun exportLocked(revisionIds: List<String>,
        ...
        require(revisionIds.isNotEmpty() && revisionIds.size <= WorkResultPackageCodec.MAX_RESULTS)
        val exporterId = V16ServicePeerTrustStore(database).localIdentity().technicianId
        val results = revisionIds.distinct().flatMap { revisionId ->
            ...
            require(provenance.documentingTechnicianId == exporterId) { "Only this V16 Service identity's finalized work can be returned" }
            ...
            dao.finalWorkItems(revisionId).mapNotNull { item ->
                ...
                val value = JSONObject()
                    ...
                    .put("followUpCaptureState", "CAPTURED_AT_REVISION")
                    .put("followUps", FinalFollowUpSnapshot.records(item.followUpsSnapshotJson))
        ...
        V16ServicePeerTrustStore(database).requireTrusted(issuers.single())
