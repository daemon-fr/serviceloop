...
class AggregateReportService(private val database: V16ServiceDatabase, private val repository: V16ServiceRepository, private val filesRoot: File,
    ...
    suspend fun reportable(scope: V16ServiceScopeFilter): List<ReportableVisitSource> {
    ...
    suspend fun generate(scope: V16ServiceScopeFilter, selectedKeys: List<String>): AggregateReportResult =
    ...
    private suspend fun generateLocked(scope: V16ServiceScopeFilter, selectedKeys: List<String>): AggregateReportResult {
    ...
    private fun ownedFile(path: String): File {
        ...
        require(file.path.startsWith(root.path + File.separator) && file.isFile) { "Photo is missing or outside V16 Service storage" }
        return file
    }
}
