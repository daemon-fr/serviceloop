...
interface DispatchDao {
    ...
    @Query("SELECT * FROM trusted_v16_service_ids ORDER BY name COLLATE NOCASE, peerId") suspend fun trustedV16ServiceIds(): List<TrustedV16ServiceIdEntity>
    @Query("SELECT * FROM trusted_v16_service_ids WHERE peerId=:peerId") suspend fun trustedV16ServiceId(peerId: String): TrustedV16ServiceIdEntity?
    @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insertTrustedV16ServiceId(value: TrustedV16ServiceIdEntity)
    @Update suspend fun updateTrustedV16ServiceId(value: TrustedV16ServiceIdEntity)
    @Query("DELETE FROM trusted_v16_service_ids WHERE peerId=:peerId") suspend fun deleteTrustedV16ServiceId(peerId: String): Int
    ...
    @Query("UPDATE work_items SET equipmentId=:equipmentId, subjectType=:subjectType, equipmentDescriptionSnapshot=:equipmentDescription, templateSnapshotId=:templateSnapshotId, equipmentNameSnapshot=:equipmentName, equipmentReferenceSnapshot=:equipmentReference, equipmentIdentifierSnapshot=:identifier, equipmentMakeSnapshot=:make, equipmentModelSnapshot=:model, equipmentSerialSnapshot=:serial, serviceNameSnapshot=:task, servicePlanId=NULL, capturedObligationId=NULL, planReferenceSnapshot=:planReference, dueDateSnapshot=:dueDate, intervalCountSnapshot=NULL, intervalUnitSnapshot=NULL, fulfillsCurrentObligation=0, confirmedNextDueDate=NULL, nextDueDateCalculated=NULL, nextDueOverrideReason=NULL WHERE id=:id") suspend fun updateBookedDispatchWork(id: String, subjectType: String, equipmentId: String?, equipmentDescription: String?, templateSnapshotId: String?, equipmentName: String?, equipmentReference: String?, identifier: String?, make: String?, model: String?, serial: String?, task: String, planReference: String?, dueDate: String?): Int
