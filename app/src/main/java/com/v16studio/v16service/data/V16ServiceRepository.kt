...
interface V16ServiceRepository {
    ...
    suspend fun importV16ServiceSync(value: V16ServiceSyncPackage): SyncImportResult = error("V16 Service sync import unavailable")
...
class RoomV16ServiceRepository(
    ...
    override suspend fun importV16ServiceSync(value: V16ServiceSyncPackage): SyncImportResult {
        val result = V16ServiceSyncImporter(database, businessTime, writeGate).apply(value)
    ...
    override suspend fun finalizeVisit(visitId: String): FinalizeResult = BusinessFileCoordinator.photoReportMetadataMutex.withLock {
        ...
        for (item in items) {
            ...
            for (photo in dao.workItemAttachments(item.id).filter { it.availability == "PRESENT" }) {
                val file = attachmentRoot?.let { File(it, photo.storedRelativePath) }
                if (file?.isFile != true || file.length() != photo.byteSize || sha256(file) != photo.sha256) return@withTransaction FinalizeResult.Blocked("A Service evidence photograph is missing or changed")
            }
            val checklistItems = item.templateSnapshotId?.let { dao.checklistItems(it) }.orEmpty()
        ...
        prepared.forEachIndexed { index, p ->
            ...
            dao.insertFinalPhotos(dao.workItemAttachments(p.item.id).filter { it.availability == "PRESENT" }.mapIndexed { photoIndex, photo -> FinalPhotoEntryEntity(stableId("final-photo", finalItemId, photo.id), finalItemId, photoIndex + 1, photo.id, photo.storedRelativePath, photo.sha256, photo.byteSize, photo.mimeType, photo.caption, includedInCustomerReport = photo.includedInCustomerReport, visibility = photo.visibility) })
