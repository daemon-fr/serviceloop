...
import com.v16studio.v16service.data.ReportRenditionEntity
import com.v16studio.v16service.data.V16ServiceDatabase
import com.v16studio.v16service.data.V16ServiceRepository
import com.v16studio.v16service.data.BusinessFileCoordinator
import com.v16studio.v16service.brand.V16ServiceBrandSpec
...
object FixedServiceRecordPdf {
    ...
    fun render(model: PublicReportModel, renditionId: String, versionNumber: Int = 1, generatedAtEpochMillis: Long, file: File, attachmentRoot: File? = null): Int {
        ...
        try {
            pages.forEachIndexed { pageIndex, lines ->
                ...
                val footer = "${V16ServiceBrandSpec.GENERATED_WITH} · ${model.visitReference} · R${model.revisionNumber} · PDF v$versionNumber · ${renditionId.take(8)} · ${Instant.ofEpochMilli(generatedAtEpochMillis).toString().take(10)} · Page ${pageIndex + 1} of $totalPages"
            ...
            photos.forEachIndexed { photoPageIndex, (line, photoIndex, photo) ->
                ...
                page.canvas.drawLine(LEFT, HEIGHT - BOTTOM - 9f, WIDTH - RIGHT, HEIGHT - BOTTOM - 9f, Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.LTGRAY; strokeWidth = 1f })
                val footer="${V16ServiceBrandSpec.GENERATED_WITH} · ${model.visitReference} · R${model.revisionNumber} · Page $pageNumber of $totalPages"; page.canvas.drawText(footer,LEFT,HEIGHT-BOTTOM+LineStyle.FOOTER.textSize,paint(LineStyle.FOOTER).apply{color=Color.DKGRAY})
    ...
    private fun buildLines(model: PublicReportModel): List<ReportDrawLine> {
        ...
        model.lines.forEach { line ->
            ...
            if (line.subjectType == com.v16studio.v16service.domain.WorkSubjectType.EQUIPMENT && line.equipmentReference != null) {
    ...
    private fun subjectLabel(line: com.v16studio.v16service.domain.PublicWorkLine): String = when {
        line.subjectType == com.v16studio.v16service.domain.WorkSubjectType.SITE -> "${line.position.toString().padStart(2, '0')} · Site task"
    ...
    private fun drawBrandMark(canvas: android.graphics.Canvas, top: Float) {
        val v16Paint = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.BLACK; textSize = 11f; typeface = Typeface.create("sans", Typeface.BOLD) }
        val servicePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.rgb(8, 102, 107); textSize = 11f; typeface = Typeface.create("sans", Typeface.BOLD) }
        val v16Width = v16Paint.measureText("V16 ")
        val serviceWidth = servicePaint.measureText("Service")
        val wordmarkWidth = v16Width + serviceWidth
        val start = (WIDTH - wordmarkWidth) / 2f
        val lineY = top + 7f
        val lineGap = V16ServiceBrandSpec.WORDMARK_LINE_GAP_PX
        val leftLineEnd = start - lineGap
        val rightLineStart = start + wordmarkWidth + lineGap
        canvas.drawLine(LEFT, lineY, leftLineEnd, lineY, Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.BLACK; strokeWidth = V16ServiceBrandSpec.LINE_THICKNESS_PX })
        canvas.drawLine(rightLineStart, lineY, WIDTH - RIGHT, lineY, Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.rgb(8, 102, 107); strokeWidth = V16ServiceBrandSpec.LINE_THICKNESS_PX })
        canvas.drawText("V16 ", start, top + 11f, v16Paint)
        canvas.drawText("Service", start + v16Width, top + 11f, servicePaint)
    }
