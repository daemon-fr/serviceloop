package com.v16studio.v16service.calendar
...
class CalendarDeviceStore(context: Context) {
    ...
    fun read(datasetId: String, adoptionToken: String = ""): CalendarDeviceState = synchronized(FILE_LOCK) {
        cached?.takeIf{it.datasetId==datasetId && it.adoptionToken==adoptionToken}?.let{return it}
...
class CalendarCoordinator(private val context:Context, private val database:V16ServiceDatabase, private val gateway:CalendarGateway, private val store:CalendarDeviceStore, private val scope:CoroutineScope) {
    ...
    suspend fun reconcile()=withContext(Dispatchers.IO){mutex.withLock{val(s,visits)=current();if(s.storeProblem||!s.enabled||!gateway.hasPermissions())return@withLock;var next=s;for(l in s.links.values){val v=visits[l.visitId];if(v==null||v.state=="CANCELED"){val deleted=runCatching{gateway.delete(l.calendarId,l.eventId)}.getOrDefault(false);next=if(deleted)next.copy(links=next.links-l.visitId)else next.copy(links=next.links+(l.visitId to l.copy(state=CalendarLinkState.DELETE_PENDING)));continue};if(v.state!="BOOKED")continue;val exists=runCatching{gateway.eventExists(l.calendarId,l.eventId)}.getOrDefault(false);if(!exists){next=next.copy(links=next.links+(l.visitId to l.copy(state=CalendarLinkState.MISSING)));continue};val event=event(v);val fp=fingerprint(event);if(l.state==CalendarLinkState.SYNCED&&l.fingerprint!=fp&&runCatching{gateway.update(l.calendarId,l.eventId,event)}.getOrDefault(false))next=next.copy(links=next.links+(l.visitId to l.copy(fingerprint=fp,lastConfirmedAt=System.currentTimeMillis())))};store.write(next);val writable=runCatching{gateway.writableCalendars().any{it.id==next.selectedCalendarId}}.getOrDefault(false);if(writable)visits.values.filter{it.state=="BOOKED"&&it.scheduledAtEpochMillis!=null&&it.id !in next.links&&it.id !in next.suppressedVisitIds}.forEach{v->next=create(next,v)};store.write(next)}}
    ...
    private suspend fun current():Pair<CalendarDeviceState,Map<String,WorkingVisitEntity>>{val dao=database.v16ServiceDao();val metadata=dao.recoveryMetadata();val dataset=metadata?.datasetId?:"uninitialized";val customers=dao.allCustomers().associateBy{it.id};val sites=dao.allSites().associateBy{it.id};val visits=dao.allVisits().associateBy({it.id}){v->val customer=customers[v.customerId];val site=sites[v.siteId];v.copy(customerNameSnapshot=customer?.name?:v.customerNameSnapshot,siteNameSnapshot=site?.name?:v.siteNameSnapshot,siteAddressSnapshot=if(site!=null)site.address else v.siteAddressSnapshot)};return store.read(dataset,metadata?.adoptionToken.orEmpty()) to visits}
