...
The source was supplied directly by the V16Service owner for local vendoring. It is licensed under the MIT license preserved in `LICENSE`. Product UI uses only the `fill` family. `v16service-icons.json` is the checked semantic selection manifest, and `phosphor_tool.py generate` deterministically creates the small Android resource subset used by the application.
