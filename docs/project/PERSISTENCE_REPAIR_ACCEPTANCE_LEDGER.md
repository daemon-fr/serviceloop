...
`IMPLEMENTED` describes code present; `TESTED` names an executed check; `DOMAIN-INSTRUMENTED` means production services in an isolated Android database; `UI-INSTRUMENTED`, `SYSTEM-HANDOFF`, and `HUMAN/RENDERED` are distinct claims. An unmentioned route is **NOT RUN**. After the shared Recovery/result/export changes, `:app:testDebugUnitTest :app:assembleDebug :app:lintDebug` passed: 564 JVM tests, zero failures/errors/skips, APK built and lint completed. A subsequent backup fault-injection hook has a focused passing test and needs the final impact gate.
...
| R06 / F03,D07 part | Canonical final-photo derivative stored once, verified and reused by result/native export; original remains separate. | Actual Android 12-photo result package/export/import/replay PASS; `B049WorkResultExportTest` restart, cleanup, relay and complete Recovery restore with identical derivative bytes PASS; EXIF rotate/flip test PASS. | Direct and successive native relay after Recovery with equal bytes under every privacy view. |
...
| R08 / F07,D06 | v2 semantic/outcome gates, typed checklist/findings/parts/follow-ups, captured recurrence agreement and calculated/override decision; author/issuer/trust/assignment checks and once-only recurrence. | `SourceCanonicalJsonTest`, `B049WorkResultImportTest` PASS; exporter-produced v2 Android result and corrected complete-visit fixture PASS (2 AVD tests). | Complete outcome/eligibility matrix with concurrent authors and captured obligation edits. |
