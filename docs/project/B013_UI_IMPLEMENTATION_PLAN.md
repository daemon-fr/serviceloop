...
- The brand strip uses a very light teal (`#F0F8F7`) light band and restrained dark teal (`#152629`) dark band. Its centered 18sp bold wordmark renders Service in neutral foreground and Loop in semantic teal between two equal-width, independently authored deterministic square trails. The Canvas geometry is fixed, code-rendered, square-only, asymmetric, and contains no runtime randomness; these are squares, explicitly not pixels.
