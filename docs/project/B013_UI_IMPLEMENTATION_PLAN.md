...
### Owner follow-up — Visit progress hierarchy and framed selection language (2026-09-14)

Visit progress now exposes the existing derived hierarchy **Equipment/Site → Services**. The top summary explicitly counts `service`/`services`, is informational, and has no click semantics. The global Show services / Hide services control is retired; every Equipment or Site-work group independently expands and collapses, with Visit overview groups expanded by default and the Service workspace expanding the current Service's group by default. Every navigable Service row opens its exact WorkItem. The current Service remains selected and actionable: its row performs an in-screen focus request rather than pushing a duplicate destination. No trailing divider follows the last Service in a group.

### Owner correction — actionable progress rows and compact inspection status (2026-09-15)

- Visit overview and Service-workspace progress rows keep exact WorkItem identity. Cross-Service taps retain the existing flush/save safeguard; a current-row tap never flushes or navigates to a duplicate route and instead issues a one-shot, WorkItem-keyed in-screen focus request. Attention focus follows visible Service order—Work performed, private note, checklist findings/reasons, Evidence captions, then completion-area raw fields—while retaining completion blockers as the fallback.
- Group controls use a leading Phosphor caret and `Show services (N)` / `Hide services (N)` wording. Only the attention suffix is italic, and it omits the repeated noun (`1 needs attention`, `2 need attention`). Group toggles and Service rows have separate hit areas.
- Service progress rows distinguish modest external group indentation from their rounded clickable surface and internal horizontal/vertical content padding. Selected and unselected rows share the same geometry, including the title, status badge, and disclosure affordance.
- STATUS answers use a dedicated semantic 2×2 control ordered `Not applicable | OK` then `Not checked | Issue found`. Circle-family Phosphor icons, subdued unselected semantic color, restrained selected tint/frame, radio semantics, and deterministic dynamic pair sizing keep labels on one line whenever the pair can fit; extreme accessibility widths stack the pair rather than wrap or clip labels.
- Technician-only visibility labels use a small decorative Phosphor eye-slash before the existing private label/content. This is presentation-only; private/public report boundaries and stored meaning are unchanged.

The inspection control's two pairs fall back independently: an unfit first pair does not force the second pair to stack, and vice versa. Automatic attention selection follows the visible Service order (Work performed, private note, checklist questions in `draft.questions` order using the active disposition field, Evidence captions, completion-area raw fields, then unknown supported fields); Failed/Invalid states retain precedence over ordinary unresolved buffers, while inactive preserved Issue/N/A drafts are not targeted.

Ordinary inline value/state selections use the shared framed-selection visual family and must not impersonate command buttons. The shared `ServiceLoopSelectionOption` owns the visible frame, theme-aware selected fill and selected text token, bold selected text, 48dp minimum target, and single-choice `selectable` semantics; `ServiceLoopChecklistChoice` remains a thin semantic wrapper. `ServiceLoopChoiceGroup` is now a vertical framed group for readable portrait and large-text behavior.

The app-wide production audit covered 24 `ServiceLoopChoiceGroup` call sites and 8 direct `FilterChip`/custom single-choice families across Daily Operations, Visit, Service, Dispatch, Calendar, Settings, History/correction, recovery, and export surfaces. Category-A value/state families migrated include Add-task subject/equipment/checklist, plan/template choices, Visit mode and appointment lead, reminder horizon and lead, appearance, team role, Dispatch subject/equipment/recipient/mode/duplicate decisions, service outcome/fulfillment, checklist status, correction destinations, backup reminder, export scope, follow-up/contact choices, and Calendar preferred calendar. True filters remain `ServiceLoopFilterSelector`; notebook tabs remain `ServiceLoopContentTabs`; weekday toggles, ordinary and Due-service checkboxes, and command/navigation/destructive controls retain their specialized semantics and styling. No Room schema, domain progress derivation, lifecycle, recurrence, checklist, Dispatch, Calendar, recovery, report, or localization behavior changed.
...
### Owner inspection correction R3.1 — selection geometry, action-stack audit, and local runtime proof

- Selectable Due-service `ServiceLoopEntityRecord` content now gives the title an explicit 48dp start inset derived from the independent selection touch target. The 20dp generated Phosphor square/check-square remains top-left inside that target, so short titles stay one line, genuinely wrapped titles grow to at most two lines, and the glyph cannot collide with title text. The selection target retains checkbox semantics and independent card navigation.
- The production vertical-command audit is complete. `ServiceLoopActionStack` now separates Reminder notification/test and save/cancel commands, Technician Identity save/copy/share, CSV import result actions, visit-detail photo actions, correction rescue/finalize actions, Data Recovery actions, CSV export actions, and report PDF share/send actions. Existing horizontal peer rows, inline links/chips, toolbar actions, and controls separated by meaningful content remain unchanged.
- The obsolete `DispatchChoiceDialog` filter helper had zero production call sites and was removed.
- Focused Compose instrumentation covers selection toggle/navigation independence, title bounds for short and long titles, and positive action-stack geometry. On the canonical local `Pixel_10a_ServiceLoop` AVD, dynamically resolved as `emulator-5556`, the focused class passed 3/3 tests; selector/shared-tab/Due-services regression classes passed 8/8; and the existing Work/Due-services, Reminder settings, and Coordinator navigation smoke passed 2/2. The debug and instrumentation APKs were built locally and installed explicitly with targeted adb. No owner visual acceptance is implied.

### Owner inspection correction R4 — visual consolidation

- Due-service selection uses the 24dp generated Phosphor check-square glyph for both states. The unchecked state is the pale-teal record border at 55% alpha; the checked state is opaque ServiceLoop teal. Its 48dp checkbox target is a card-edge top-left overlay independent of ordinary 16dp content padding, with title-only clearance and full-width context below the selection zone.
- A nonblank Equipment private note is inline with identity information before Edit / Start-resume. It uses a compact heading, subdued `PRIVATE · Not included in customer report` metadata, and normal body text; the separate bottom card is retired.
- Legacy surfaces now follow their meaning: Home dashboard gateways, Home attention rows, grouped dense Settings navigation, one Work-level More menu, compact provenance/version rows, and conditionally navigable Visit work-item rows. `SummaryRow` and `DailyRow` are retired.
- The brand strip uses a very light teal (`#F0F8F7`) light band and restrained dark teal (`#152629`) dark band. Its centered 18sp bold wordmark renders Service in neutral foreground and Loop in semantic teal between two balanced thin horizontal line systems: neutral on the left and teal on the right. The treatment is Compose-drawn, restrained, and theme-aware.
- Reminder summary days use an adaptive width contract: seven equal controls are shown only when `7 × touchMin + 6 × dayGap` fits (currently `7 × 48dp + 6 × 4dp = 360dp`) and the font scale remains comfortable; otherwise the days wrap as balanced 4+3 rows. Every day control keeps at least the 48dp touch width and height.
- Team-role settings now use combined one-line role labels, place `Send report copies to (optional)` after the role explanation, and embed Technician identity only for Member. The identity content is shared with the former standalone surface; Solo and Coordinator do not render it.
- Appointment lead choices are the compact `1h`, `3h`, `6h`, `12h`, `24h`, and `48h` presets, with a fresh default of 3 hours. Existing 2-hour values remain valid and are called out as legacy until replaced. The reminder save banner and Save action have a 16dp separation.
...
- Checklist completeness is a readiness prerequisite for every outcome. The checklist header exposes only `Required complete X of Y`; incomplete state uses the concrete `Complete all required checklist questions` blocker. A response edit never erases an automatic Performed fulfillment or an explicit Partly decision; finalization remains the exactly-once boundary for consuming/advancing an obligation.
- The technician-facing Review introduction is exactly `Review the service details before finalizing.`. Domain explanations about checklist facts and separate outcome/fulfillment decisions are retained in source/tests/docs only and are intentionally absent from the UI.
- Visit detail separates the identity, relationship actions, date landmark, dispatch, calendar, and progress regions. The date landmark is labelled `APPOINTMENT` for Booked visits and `SERVICE DATE` otherwise, with a compact `d MMM uuuu` display and persisted-date fallback.
- Home saved-state icons are compact in the dashboard only. Long-text expand controls retain a 48dp target with a modest local glyph and local bottom clearance rather than reserving a full-width suffix on every line.
