...
5. Aggregate report selection is per completed Visit. One remote multi-Service Visit freezes every exact source revision in ordered `aggregate_report_sources` and renders one Visit model with item-level documenting-technician attribution. Prior aggregate sources and PDFs remain unchanged after correction.
