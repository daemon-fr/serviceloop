...
3. `visits.csv` in Work Performed now contains only completed Visits with non-voided local or applied remote final truth; Booked, Working, and unperformed Canceled Visits stay out. Image Archive keeps separate scoping and includes frozen evidence.
4. Export Center and aggregate reporting share deterministic effective imported-result selection by stable `resultId` and source chronology. The previous-revisions switch controls whether earlier applied revisions appear in readable export.
5. Aggregate report selection is per completed Visit after one Customer is chosen. One remote multi-Service Visit freezes every exact source revision in ordered `aggregate_report_sources` and renders one Visit model with item-level documenting-technician attribution. Prior aggregate sources and PDFs remain unchanged after correction.
6. The redundant ordinary Verify row and `data-transfer/verify` route were removed. Import shared data still performs file validation and preview.
7. Direct `work-results/export` navigation now requires the existing receiver capability (Subcontractor, Employee, Team Leader). The B049 route audit also gates generic import/data transfer for Solo, matching the adopted Team matrix.

Final B049 schema remains Room v19: migration 18→19 adds `attachments.visibility` (legacy `PUBLIC`) and `final_photo_entries.includedInCustomerReport`/`visibility` (legacy `true`/`PUBLIC`). Generated `19.json` matches. Recovery remains v18; v17 rows receive the same defaults before validation, while current v18 flags survive round trip. `.slsync` envelope remains v2; register remains v3; WORK_RESULT remains v1; app remains `1.3.0`/code `5`.

Correction verification: 497 JVM tests PASS (0 failures/errors/skips); debug APK, Android-test APK, lintDebug, and release APK PASS; Room 18→19 migration, aggregate PDF, Export Center UI/role route tests PASS on the canonical AVD (5 focused tests), plus the two-item WORK_RESULT canonical-completion/recurrence/aggregate journey PASS as one additional DOMAIN-INSTRUMENTED test. The aggregate Visit-selection screen also passed a light/dark UI render test; both captures were retrieved and visually inspected. The final debug APK was installed with `adb -s <resolved serial> install -r`; eight-second delayed startup remained alive and sampled AndroidRuntime fatal output was empty. The preserved canonical database was copied in place with matching SHA-256 for the database and WAL, then its intermediate development-only v19 layout received the three additive columns and generated Room identity through a temporary debug provider. That provider was removed, the final APK rebuilt and installed, and the app opened without clearing data. No production 19→19 migration was introduced.

Correction-pass SYSTEM-HANDOFF was NOT RUN; earlier B049 picker evidence remains historical. HUMAN/RENDERED evidence in this pass covers the aggregate light/dark captures only. B-008 pilot and owner acceptance remain outstanding. The final correction commit and remote parity are recorded by Git and in the handoff.
...
## B049 owner amendment — native selective sharing (2026-09-23)

This later owner decision supersedes Stage G's template-only DATA_TRANSFER scope and readable-only Export Center wording. `.slsync` is the native ServiceLoop-to-ServiceLoop file for four distinct purposes: FULL_WORKSPACE replaces the portable workspace; DATA_TRANSFER merges selected ordinary data; WORK_ASSIGNMENT delegates work; WORK_RESULT returns immutable delegated results. A partial export is DATA_TRANSFER, never a filtered FULL_WORKSPACE.

Export Center retains one preset, family, Customer/Site/Equipment, inclusive date, privacy, inactive, and revision selection model. Its default output is **ServiceLoop file (.slsync)** for import into another workspace; **Readable archive (.zip)** remains available for people and other software. The create action names the selected format. New native exports use DATA_TRANSFER metadata v2 in the existing envelope v2. Legacy TEMPLATE_SHARE and DATA_TRANSFER v1 template files remain readable; new template files use DATA_TRANSFER v2.

DATA_TRANSFER v2 declares family identities, versions, sections, and validated binary entries. Its families are REGISTER, SERVICE_PLANS, INSPECTION_TEMPLATES, PERFORMED_WORK, FOLLOW_UPS, CONTACT_NOTES, CHANGE_HISTORY, and EVIDENCE. Export automatically includes required directory/template or immutable context so each selected slice is intelligible. Import validates the whole package and trust before a single atomic merge. Absence never deletes local data. Current directory and plans use source-workspace bindings and conservative conflict classification; source changes or local divergence never silently overwrite obligations. Transferred performed work, follow-ups, notes, changes, and evidence remain provenance-bearing historical data. They must not fabricate local execution or advance recurrence. Exact retries are idempotent; changed immutable identities conflict; corrections append revisions. Re-export preserves original origin identity through relay workspaces.
