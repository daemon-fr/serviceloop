# V16 Service — UI/UX Implementation Reference v1.0

**Canonical presentation manual.** This owner-adopted reference defines visual hierarchy, component meaning, interaction, accessibility, light/dark behavior, and representative screens. It is not runtime certification.

The [source of truth](../../project/SOURCE_OF_TRUTH.md) and [baseline decisions](../../project/BASELINE_DECISIONS.md) govern business meaning. Current code establishes implementation. The manual does not authorize changes to persistence, privacy, recurrence, Dispatch, Recovery, reminders, Calendar, or scope.

The B051 identity cutover retains this presentation contract. The current implementation branch and evidence are recorded in [`../../project/IMPLEMENTATION_STATE.md`](../../project/IMPLEMENTATION_STATE.md). This manual is repository-relative and has no external repository dependency.

<a id="overview"></a>

# 1. How to use this reference

Start from a screen composition, then follow its action/input rows to component and interaction contracts. Local presentation overrides are explicit; visual guidance cannot change adopted business behavior. Romanian wording examples are translation proposals, not a claim that localization is implemented.

<a id="source"></a>

# 2. Current scope and authority

The adopted V16 Service product maps and owner amendments are authoritative. The current specialist contracts cover lifecycle, roles, native exchange, reports, persistence, reminders, Calendar, and quality. The unadopted alternate functional proposal is not combined with the adopted baseline.

This reference was authored as presentation guidance. Its static design/action inventory is not a compiler census, runtime test count, accessibility certification, or technician-pilot result. Keep those evidence boundaries explicit.

<a id="references"></a>

# 3. Reference mockups and direction

## Observations from the three retained reference images

The images share one visual language: pale neutral canvas; white surfaces; dark slate primary text; muted secondary text; deep teal commands; restrained outlines; modest corners; airy grouping; mint selected rows; amber Draft and blue Dispatched badges. Headers are confident but not oversized. Optional content can collapse. Review summaries use unboxed rows and dividers.

The selected Outbox uses a contextual bottom bar with count/Clear and Export/More. The editor has one principal Save draft action and an explicit not-sent fact. The native-file review explains assignment visibility and that a created file is not proof of receipt. The older fourth review image contained a retired standalone file suffix and has been removed; the current file identity is specified in the exchange contract.

The retained images are 618×1328 pixels and have no authoritative dp scale. Literal token choices in this manual are Android design values, not pixels relabelled as dp. OS chrome, device outlines, sample totals, and accidental line breaks are not requirements. Long names remain accessible even when summaries shorten them.

### Minimal justified adjustments

1. Meaningful controls need stronger boundaries than decorative cards. Keep hairlines for grouping; use control outlines where they carry meaning.
2. Large-text layouts grow and stack. Do not preserve side-by-side date/time at the cost of clipping. Names wrap fully; bottom regions never cover the last item.
3. Outbox checkbox selects; row body opens. A badge is not a button. A picker summary is not a text input.
4. Counts come from the same filtered/prepared records. First-export/version wording maps per Visit.
5. Dark mode uses neutral blue-slate surfaces and mint/teal actions, not color inversion, pure black everywhere, or translucent green panels.
...
<figure class="reference"><img src="references/mockup_outbox.png" alt="Outbox unselected visual reference"/><figcaption>M01 — Outbox unselected; textual contracts govern implementation.</figcaption></figure>
<figure class="reference"><img src="references/mockup_selected.png" alt="Outbox selected visual reference"/><figcaption>M02 — Outbox selected; textual contracts govern implementation.</figcaption></figure>
<figure class="reference"><img src="references/mockup_editor.png" alt="New Dispatch Visit visual reference"/><figcaption>M03 — New Dispatch Visit; textual contracts govern implementation.</figcaption></figure>

The PNGs are reference inputs only. This reference does not claim a rendered Android test.
...
**V16 Service decision:** This proposal uses48dp targets and separates checkbox selection from body navigation; a one-action selectable row has one semantics node.
...
**V16 Service decision:** Use headings, pane titles, roles, selected/state descriptions, live regions and deliberate merge boundaries; do not announce decorative marks as content.
...
**V16 Service decision:** Choose4.5:1 for all informative text,3:1 for meaningful control/selection/focus graphics,200% type test; use labels as redundant cues. Decorative borders/disabled commands explicitly excluded from claims.
...
**V16 Service decision:** Use the criteria as engineering targets, not an assertion that CSS pixels are Android dp or that a PDF is tagged/accessible by default.
...
**V16 Service decision:** One inset owner per layer, measured bottom bar, focused input and last row kept visible; do not hardcode fictional OS chrome from PNG.
...
**V16 Service decision:** Use600/840 width classes and compact-height handling; our360dp/1.3-scale form-pair limits are project design decisions, not mandatory platform breakpoints.
...
**V16 Service decision:** Keep record buffers keyed to record identity/version. Never equate remembered input with a confirmed durable save.
...
**V16 Service decision:** Preserve adopted Working/correction persistence and explicit general-form buffers; do not promise reboot recovery merely from rememberSaveable.
...
**V16 Service decision:** Show customer/site/equipment context with stable references; label major actions and expose relevant current states. No claimed field-productivity percentage.
...
**V16 Service decision:** Collapse optional notes/reference fields; keep required data, blockers and save failures visible. More contains named uncommon actions, not undiscoverable gestures.
...
**V16 Service decision:** Choose12dp intra-group/24dp inter-section and persistent labels near values; do not transfer unrelated study conversion results to V16 Service.
...
**V16 Service decision:** Use persistent visible field labels, including populated and error states; placeholders are examples only.
...
**V16 Service decision:** Provide deliberate neutral dark surfaces and full contrast, follow system preference, and test in bright/dim conditions; no outdoor-use guarantee.
...
**V16 Service decision:** Define complete role mapping plus semantic extensions. Stable V16 Service color rather than dynamic wallpaper color is our branding choice.
...
**V16 Service decision:** Retain existing108dp vector mark and monochrome reference, check safe central shape across masks; no fake app-controlled launcher tint.
...
**V16 Service decision:** Keep existing mark and solid background; no added decorative waiting interval or extra splash Activity.
...
**V16 Service decision:** Use available accessibility checks plus TalkBack, keyboard and rendered scaling tests; color calculations alone do not pass runtime access.
...
**V16 Service decision:** Use only selected static vectors, preserve licence notices and source pin; no bundled icon-font files or unrelated paid asset pack.
...
| Anatomy | 48dp Back slot; title; at most two 48dp actions. Root Home uses V16 Service eyebrow + Home. Root Work/Customers use title. Detail contains no duplicate heading below. |
...
| Accessibility | Back name “Back”; More name “More actions”. Decorative glyph in labelled action has null description. Do not read V16 Service twice. |
...
| Contract field | Specification |
| --- | --- |
| Purpose / exclusions | Retain established V16 Service mark; separate customer-report branding. |
| Anatomy | V16 Service text and existing loop/document mark for app identity. Business name/technician/logo only from saved report identity snapshot. |
...
| Accessibility | Launcher label V16 Service. Decorative inline brand icon hidden when wordmark present. Missing business name is a report blocker, not a fabricated logo. |
...
`SYNCED` · `color.infoInk` / `color.infoContainer` · glyph alias `link`. Actual managed local provider link; external changes do not update V16 Service.
...
**Romanian proposed title:** Bun venit. **Source symbol:** `V16ServiceApp / initialization`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** adopted/pending.
...
**Top-to-bottom composition:** App mark/V16 Service; proposition “Know what needs service. Record the work. Keep the history.”; local-storage explanation; Start empty; Import existing directory; Restore backup; local-storage help. No sample records, forced business setup or permission prompt.
...
**Romanian proposed title:** Acasă. **Source symbol:** `HomeScreen / RootScaffold`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Lucrări. **Source symbol:** `WorkScreen / DueServicesScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Căutare. **Source symbol:** `SearchScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Clienți. **Source symbol:** `CustomersScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Client. **Source symbol:** `CustomerDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare client. **Source symbol:** `CustomerEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Locație. **Source symbol:** `SiteDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare locație. **Source symbol:** `SiteEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Echipament. **Source symbol:** `EquipmentScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare echipament. **Source symbol:** `EquipmentEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Mutare echipament. **Source symbol:** `MoveEquipmentScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Plan de service. **Source symbol:** `PlanDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare plan de service. **Source symbol:** `PlanEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Șabloane de inspecție. **Source symbol:** `TemplateListScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Șablon. **Source symbol:** `TemplateDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare șablon. **Source symbol:** `TemplateEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Pregătire vizită. **Source symbol:** `NewVisitScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Vizită. **Source symbol:** `VisitDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Vizită în lucru. **Source symbol:** `VisitDetailScreen / InspectionScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Lucrare și listă de verificare. **Source symbol:** `InspectionScreen / QuestionBlock / ValueQuestion`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Constatare. **Source symbol:** `QuestionBlock / FieldEvidenceScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** adopted/pending; inline description implemented.
...
**Romanian proposed title:** Revizuire finalizare. **Source symbol:** `CompletionReviewScreen / CompletionLineCard`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Fișă de service finalizată. **Source symbol:** `FinalRecordScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Corectare și anulare fișă. **Source symbol:** `CorrectionScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Acțiune ulterioară. **Source symbol:** `FollowUpDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare acțiune ulterioară. **Source symbol:** `FollowUpEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Setări. **Source symbol:** `SettingsScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Identitate profesională și rapoarte. **Source symbol:** `BusinessProfileScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Effect / cancellation / guard:** Stage business logo, future explicit captures only; not V16 Service branding or historical rewrite.
...
**Romanian proposed title:** Mementouri. **Source symbol:** `ReminderSettingsScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Date și recuperare. **Source symbol:** `DataRecoveryScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Creare copie de siguranță. **Source symbol:** `BackupScreen(create)`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Verificare sau restaurare copie. **Source symbol:** `BackupScreen(inspect/restore)`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Export CSV. **Source symbol:** `CsvExportScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Import nomenclator. **Source symbol:** `CsvImportScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Confidențialitate, ajutor și aplicație. **Source symbol:** `No standalone route in pinned navigation`. **Source:** [docs/product/V16Service_Conceptual_App_Map_v0_1.md](../../../docs/product/V16Service_Conceptual_App_Map_v0_1.md). **Classification:** adopted/pending.
...
**Romanian proposed title:** Raport pentru client. **Source symbol:** `ReportPreviewScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Istoric. **Source symbol:** `HistoryScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Modificări nesalvate. **Source symbol:** `UnsavedChangesGuard / SaveStateBanner`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Selectare înregistrare. **Source symbol:** `DispatchSitePickerDialog / EquipmentSiteSelectorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Verificare dependențe. **Source symbol:** `LifecycleScreen / MoveEquipmentScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Fotografii. **Source symbol:** `FieldEvidenceScreen / CorrectionScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Contact și navigare. **Source symbol:** `CustomerDetailScreen / handoff`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Schimbare dată sau stare. **Source symbol:** `LifecycleScreen / VisitDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Închidere sau redeschidere acțiune. **Source symbol:** `FollowUpDetailScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Editare punct de verificare. **Source symbol:** `TemplateEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Piesă utilizată. **Source symbol:** `FieldEvidenceScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Permisiune notificări. **Source symbol:** `ReminderSettingsScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Progres operație fișier. **Source symbol:** `BackupScreen / CsvImportScreen / ReportPreviewScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Ștergere date locale. **Source symbol:** `EraseScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Posibil duplicat. **Source symbol:** `CsvImportScreen / ImportDispatchPackageScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented components / adopted variants.
...
**Romanian proposed title:** Instrumente coordonator. **Source symbol:** `DispatchSettings`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Identitate tehnician. **Source symbol:** `TechnicianIdentityScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Effect / cancellation / guard:** Create bounded .v16service and launch chooser; no receipt assertion.
...
**Romanian proposed title:** Tehnicieni. **Source symbol:** `DispatchTechniciansScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.

**Entry/exit:** use the corresponding source action links in this ledger; toolbar/system Back returns to the actual caller under P01/P04/P05. Root surfaces Home/Work/Customers preserve session context. Notification/file entries reconstruct their legitimate parent; they do not open unrelated records or replay committed actions.

**Top-to-bottom composition:** Directory list; Import .v16service; Add manually expansion with ID+name; same-ID rename review.

**Layout contract:** [L03](#l03). **Primary action:** Import .v16service. **Secondary/overflow:** Add manually; Keep existing/Update name in review.
...
**Accessible contract:** [C07](#c07) + [P19](#p19). **Copy:** `action_i_x03_a01` → Importă .v16service. **Test:** `T-I-X03-A01` applies success/cancel/blocked/retry to this exact effect.
...
**Romanian proposed title:** Echipe. **Source symbol:** `DispatchTeamsScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** De trimis. **Source symbol:** `DispatchOutboxScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare vizită de repartizat. **Source symbol:** `DispatchVisitEditorScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Alegere locație pentru repartizare. **Source symbol:** `DispatchSitePickerDialog`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Alegere echipe. **Source symbol:** `DispatchTeamPickerDialog`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editare lucrare repartizată. **Source symbol:** `DispatchWorkItemDialog`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Revizuire export. **Source symbol:** `DispatchExportReviewScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Import pachet de lucrări. **Source symbol:** `ImportDispatchPackageScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Layout contract:** [L09](#l09). **Primary action:** Apply N safe visits. **Secondary/overflow:** Choose .v16service; Create separate; Skip branch; View visits.
...
**I:X11-A01 · Choose .v16service file**
...
**Accessible contract:** [C07](#c07) + [P19](#p19). **Copy:** `action_i_x11_a01` → Alege fișierul .v16service. **Test:** `T-I-X11-A01` applies success/cancel/blocked/retry to this exact effect.
...
**Romanian proposed title:** Responsabilitate documentare. **Source symbol:** `DispatchVisitPanel`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Predare documentare. **Source symbol:** `DispatchVisitPanel(handoff-picker)`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Revizuire nume tehnician. **Source symbol:** `DispatchTechniciansScreen(technician-rename-dialog)`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Revizuire închidere/redeschidere. **Source symbol:** `DispatchOutboxScreen(dispatch-status-confirm)`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DispatchCoordinatorUiV2.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Integrare calendar. **Source symbol:** `CalendarSettingsScreen`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt](../../../app/src/main/java/com/v16studio/v16service/ui/V16ServiceApp.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Calendarul vizitei. **Source symbol:** `VisitDetailScreen(visit-calendar)`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
**Effect / cancellation / guard:** Actual event ID handoff; external editing does not change V16 Service.
...
**Romanian proposed title:** Notificare. **Source symbol:** `Reminder settings and reminder delivery`. **Source:** [docs/project/IMPLEMENTATION_STATE.md](../../../docs/project/IMPLEMENTATION_STATE.md). **Classification:** external/system-owned shell; app-owned content/handoff.
...
**Romanian proposed title:** Recuperare restricționată. **Source symbol:** `DataRecoveryScreen / V16ServiceApp`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt](../../../app/src/main/java/com/v16studio/v16service/ui/Stage4Ui.kt). **Classification:** implemented / adopted gaps recorded.
...
**Romanian proposed title:** Editor text extins. **Source symbol:** `LongTextEditor`. **Source:** [app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt](../../../app/src/main/java/com/v16studio/v16service/ui/DailyOperationsUi.kt). **Classification:** implemented / adopted gaps recorded.
...
1. Business/display name and optional business logo within96×48pt fit bounds; never the V16 Service brand in place of the issuer. Logo slot is adopted/pending where not yet in the report model. Technician name and supplied contact follow. Missing optional data is omitted or labelled Not recorded where it affects identification.
...
| clipboard_private | Copied text may be visible outside V16 Service. It is not a backup. | Textul copiat poate fi vizibil în afara V16 Service. Nu este o copie de siguranță. | Shared app-owned copy |
...
| file_wrong_format | This is not a supported V16 Service file. | Acesta nu este un fișier V16 Service acceptat. | Shared app-owned copy |
| file_newer | This file needs a compatible newer V16 Service version. | Acest fișier necesită o versiune V16 Service mai nouă, compatibilă. | Shared app-owned copy |
...
| restore_title | Replace all local V16 Service data? | Înlocuiești toate datele V16 Service locale? | Shared app-owned copy |
...
| erase_title | Erase this device’s V16 Service data? | Ștergi datele V16 Service de pe acest dispozitiv? | Shared app-owned copy |
...
| external_return | Returned to V16 Service. No contact outcome was recorded automatically. | Ai revenit în V16 Service. Nu s-a înregistrat automat niciun rezultat al contactării. | Shared app-owned copy |
...
| package_plaintext | The .v16service file contains readable customer, site and assignment information. Share it only with intended recipients. | Fișierul .v16service conține informații lizibile despre clienți, locații și repartizări. Distribuie-l doar destinatarilor intenționați. | Shared app-owned copy |
...
| calendar_oneway | V16 Service remains authoritative. Calendar edits do not change V16 Service. | V16 Service rămâne sursa de referință. Modificările din calendar nu schimbă V16 Service. | Shared app-owned copy |
...
| notification_summary_title | V16 Service work summary | Rezumat lucrări V16 Service | Shared app-owned copy |
| notification_appointment_title | Upcoming appointment | Programare apropiată | Shared app-owned copy |
| notification_appointment_body | Appointment at %1$s. Open V16 Service for details. | Programare la %1$s. Deschide V16 Service pentru detalii. | Shared app-owned copy |
| notification_test | V16 Service test notification | Notificare de test V16 Service | Shared app-owned copy |
...
| action_i_x03_a01 | Import .v16service | Importă .v16service | I:X03-A01 |
...
| action_i_x11_a01 | Choose .v16service file | Alege fișierul .v16service | I:X11-A01 |
...
`V16Service_UI_UX_Implementation_Reference_v1_0.md` is canonical narrative. The HTML is generated from it with embedded reference images and the same component specimens; reading-edition navigation/CSS are presentation only. `V16Service_UI_Tokens_v1_0.json` contains literal roles, geometry, Material mapping, semantic adapters and calculated pairs. `V16Service_UI_Coverage_v1_0.csv` contains every enumerated group/variant/input with source, status, location, component, behavior, accessibility, copy and test references. `V16Service_UI_Audit_v1_0.json` and `V16Service_Source_Manifest_v1_0.json` record the package checks and evidence boundary. None is committed to the repository.
